...
CAccount * CChat::FindChatName( const TCHAR * pszName ) // static
{
	for ( int i=0; i<g_World.m_Accounts.GetCount(); i++)
	{
		if ( ! g_World.m_Accounts[i]->m_sChatName.CompareNoCase( pszName ))
			return( g_World.m_Accounts[i] );
	}
	return( NULL );
}

void CChat::Event( CClient * pClient, const char * pszLang, const NCHAR * pszText, int len) // Text from a client
{
	...
	TCHAR text[2048];
	if ( len > COUNTOF(text)) len = COUNTOF(text)-1;
	CvtNUNICODEToSystem( text, pszText, len+1 );
...
not_in_a_channel:
		...
		pChannel->MemberTalk(pMe, szMsg, pszLang);
		break;
	};
	...
	case 'z':	// z = emote
		if (!pChannel)
			goto not_in_a_channel;
		pChannel->Emote(pMe->GetChatName(), szMsg, pszLang);
		break;
	};
}
...
void CChat::DoCommand(CChatChanMember * pBy, const TCHAR * szMsg)
{
	static const TCHAR * szCmd_Chat[] =
	{
		"ALLKICK",
		"BC",
		"BCALL",
	...
	switch ( FindTableSorted( pszCommand, szCmd_Chat, COUNTOF(szCmd_Chat)))
	{
	case 0: // "ALLKICK"
	{
		if (!pChannel)
...
need_gm_privs:
	...
	default:
	{
		...
		CGString sFrom;
		DecorateName(sFrom, NULL, true);
		pBy->SendChatMsg(CHATMSG_PlayerTalk, sFrom, sMsg);
		return;
	}
	}
}
...
void CChat::WhereIs(CChatChanMember * pBy, const TCHAR * pszName ) const
{
	...
	for ( ; pClient; pClient = pClient->GetNext())
	{
		...
		if (! pClient->IsChatActive() || !pClient->GetChannel())
			sMsg.Format( "%s is not currently in a conference.", pszName);
		else
			sMsg.Format( "%s is in conference '%s'.", pszName, pClient->GetChannel()->GetName());
		CGString sFrom;
		DecorateName(sFrom, NULL, true);
		pBy->SendChatMsg( CHATMSG_PlayerTalk, sFrom, sMsg );
		return;
...
void CChat::Broadcast(CChatChanMember * pFrom, const char * pszText, const char * pszLang, bool fOverride)
{
	CClient * pClient = g_Serv.GetClientHead();
	for ( ; pClient; pClient = pClient->GetNext())
	{
		...
		if ( fOverride || ( ! fOverride && pClient->IsReceivingAllowed()))
		{
			...
			pClient->SendChatMsg(CHATMSG_PlayerTalk, sName, pszText, pszLang);
		}
	}
}

void CChat::CreateJoinChannel(CChatChanMember * pByMember, const char * pszName, const char * pszPassword)
{
...
void CChatChannel::Emote(const char * pszBy, const char * pszMsg, const char * pszLang)
{
	if (HasVoice(pszBy))
		Broadcast(CHATMSG_PlayerEmote, pszBy, pszMsg, pszLang);
	else
...
void CChatChannel::KickMember(CChatChanMember *pByMember, CChatChanMember * pMember )
{
	...
	const TCHAR * pszName = pMember->GetChatName();
...
void CChatChannel::MemberTalk(CChatChanMember * pByMember, const char * pszText, const char * pszLang)
{
	// Do I have a voice?
	...
	Broadcast(CHATMSG_PlayerTalk, pByMember->GetChatName(), pszText, pszLang);
}
...
void CChatChannel::Broadcast(CHATMSG_TYPE iType, const char * pszName, const char * pszText, const char * pszLang, bool fOverride )
{
	CGString sName;
	...
	for (int i = 0; i < m_Members.GetCount(); i++)
	{
		...
		if (!m_Members[i]->IsIgnoring(pszName))
			m_Members[i]->SendChatMsg(iType, sName, pszText, pszLang);
...
int CChatChanMember::FindIgnoringIndex(const TCHAR * pszName) const
{
...
void CChatChanMember::SendChatMsg(CHATMSG_TYPE iType, const TCHAR * pszName1, const TCHAR * pszName2, const char * pszLang)
{
	DEBUG_CHECK( IsChatActive());
	GetClient()->addChatSystemMessage(iType, pszName1, pszName2, pszLang);
}
...
const TCHAR * CChatChanMember::GetChatName() const
{

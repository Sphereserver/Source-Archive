...
CItem * CItem::CreateDupeItem( const CItem * pItem, CChar * pSrc )	// static
{
	// Dupe this item.
	if ( pItem == NULL )
		return( NULL );
	...
	case CIV_DUPE:
		{
			...
			while ( iCount-- )
			{
				CItem::CreateDupeItem( this, dynamic_cast <CChar *> (pSrc) )->MoveNearObj( this, 1 );
			}
		}
		break;
	case CIV_EQUIP:
	...
	if ( pChar )
	{
		...
		if ( pChar->m_pPlayer )
		{
			...
			for ( i=0; dwPlot; i++, dwPlot >>= 1 )
			{
				...
				{
					DEBUG_ERR(( "0%x '%s' has unhandled [PLOTITEM %d]" DEBUG_CR, (DWORD) pChar->GetUID(), pChar->GetName(), i ));

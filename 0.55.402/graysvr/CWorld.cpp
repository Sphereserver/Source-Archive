...
{
	Broadcast( g_Cfg.GetDefaultMsg( "server_worldsave" ) );
...
bool CWorld::LoadFile( LPCTSTR pszLoadName, bool fError ) // Load world from script
{
	...
	while ( s.FindNextSection())
	{
		...
		{
			g_Log.CatchEvent( &e, "Load Exception line %d " GRAY_TITLE " is UNSTABLE!" DEBUG_CR, s.GetContext().m_iLineNum );
		}
		catch (...)
		{
			g_Log.CatchEvent( NULL, "Load Exception line %d " GRAY_TITLE " is UNSTABLE!" DEBUG_CR, s.GetContext().m_iLineNum );
		}
	}
...
bool CWorld::LoadAll( LPCTSTR pszLoadName ) // Load world from script
{
	...
	for ( int j=0; j<SECTOR_QTY; j++ )
	{
		...
		if ( iCount > 1024 )	// g_Cfg.m_iMaxItemComplexity ?
		{

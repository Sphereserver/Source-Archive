...
void CClient::Event_VendorBuy( CGrayUID uidVendor, const CEvent * pEvent )
{
	...
	for ( ;i<nItems;i++)
	{
		...
		if ( ! iPrice ||
			...
			pEvent->VendorBuy.m_item[i].m_amount < 1) //Torfo
		{
			// fSoldout = true;
			// continue;
...
soldout:
		//Torfo start
		//teot fix
		if (pEvent->VendorBuy.m_item[i].m_amount < 1)
		{
			DEBUG_ERR(("CHEATER ALERT: %s used invalid buy packet" DEBUG_CR, (LPCTSTR) m_pChar->GetName()));
		}
		//Torfo end

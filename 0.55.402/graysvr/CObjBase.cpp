...
*/
	...
	case OC_TAGAT:
	/*	{
			g_Log.Event( LOGL_ERROR, "Our argument is: %s", pszKey);
			pszKey += 5;				// eat the 'TAGAT'
			g_Log.Event( LOGL_ERROR, "After 5 char eat its %s", pszKey);
			if ( *pszKey == '.' )		// do we have an argument?
			{
				g_Log.Event( LOGL_ERROR, "We have an argument! %s", pszKey);
				SKIP_SEPERATORS( pszKey );
				g_Log.Event( LOGL_ERROR, "Seperators skipped: %s ....", pszKey);
				int iQty = Exp_GetVal( pszKey ); 
				g_Log.Event( LOGL_ERROR, "exp_getVal() is %s", iQty);
				CVarDefBase * pTagAt = m_TagDefs.GetAt( iQty );
				if ( !pTagAt )
					return( false );	// tryig to get non-existant tag
				SKIP_SEPERATORS( pszKey );
				g_Log.Event( LOGL_ERROR, "Speprators skipped again: %s", pszKey);
				if ( ! *pszKey )	
				{
					g_Log.Event( LOGL_ERROR, "GetKey(): %s", pTagAt->GetKey());
					g_Log.Event( LOGL_ERROR, "GetValStr(): %s", pTagAt->GetValStr());
					sVal.Format( "%s=%s", (LPCTSTR) pTagAt->GetKey(), (LPCTSTR) pTagAt->GetValStr() );
					return( true );
				}
				g_Log.Event( LOGL_ERROR, "We have key/val arg: %s", pTagAt->GetKey());
...
bool CObjBase::r_Verb( CScript & s, CTextConsole * pSrc ) // Execute command from script
{
	...
	switch (index)
	{
	case OV_CLEARTAGS: // args - null; clears all the tags on the object
		m_TagDefs.ClearKeys();
		break;

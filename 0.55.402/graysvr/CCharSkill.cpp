...
bool CChar::Skill_MakeItem_Success()
{
	...
	else
	{
		...
		case 0:
			// Shoddy quality
			sMakeMsg.Format(g_Cfg.GetDefaultMsg("makesuccess_1"));
			quality = Calc_GetRandVal( 25 ) + 1;
			break;
		case 1:
			// Poor quality
			sMakeMsg.Format(g_Cfg.GetDefaultMsg("makesuccess_2"));
			quality = Calc_GetRandVal( 25 ) + 26;
			break;
		case 2:
			// Below average quality
			sMakeMsg.Format(g_Cfg.GetDefaultMsg("makesuccess_3"));
		...
		case 4:
			...
			sMakeMsg.Format(g_Cfg.GetDefaultMsg("makesuccess_4"));
			quality = Calc_GetRandVal( 25 ) + 126;
			break;
		case 5:
			// Excellent quality
			sMakeMsg.Format(g_Cfg.GetDefaultMsg("makesuccess_5"));
		...
		if (( iSkillLevel > 999 && ( quality > 175 )) && ( !g_Cfg.IsSetOF(OF_NoItemNaming)))
		{
			// A GM made this, and it is of high quality
			CGString csNewName;
			csNewName.Format( g_Cfg.GetDefaultMsg( "grandmaster_mark" ), (LPCTSTR) pItem->GetName(), (LPCTSTR) GetName());
...
int CChar::Skill_Mining( SKTRIG_TYPE stage )
{
	...
	if ( pResBit == NULL )
	{
		SysMessageDefault( "mining_1" );
		return( -SKTRIG_QTY );
	}
	if ( pResBit->GetAmount() == 0 )
	{
		SysMessageDefault( "mining_2" );
		return( -SKTRIG_QTY );
	}
	if ( stage == SKTRIG_FAIL )
		return 0;
...
int CChar::Skill_Fishing( SKTRIG_TYPE stage )
{
	...
	if ( pResBit == NULL )
	{
		SysMessageDefault( "fishing_1" );
		return( -SKTRIG_QTY );
	}
	if ( pResBit->GetAmount() == 0 )
	{
		SysMessageDefault( "fishing_2" );
		return( -SKTRIG_QTY );
	}
...
int CChar::Skill_Lumberjack( SKTRIG_TYPE stage )
{
	...
	if ( m_Act_p.m_x == 0xFFFF )
	{
		SysMessageDefault( "lumberjacking_6" );
		return( -SKTRIG_QTY );
	}
	...
	if ( pResBit == NULL )
	{
		SysMessageDefault( "lumberjacking_1" );
		return( -SKTRIG_QTY );
	}
	if ( pResBit->GetAmount() == 0 )
	{
		SysMessageDefault( "lumberjacking_2" );
		return( -SKTRIG_QTY );
	}
...
int CChar::Skill_Provocation( SKTRIG_TYPE stage )
{
	...
	{
		SysMessage( "You can not provoke players!" );
		return -SKTRIG_ABORT;
	}
	if ( stage == SKTRIG_FAIL )
	{
...
int CChar::Skill_Taming( SKTRIG_TYPE stage )
{
	...
	if ( ! CanSeeLOS( pChar ) )
	{
		SysMessage( "You can not see the creature." );
		return -SKTRIG_QTY;
	}
	UpdateDir( pChar );
...
bool CChar::Skill_Start( SKILL_TYPE skill, int iDifficulty )
{
	...
	if ( skill != SKILL_NONE )
	{
		...
		if ( !g_Cfg.IsSetOF( OF_Skill_DiffFirst ) )
		{

...
bool CMailSMTP::SendMail( LPCTSTR pszDstMail, LPCTSTR pszSrcMail, CGObArray< CGString * > * pStrings )
{
	...
	int i;
	for ( i=0;sm_Lines[i] != NULL;i++)
	{
		CGString sMsg;
		switch ( i )
		{
		case 0:	sMsg.Format( sm_Lines[0], (LPCTSTR) g_Serv.m_ip.GetAddrStr()); break;
		case 1: sMsg.Format( sm_Lines[1], pszSrcMail ); break;
		case 2: sMsg.Format( sm_Lines[2], pszDstMail ); break;
		default:
			sMsg = sm_Lines[i];
			break;
		}
...
bool CMailPOP::Init()
{
	...
	if ( ! Bind( PORT_SMTP ))
	{
		...
		// g_Log.Event( LOGL_ERROR, "Unable to bind listen socket port %d - Error code: %i" DEBUG_CR, m_ip.GetPort(), bcode );

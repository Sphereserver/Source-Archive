...
void CChar::Guild_Resign( MEMORY_TYPE MemType )
{
	...
	if ( pMember->IsPrivMember())
	{
		...
		if ( pMemFight )
		{
			...
			ASSERT(pMyStone);
			SysMessagef( g_Cfg.GetDefaultMsg( "msg_guildresign" ), (LPCTSTR) pMyStone->GetTypeName() );
		}
	}
	...
	if ( ! IsPriv( PRIV_PRIV_NOSHOW ))
	{
		...
		if ( IsPriv( PRIV_GM ))
		{
			switch ( GetPrivLevel())
			{
				...
				case PLEVEL_GM: return( "GM " );
				case PLEVEL_Dev: return( "Dev " );
				case PLEVEL_Admin: return( "Admin " );
				case PLEVEL_Owner: return( "Owner " );
			}
		}
		//Torfo end
	}
...
void CChar::Noto_Murder()
{
	...
	if ( Noto_IsMurderer())
	{
		SysMessageDefault( "msg_murderer" );
	}
...
void CChar::Noto_Criminal()
{
	...
	if ( IsPriv(PRIV_GM))
		return;
	SysMessageDefault( "msg_criminal" );
...
#define NOTO_FACTOR 300
	...
	SysMessagef( g_Cfg.GetDefaultMsg( "msg_noto_change_0" ),
		( iDelta < 0 ) ? g_Cfg.GetDefaultMsg( "msg_noto_change_lost" ) : g_Cfg.GetDefaultMsg( "msg_noto_change_gained" ),
...
void CChar::Noto_ChangeNewMsg( int iPrvLevel )
{
	if ( iPrvLevel != Noto_GetLevel())
	{
		...
		SysMessagef( g_Cfg.GetDefaultMsg( "msg_noto_gettitle" ), (LPCTSTR) Noto_GetTitle());
	}
}
...
void CChar::OnNoticeCrime( CChar * pCriminal, const CChar * pCharMark )
{
	...
	{
		Speak( g_Cfg.GetDefaultMsg( "npc_generic_crim" ) );
	}
...
bool CChar::CheckCrimeSeen( SKILL_TYPE SkillToSee, CChar * pCharMark, const CObjBase * pItem, LPCTSTR pAction )
{
	...
	while (true)
	{
		...
		if ( pAction != NULL )
		{
			if ( pCharMark == NULL )
			{
				sMsg.Format( g_Cfg.GetDefaultMsg( "msg_younotice_1" ), (LPCTSTR) GetName(), pAction, (LPCTSTR) pItem->GetName());
			}
			else
			{
				sMsg.Format( g_Cfg.GetDefaultMsg( "msg_younotice_2" ),
					(LPCTSTR) GetName(), pAction, fYour ? g_Cfg.GetDefaultMsg( "msg_younotice_your" ) : (LPCTSTR) pCharMark->GetName(),
					fYour ? "" : g_Cfg.GetDefaultMsg( "msg_younotice_s" ),
...
bool CChar::Skill_Snoop_Check( const CItemContainer * pItem )
{
	...
	case IT_SHIP_HOLD:
		...
		{
			SysMessage( _TEXT( g_Cfg.GetDefaultMsg( "itemuse_hatch_fail" )));
			return( true );
		}
		break;
	case IT_EQ_BANK_BOX:
...
int CChar::Skill_Snooping( SKTRIG_TYPE stage )
{
	...
	if ( ! CanTouch( pCont ))
	{
		SysMessageDefault( "snooping_reach" );
		return( -SKTRIG_QTY );
	}

	if ( GetTopDist3D( pCharMark ) > 2 )
	{
		SysMessageDefault( "snooping_mark" );
		return( -SKTRIG_QTY );
	}
	...
	if ( CheckCrimeSeen( SKILL_SNOOPING, pCharMark, pCont, (stage == SKTRIG_FAIL)? g_Cfg.GetDefaultMsg( "snooping_your" ) : g_Cfg.GetDefaultMsg( "snooping_someone" ) ))
	{
...
int CChar::Skill_Stealing( SKTRIG_TYPE stage )
{
	...
	if ( pItem == NULL )	// on a chars head ? = random steal.
	{
		...
		if ( pCharMark == NULL )
		{
			SysMessageDefault( "stealing_nothing" );
			return( -SKTRIG_QTY );
		}
		...
		{
cantsteal:
			SysMessageDefault( "stealing_empty" );
			return( -SKTRIG_QTY );
		}
	...
	if ( pContainer ) 
	{
		...
		if ( pCorpse )
		{
			SysMessageDefault( "stealing_corpse" );
			return( -SKTRIG_ABORT );
		}
	}
   ...
   if ( pCItem )
   {
	   ...
	   {
		   SysMessageDefault( "stealing_gameboard" );
		   return( -SKTRIG_ABORT );
	   }
	   if ( pCItem->GetType() == IT_EQ_TRADE_WINDOW )
	   {
		   SysMessageDefault( "stealing_trade" );
		   return( -SKTRIG_ABORT );
	   }
   }
	...
	{
		SysMessageDefault( "stealing_pickpocket" );
		return -SKTRIG_QTY;
	}
	...
	if ( ! CanTouch( pItem ))
	{
		SysMessageDefault( "stealing_reach" );
		return( -SKTRIG_ABORT );
	}
	if ( ! CanMove( pItem ) ||
		! CanCarry( pItem ))
	{
		SysMessageDefault( "stealing_heavy" );
		return( -SKTRIG_ABORT );
	}
	if ( ! IsTakeCrime( pItem, & pCharMark ))
	{
		SysMessageDefault( "stealing_noneed" );
	...
	{
		SysMessageDefault( "stealing_stop" );
		return( -SKTRIG_QTY );
	}
	...
	if ( CheckCrimeSeen( SKILL_STEALING, pCharMark, pItem, (stage == SKTRIG_FAIL)? g_Cfg.GetDefaultMsg( "stealing_your" ) : g_Cfg.GetDefaultMsg( "stealing_someone" ) ))
	{
...
void CChar::CallGuards( CChar * pCriminal )
{
	...
	if ( OnTrigger( CTRIG_CallGuards, pCriminal ) == TRIGRET_RET_TRUE )
		return;
	...
	if ( pGuard == NULL )
	{
		...
		if ( pVar )
		{
			rid = RESOURCE_ID( RES_CHARDEF, pVar->GetValNum());
		}
		else
		{
...
void CChar::OnHarmedBy( CChar * pCharSrc, int iHarmQty )
{
	...
	if ( IsClient() )
	{
		StatFlag_Set( STATF_War ); //attacked Chars change in warmode
...
bool CChar::OnAttackedBy( CChar * pCharSrc, int iHarmQty, bool fCommandPet )
{
	...
	if ( pCharSrc->IsStatFlag( STATF_INVUL ) && ! pCharSrc->IsPriv( PRIV_GM ))
	{
		...
		pCharSrc->SysMessageDefault( "msg_magic_block" );
		return( false );
	}
...
//	CCharBase * pCharDef = Char_GetDef();
//	ASSERT(pCharDef);
//	signed int iQty = ( iDefenseTotal / 100 ) + pCharDef->m_defense + m_ModAr;
//	if ( iQty < 0 )
//		iQty = 0;
//	iQty -= pCharDef->m_defense;
//	return( iQty );
...
int CChar::OnTakeDamageHitPoint( int iDmg, CChar * pSrc, DAMAGE_TYPE uType )
{
		...
		! (uType & (DAMAGE_GOD|DAMAGE_ELECTRIC)))
	{
		...
		if ( pShield != NULL && Skill_UseQuick( SKILL_PARRYING, Calc_GetRandVal((pSrc!=NULL) ? (pSrc->Skill_GetBase(SKILL_TACTICS)/10) : 100 )))
		{
			...
			{
				SysMessageDefault( "combat_parry" );	
			}
		...
		! (uType & (DAMAGE_GENERAL|DAMAGE_GOD)))
	{
		...
		{
			"combat_hit_head1s", "combat_hit_head1o",
			"combat_hit_head2s", "combat_hit_head2o",
			"combat_hit_head3s", "combat_hit_head3o",
		};
		static LPCTSTR const sm_Hit_Head2[][2] =
		{
			"combat_hit_head4s", "combat_hit_head4o",
			"combat_hit_head5s", "combat_hit_head5o",
			"combat_hit_head6s", "combat_hit_head6o",
		};
		static LPCTSTR const sm_Hit_Chest1[][2] =
		{
			"combat_hit_chest1s", "combat_hit_chest1o",
			"combat_hit_chest2s", "combat_hit_chest2o",
			"combat_hit_chest3s", "combat_hit_chest3o",
		};
		static LPCTSTR const sm_Hit_Chest2[][2] =
		{
			"combat_hit_chest4s", "combat_hit_chest4o",
			"combat_hit_chest5s", "combat_hit_chest5o",
			"combat_hit_chest6s", "combat_hit_chest6o",
		};
		static LPCTSTR const sm_Hit_Arm[][2] =
		{
			"combat_hit_arm1s", "combat_hit_arm1o",
			"combat_hit_arm2s", "combat_hit_arm2o",
			"combat_hit_arm3s", "combat_hit_arm3o",
		};
		static LPCTSTR const sm_Hit_Legs[][2] =
		{
			"combat_hit_legs1s", "combat_hit_legs1o",
			"combat_hit_legs2s", "combat_hit_legs2o",
			"combat_hit_legs3s", "combat_hit_legs3o",
		};
		static LPCTSTR const sm_Hit_Hands[][2] =	// later include exclusion of left hand if have shield
		{
			"combat_hit_hand1s", "combat_hit_hand1o",
			"combat_hit_hand2s", "combat_hit_hand2o",
			"combat_hit_hand3s", "combat_hit_hand3o",
		};
		static LPCTSTR const sm_Hit_Neck1[2] =
		{
			"combat_hit_neck1s", "combat_hit_neck1o",
		};
		static LPCTSTR const sm_Hit_Neck2[2] =
		{
			"combat_hit_neck2s", "combat_hit_neck2o",
		};
		static LPCTSTR const sm_Hit_Back[2] =
		{
			"combat_hit_back1s", "combat_hit_back1o",
		};
		static LPCTSTR const sm_Hit_Feet[2] =
		{
			"combat_hit_feet1s", "combat_hit_feet1o",
		};
		...
		if ( pSrc != this )
		{
			...
			{
				SysMessagef( g_Cfg.GetDefaultMsg( ppMsg[0] ), ( pSrc == NULL ) ? "It" : (LPCTSTR) pSrc->GetName());
			}
			if ( pSrc != NULL && pSrc->IsPriv(PRIV_DETAIL))
			{
				pSrc->SysMessagef( g_Cfg.GetDefaultMsg( ppMsg[1] ), (LPCTSTR) GetName());
			}
		}
	}
...
void CChar::Memory_Fight_Retreat( CChar * pTarg, CItemMemory * pFight )
{
	...
	SysMessagef( fCowardice ?
		g_Cfg.GetDefaultMsg( "msg_coward_1" ) :
		g_Cfg.GetDefaultMsg( "msg_coward_2" ), (LPCTSTR) pTarg->GetName());
...
void CChar::Memory_Fight_Start( const CChar * pTarg )
{
	...
	if ( GetTopSector()->GetCharComplexity() < 7 )
	{
		// too busy for this.
		CGString sMsgThem;
		sMsgThem.Format( g_Cfg.GetDefaultMsg("combat_attacko"), (LPCTSTR) GetName(), (LPCTSTR) pTarg->GetName());
	...
	{
		CGString sMsgYou;
		sMsgYou.Format( g_Cfg.GetDefaultMsg("combat_attacks"), (LPCTSTR) GetName());
...
//		return( WAR_SWING_INVALID );
	...
	if ( m_pArea->IsFlag(REGION_FLAG_SHIP) )
	{
		...
		{
			SysMessage("You cannot hit character outside the ship.");
...
// Torfo end
	...
	if ( skill == SKILL_ARCHERY )
	{
		...
		{
			SysMessageDefault( "itemuse_bow_shield" );
		...
		if ( dist < iMinDist )
		{
			...
			SysMessageDefault( "combat_arch_tooclose" );
		...
		if ( AmmoID )
		{
			...
			{
				SysMessageDefault( "combat_arch_noammo" );
				Skill_Start( SKILL_NONE );
				return( WAR_SWING_INVALID );
			}
		}
	...
	if ( m_Act_Difficulty < 0 )		// if not changed within trigger
	{
		...
		if ( IsPriv(PRIV_DETAIL))
		{
			SysMessagef( g_Cfg.GetDefaultMsg("combat_misss"), (LPCTSTR) pCharTarg->GetName());
		}
		if ( pCharTarg->IsPriv(PRIV_DETAIL))
		{
			pCharTarg->SysMessagef( g_Cfg.GetDefaultMsg("combat_misso"), (LPCTSTR) GetName());
		}
	...
	{
		pCharTarg->SysMessageDefault( "healing_interrupt" );

...
void CChar::NPC_OnPetCommand( bool fSuccess, CChar * pMaster )
{
	...
	if ( NPC_CanSpeak())
	{
		Speak( fSuccess? g_Cfg.GetDefaultMsg( "npc_pet_success" ) : g_Cfg.GetDefaultMsg( "npc_pet_failure" ) );
	}
	else
	{
...
bool CChar::NPC_OnHearPetCmd( LPCTSTR pszCmd, CChar * pSrc, bool fAllPets )
{
	...
	case PC_ATTACK:
do_attack:
		pTargPrompt = g_Cfg.GetDefaultMsg( "npc_pet_targ_att" );
		fMayBeCrime = true;
		break;
	case PC_CASH:
		// Drop all we have.
			...
			return( false );
		{
			...
			{
				sMsg.Format( g_Cfg.GetDefaultMsg( "npc_pet_getgold_1" ),
			...
			{
				sMsg.Format( g_Cfg.GetDefaultMsg( "npc_pet_getgold_2" ),
...
do_release:
	...
	case PC_DROP:
		...
		// "GIVE" ?
		{
			...
			if ( NPC_CanSpeak())
			{
				Speak( g_Cfg.GetDefaultMsg( "npc_pet_carrynothing") );
			}
			return( true );
		}
		break;
	case PC_DROP_ALL:
		...
		break;
	case PC_FETCH:
		pTargPrompt = g_Cfg.GetDefaultMsg( "npc_pet_targ_fetch" );
		break;
	case PC_FOLLOW:
		pTargPrompt = g_Cfg.GetDefaultMsg( "npc_pet_targ_follow" );
		break;
	case PC_FRIEND:
		pTargPrompt = g_Cfg.GetDefaultMsg( "npc_pet_targ_friend" );
		break;
	case PC_GO:
		pTargPrompt = g_Cfg.GetDefaultMsg( "npc_pet_targ_go" );
		fTargAllowGround = true;
		break;
	case PC_GUARD:
		pTargPrompt = g_Cfg.GetDefaultMsg( "npc_pet_targ_guard" );
		fMayBeCrime = true;
		break;
	case PC_KILL:
		goto do_attack;
	case PC_PRICE:
		if ( ! NPC_IsVendor())
			return( false );
		pTargPrompt = g_Cfg.GetDefaultMsg( "npc_pet_setprice" );
		break;
	case PC_RELEASE:
		goto do_release;
	case PC_SPEAK:
	...
	case PC_STATUS:
		if ( NPC_CanSpeak())
		{
			...
			if ( NPC_IsVendor())
			{
				...
				if ( iWage )
				{
					sMsg.Format( "I have %d gold on hand. "				//g_Cfg.GetDefaultMsg( "npc_pet_status_gold_1" ) 
							 "for which I will work for %d more days. " //g_Cfg.GetDefaultMsg( "npc_pet_status_gold_2" ) 
							 "I have %d items to sell.",				//g_Cfg.GetDefaultMsg( "npc_pet_status_gold_3" ),
						pBank->m_itEqBankBox.m_Check_Amount,
						pBank->m_itEqBankBox.m_Check_Amount / iWage,
						pCont->GetCount());
				}
				...
				{
					sMsg.Format( "I have %d gold on hand. "				//g_Cfg.GetDefaultMsg( "npc_pet_status_gold_4" ) 
							 "I restock to %d gold in %d minutes or every %d minutes. "	//g_Cfg.GetDefaultMsg( "npc_pet_status_gold_5" ) 
							 "I have %d items to sell.",				//g_Cfg.GetDefaultMsg( "npc_pet_status_gold_6" ),
						pBank->m_itEqBankBox.m_Check_Amount,
			...
			{
				sMsg.Format( g_Cfg.GetDefaultMsg( "npc_pet_days_left" ),
					pBank->m_itEqBankBox.m_Check_Amount / iWage );
			}
			else
			{
...
do_stay:
	...
	case PC_STOP:
		goto do_stay;
	case PC_TRANSFER:
		pTargPrompt = g_Cfg.GetDefaultMsg( "npc_pet_targ_transfer" );
		break;
	...
	case PC_INVENTORY:
		...
		if ( ! NPC_IsVendor())
			return( false );
		Speak( g_Cfg.GetDefaultMsg( "npc_pet_items_sell" ) );
	...
	case PC_RECIEVED:
		if ( ! NPC_IsVendor())
			return( false );
		Speak( g_Cfg.GetDefaultMsg( "npc_pet_items_buy" ) );
	...
	case PC_SAMPLES:
		if ( ! NPC_IsVendor())
			return( false );
		Speak( g_Cfg.GetDefaultMsg( "npc_pet_items_sample" ) );
...
bool CChar::NPC_CheckHirelingStatus()
{
	...
	{
		Speak( g_Cfg.GetDefaultMsg( "npc_pet_wage_cost" ), iWage );

		CChar * pOwner = NPC_PetGetOwner();
		if ( pOwner )
		{
			Speak( g_Cfg.GetDefaultMsg( "npc_pet_hire_timeup" ) );
...
void CChar::NPC_OnHirePayMore( CItem * pGold, bool fHire )
{
	...
	CGString sMsg;
	sMsg.Format( g_Cfg.GetDefaultMsg( "npc_pet_hire_time" ), pBank->m_itEqBankBox.m_Check_Amount / iWage );
	Speak( sMsg );
}
...
bool CChar::NPC_OnHirePay( CChar * pCharSrc, CItemMemory * pMemory, CItem * pGold )
{
	...
	if ( IsStatFlag( STATF_Pet ))
	{
		...
		{
			Speak( g_Cfg.GetDefaultMsg( "npc_pet_employed" ) );
			return false;
		}
	}
	else
	{
		...
		if ( ! iWage )
		{
			Speak( g_Cfg.GetDefaultMsg( "npc_pet_not_for_hire" ) );
			return false;
		}
		if ( pGold->GetAmount() < iWage )
		{
			Speak( g_Cfg.GetDefaultMsg( "npc_pet_not_enough" ) );
			return false;
		}
		// NOTO_TYPE ?
		if ( pMemory->IsMemoryTypes( MEMORY_FIGHT|MEMORY_HARMEDBY|MEMORY_IRRITATEDBY ))
		{
			Speak( g_Cfg.GetDefaultMsg( "npc_pet_not_work" ) );
			return false;
		}
...
bool CChar::NPC_OnHireHear( CChar * pCharSrc )
{
	...
	if ( IsStatFlag( STATF_Pet ))
	{
		Speak( g_Cfg.GetDefaultMsg( "npc_pet_not_employed" ) );
		return false;
	}

	CGString sMsg;
	sMsg.Format( Calc_GetRandVal(2) ?
		g_Cfg.GetDefaultMsg( "npc_pet_hire_amnt" ) :
		g_Cfg.GetDefaultMsg( "npc_pet_hire_rate" ), iWage );
	Speak( sMsg );
...
bool CChar::NPC_SetVendorPrice( CItem * pItem, int iPrice )
{
	...
	{
		Speak( g_Cfg.GetDefaultMsg( "npc_pet_inv_only" ) );
		return( false );
	}
	...
	if ( pVendItem == NULL )
	{
		Speak( g_Cfg.GetDefaultMsg( "npc_pet_cantsell") );
		return( false );
	}
	...
	CGString sMsg;
	sMsg.Format( g_Cfg.GetDefaultMsg( "npc_pet_setprice" ), (LPCTSTR) pVendItem->GetName(), iPrice );
	Speak( sMsg );
...
void CChar::NPC_PetDesert()
{
	...
	{
		pCharOwn->SysMessagef( g_Cfg.GetDefaultMsg( "npc_pet_deserted" ), (LPCTSTR)GetName());
	}
	...
	{
		CGString sMsg;
		sMsg.Format( g_Cfg.GetDefaultMsg( "npc_pet_decide_master" ), (LPCTSTR)GetName());
		Speak( sMsg );

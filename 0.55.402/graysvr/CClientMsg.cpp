...
*/
	if (g_Cfg.IsSetEF(EF_No_Pause_Packet))
		return;
...
void CClient::addBarkParse( LPCTSTR pszText, const CObjBaseTemplate * pSrc, HUE_TYPE wHue, TALKMODE_TYPE mode, FONT_TYPE font )
{
	...
	if ( *pszText == '@' )
	{
		...
		{
			pszText++;
			g_Log.Event( LOGM_CLIENTS_LOG, "%s" DEBUG_CR, pszText );
			g_Log.Event( LOGL_ERROR, "(NOTE: replace SYSMESSAGE @c with SERV.LOG, this will be discontinued)" DEBUG_CR );
			return;
		}
/*
		HUE_TYPE	wHue1;
...
*/
	{
		...
		strcat( szTemp, pChar->GetKeyStr( "NAME.SUFFIX" ) );
	}
	...
	if ( g_Cfg.m_fCharTags || fAllShow )
	{
		...
		if ( fAllShow )
		{
			...
			{
				// Show UID
				sprintf( szTemp+strlen(szTemp), " [0%lx]", (DWORD) pChar->GetUID());
			}
		}
	}
...
void	CClient::addMapplane( const CPointMap * pOldP )
{
	if ( !g_Cfg.IsSetEF( EF_New_Maps ) )
		return;

	CPointMap pt = m_pChar->GetTopPoint();
	if ( pOldP && pOldP->m_mapplane == pt.m_mapplane )
		return;

	CExtData ExtData;
#define MAX_MAP_PLANES 4
	if ( pt.m_mapplane > 0 && pt.m_mapplane < MAX_MAP_PLANES )
	{
		ExtData.MapChange.m_state = pt.m_mapplane;
		addExtData( EXTDATA_Map_Change, &ExtData, sizeof(ExtData.MapChange) );
	}
	else if ( !pOldP || (pOldP->m_mapplane > 0 && pOldP->m_mapplane < MAX_MAP_PLANES ) )
	{
		ExtData.MapChange.m_state = 0;
		addExtData( EXTDATA_Map_Change, &ExtData, sizeof(ExtData.MapChange));
	}	
}
...
void CClient::addCharStatWindow( CGrayUID uid, bool fRequested ) // Opens the status window
{
	...
	{
	   int iMaxHits = pChar->Stat_GetMax(STAT_STR);
	   if ( iMaxHits < 1 ) iMaxHits = 1;
	   cmd.Status.m_maxhealth = 50;
      	   cmd.Status.m_health = (pChar->Stat_GetVal(STAT_STR) * 50) / iMaxHits;
...
*/
	...
	if ( m_pChar != pChar )
	{	
		int iMaxStr = pChar->Stat_GetMax(STAT_STR); //Torfo
		if (iMaxStr <= 0) iMaxStr = 1; //Torfo
		cmd.StatChng.m_max = 50;
		cmd.StatChng.m_val = ( pChar->Stat_GetVal(STAT_STR) * 50 ) / iMaxStr; //Torfo just modified
	}
	else
	{
...
bool CClient::Setup_Start( CChar * pChar ) // Send character startup stuff to player
{
	...
	addBark( (LPCTSTR)tVersionOut, NULL, HUE_YELLOW, TALKMODE_SYSTEM, FONT_NORMAL );
	...
	else if ( !fNoMessages )
	{
		...
		sMsg.Format( (g_Serv.m_Clients.GetCount()==2) ?
			g_Cfg.GetDefaultMsg( "login_player" ) : g_Cfg.GetDefaultMsg( "login_players" ),
...
LOGIN_ERR_TYPE CClient::LogIn( CAccountRef pAccount, CGString & sMsg )
{
	...
	if ( pClientPrev != NULL )
	{
		...
		if ( ! m_PeerName.IsSameIP( pClientPrev->m_PeerName ))
		{
...
badformat:
	...
	if ( fGuestAccount )
	{
		...
		for ( int i=0; 1; i++ )
		{
			...
			CAccountRef pAccount = g_Accounts.Account_FindCreate( sName, true );
			ASSERT( pAccount );
	...
	bool fAdmin = ( ! strcmpi( pszAccName, "Administrator" ) || ! strcmpi( pszAccName, "RemoteAdmin" ));
	bool fAutoCreate = ( g_Serv.m_eAccApp == ACCAPP_Free || g_Serv.m_eAccApp == ACCAPP_GuestAuto || g_Serv.m_eAccApp == ACCAPP_GuestTrial );

	CAccountRef pAccount = g_Accounts.Account_FindCreate( pszAccName, fAutoCreate && ! fAdmin );
	if ( ! pAccount )
	{
...
{
	BYTE	szTmp[ SCRIPT_MAX_LINE_LEN ];
	int	iLen;
	DWORD	iVal;
	...
	while ( *pszKey )
	{
		...
		else if ( toupper(*pszKey) == 'D' )
		{
			...
			szTmp[++iLen]	= (BYTE) ((iVal >> 24) & 0xFF);
			szTmp[++iLen]	= (BYTE) ((iVal >> 16) & 0xFF);
			szTmp[++iLen]	= (BYTE) ((iVal >>  8) & 0xFF);
			szTmp[++iLen]	= (BYTE) ((iVal      ) & 0xFF);
		}
		...
		else
		{
			...
			szTmp[++iLen]		= (BYTE) iVal;
		}
	}

	xSendReady( (void*) &szTmp[0], iLen+1 );
}

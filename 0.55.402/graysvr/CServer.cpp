...
void CServer::SetResyncPause( bool fPause, CTextConsole * pSrc )
{
	...
	if ( fPause )
	{
		...
		g_World.Broadcast( g_Cfg.GetDefaultMsg( "server_resync_start" ) );
	...
	else
	{
		...
		if ( ! g_Cfg.Load(true))
		{
			...
			g_World.Broadcast( g_Cfg.GetDefaultMsg( "server_resync_failed" ) );
		}
		else
		{
			pSrc->SysMessage( "Resync Complete!" DEBUG_CR );
			g_World.Broadcast( g_Cfg.GetDefaultMsg( "server_resync_success" ) );
		}
		m_fResyncPause = false;
...
bool CServer::SocketsInit() // Initialize sockets
{
	...
	if ( g_Cfg.CanRunBackTask() )
		...
		g_Log.Event( LOGM_INIT, "Background task not required." DEBUG_CR );
	return( true );
}
...
bool CServer::Load()
{
	...
	g_Log.Event( LOGM_INIT, g_szServerDescription, GRAY_VERSION );
	...
	g_Log.Event( LOGM_INIT, _TEXT("DisguiseVersion=%s" DEBUG_CR), (LPCTSTR)g_Cfg.m_sDisguiseVersion.GetPtr() );

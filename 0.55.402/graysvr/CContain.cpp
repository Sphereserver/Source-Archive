...
bool CItemContainer::CanContainerHold( const CItem * pItem, const CChar * pCharMsg )
{
	...
	if ( IsAttr(ATTR_MAGIC))
	{
		...
		pCharMsg->SysMessageDefault( "cont_magic" );
		return( false );
	}

	if ( GetCount() >= MAX_ITEMS_CONT-1 )
	{
		pCharMsg->SysMessageDefault( "cont_full" );
		return( false );
	}
		...
		pItem->Item_GetDef()->GetVolume() >= Item_GetDef()->GetVolume())
	{
		...
		pCharMsg->SysMessageDefault( "cont_toosmall" );
		return( false );
	}
	...
	case IT_EQ_BANK_BOX:
		...
		{
			pCharMsg->SysMessageDefault( "bvbox_full_items" );
			return( false );
		}
		...
		{
			pCharMsg->SysMessageDefault( "bvbox_full_weight" );
			return( false );
		}
		}
		break;
	...
	case IT_KEYRING: // empty key ring.
		...
		{
			pCharMsg->SysMessage( "You can't put blank keys on a keyring." );
			return( false );
		}
		break;
	...
	case IT_TRASH_CAN:
		...
		pCharMsg->SysMessage( "You trash the item. It will decay in 15 seconds." );
...
void CItemContainer::OnOpenEvent( CChar * pCharOpener, const CObjBaseTemplate * pObjTop )
{
		...
		IsType(IT_EQ_VENDOR_BOX))
	{
		...
		{
			sMsg.Format( g_Cfg.GetDefaultMsg("bvbox_open_self"), iStones, (LPCTSTR) GetName() );
		}
		else
		{
			sMsg.Format( g_Cfg.GetDefaultMsg("bvbox_open_other"), pCharTop->GetPronoun(), iStones, (LPCTSTR) pCharTop->GetPossessPronoun(), (LPCTSTR) GetName() );
		}

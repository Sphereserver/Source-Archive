...
bool CChar::NPC_OnVerb( CScript &s, CTextConsole * pSrc ) // Execute command from script
{
	...
	case NV_BUY:
		...
		{
			Speak( g_Cfg.GetDefaultMsg( "npc_vendor_offduty" ) );
			return( true );
		}
		if ( ! pCharSrc->GetClient()->addShopMenuBuy( this ))
		{
			if ( GetKeyItemBase( "RESEARCH.ITEM" ) )
				Speak( g_Cfg.GetDefaultMsg( "npc_vendor_nothing_buy" ) );
			else
				Speak( g_Cfg.GetDefaultMsg( "npc_vendor_no_goods" ) );
		}
		break;
	case NV_BYE:
...
bool CChar::NPC_StablePetSelect( CChar * pCharPlayer )
{
	...
	{
		Speak( g_Cfg.GetDefaultMsg( "npc_stablemaster_full" ) );
		return( false );
	}
	...
	if ( iCount > 10 )
	{
		Speak( g_Cfg.GetDefaultMsg( "npc_stablemaster_toomany" ) );
		return( false );
	}

	pCharPlayer->m_pClient->m_Targ_PrvUID = GetUID();
	pCharPlayer->m_pClient->addTarget( CLIMODE_TARG_PET_STABLE, g_Cfg.GetDefaultMsg( "npc_stablemaster_targ_stable" ) );
...
bool CChar::NPC_StablePetRetrieve( CChar * pCharPlayer )
{
	...
	if ( ! iCount )
	{
		Speak( g_Cfg.GetDefaultMsg( "npc_stablemaster_nopets" ) );
		return( false );
	}
	else
	{
		Speak( g_Cfg.GetDefaultMsg( "npc_stablemaster_treatwell" ) );
	}
	...
	case NPCACT_TALK_FOLLOW:
		...
		if ( m_Act_Targ != pSrc->GetUID())
		{
			if ( NPC_Act_Talk())
			{
				...
				CGString sMsg;
				sMsg.Format( g_Cfg.GetDefaultMsg( "npc_generic_interrupt" ),
...
int CChar::NPC_OnTrainCheck( CChar * pCharSrc, SKILL_TYPE Skill )
{
	...
	{
		Speak( g_Cfg.GetDefaultMsg( "npc_trainer_dunno_1" ) );
		return( 0 );
	}
	...
	if ( iSkillVal <= 0 )
	{
		pszMsg = g_Cfg.GetDefaultMsg( "npc_trainer_dunno_2" );
	}
	else if ( iSkillSrcVal > iSkillVal )
	{
		pszMsg = g_Cfg.GetDefaultMsg( "npc_trainer_dunno_3" );
	}
	else if ( iTrainCost <= 0 )
	{
		pszMsg = g_Cfg.GetDefaultMsg( "npc_trainer_dunno_4" );
	}
	else
	{
		return( iTrainCost );
...
bool CChar::NPC_OnTrainPay( CChar * pCharSrc, CItemMemory * pMemory, CItem * pGold )
{
	...
	if ( ! IsSkillBase( skill ))
	{
		Speak( g_Cfg.GetDefaultMsg( "npc_trainer_forgot" ) );
		return( false );
	}
	...
	Speak( g_Cfg.GetDefaultMsg( "npc_trainer_success" ) );
	...
	{
		Speak( g_Cfg.GetDefaultMsg( "npc_trainer_thatsall_1" ) );
		pMemory->m_itEqMemory.m_Action = NPC_MEM_ACT_NONE;
	}
	else
	{
		Speak( g_Cfg.GetDefaultMsg( "npc_trainer_thatsall_2" ) );
		pMemory->m_itEqMemory.m_Action = NPC_MEM_ACT_NONE;
...
bool CChar::NPC_OnTrainHear( CChar * pCharSrc, LPCTSTR pszCmd )
{
	...
	{
		Speak( g_Cfg.GetDefaultMsg( "npc_trainer_enemy" ) );
		return false;
	}
	...
	for ( ; i<MAX_SKILL; i++ )
	{
		...
		CGString sMsg;
		sMsg.Format( g_Cfg.GetDefaultMsg( "npc_trainer_price" ),
	...
	strcpy( szMsg, g_Cfg.GetDefaultMsg( "npc_trainer_price_1" ) );
	...
	for ( i=SKILL_NONE+1; i<MAX_SKILL; i++ )
	{
		...
		if ( iCount > 6 )
		{
			pPrvSkill = g_Cfg.GetDefaultMsg( "npc_trainer_price_2" );
			break;
		}
		if ( iCount > 1 )
		{
			strcat( szMsg, g_Cfg.GetDefaultMsg( "npc_trainer_price_3" ) );
		}
		if ( pPrvSkill )
		{
	...
	if ( iCount == 0 )
	{
		Speak( g_Cfg.GetDefaultMsg( "npc_trainer_thatsall_3" ) );
		return true;
	}
	if ( iCount > 1 )
	{
		strcat( szMsg, g_Cfg.GetDefaultMsg( "npc_trainer_thatsall_4" ) );
	}
...
int CChar::NPC_WalkToPoint( bool fRun )
{
	...
	if ( ! CanMoveWalkTo(pt, true, false, Dir ) )
	{
...
bool CChar::NPC_LookAtCharGuard( CChar * pChar )
{
	...
	if ( ! pChar->m_pArea->IsGuarded())
	{
		...
		if ( Calc_GetRandVal(10))
			return( false );
		CGString sMsg;
		sMsg.Format( g_Cfg.GetDefaultMsg( sm_szSpeakGuardJeer[ Calc_GetRandVal( COUNTOF( sm_szSpeakGuardJeer )) ] ), (LPCTSTR) pChar->GetName());
		Speak( sMsg );
		UpdateDir( pChar );
		return false;
	}
...
bool CChar::NPC_LookAtCharHuman( CChar * pChar )
{
		...
		! Calc_GetRandVal( 3 ))
	{
		Speak( pChar->IsStatFlag( STATF_Criminal) ?
			 g_Cfg.GetDefaultMsg( "npc_generic_seecrim" ) :
			 g_Cfg.GetDefaultMsg( "npc_generic_seemons" ) );
		// Find a guard.
		CallGuards( pChar );
...
bool CChar::NPC_LookAtCharHealer( CChar * pChar )
{
	...
	{
		pszRefuseMsg = g_Cfg.GetDefaultMsg( "npc_healer_manifest" );
refuse_to_rez:
	...
	if ( iDist > 3 )
	{
		if ( Calc_GetRandVal(5))
			return false;
		Speak( g_Cfg.GetDefaultMsg( "npc_healer_range" ) );
		return( true );
	}
	...
	{
		pszRefuseMsg = g_Cfg.GetDefaultMsg( sm_szHealerRefuseCriminals[ Calc_GetRandVal( COUNTOF( sm_szHealerRefuseCriminals )) ] );
		goto refuse_to_rez;
	}

	if (( !ImNeutral && !ImEvil) && NotoThem >= NOTO_NEUTRAL )
	{
		pszRefuseMsg = g_Cfg.GetDefaultMsg( sm_szHealerRefuseEvils[ Calc_GetRandVal( COUNTOF( sm_szHealerRefuseEvils )) ] );
		goto refuse_to_rez;
	}

	if (( ImNeutral || ImEvil ) && NotoThem == NOTO_GOOD )
	{
		pszRefuseMsg = g_Cfg.GetDefaultMsg( sm_szHealerRefuseGoods[ Calc_GetRandVal( COUNTOF( sm_szHealerRefuseGoods )) ] );
		goto refuse_to_rez;
	}
	...
	if ( ! pChar->OnSpellEffect( SPELL_Resurrection, this, 1000, NULL ))
	{
		if ( Calc_GetRandVal(2))
			Speak( g_Cfg.GetDefaultMsg( "npc_healer_fail_1" ) );
		else
			Speak( g_Cfg.GetDefaultMsg( "npc_healer_fail_2" ) );
	}
...
bool CChar::NPC_LookAtItem( CItem * pItem, int iDist )
{
	...
	if ( g_Cfg.IsSetEF( EF_New_Triggers ) )
	{
		if ( !pItem->IsAttr( ATTR_MOVE_NEVER ) )
		{
			CScriptTriggerArgs	Args( iDist, 0, pItem );
...
bool CChar::NPC_Act_Talk()
{
	...
	if ( m_atTalk.m_WaitCount <= 1 )
	{
		if ( NPC_CanSpeak())
		{
			...
			CGString sMsg;
			sMsg.Format( g_Cfg.GetDefaultMsg( sm_szText[ Calc_GetRandVal( COUNTOF( sm_szText )) ] ), (LPCTSTR) pChar->GetName() );
			Speak( sMsg );
		}
		return( false );
	}
...
bool CChar::NPC_OnItemGive( CChar * pCharSrc, CItem * pItem )
{
	...
	if ( NPC_IsOwnedBy( pCharSrc ))
	{
		...
		if ( NPC_IsVendor())
		{
			...
			{
				Speak( g_Cfg.GetDefaultMsg( "npc_pet_money" ) );
				NPC_OnHirePayMore(pItem);
			}
			else
			{
				Speak( g_Cfg.GetDefaultMsg( "npc_pet_sell" ) );
		...
		if ( NPC_CanSpeak())
		{
			if ( Food_CanEat(pItem))
			{
				if ( NPC_WantThisItem( pItem ) &&
					Use_Eat( pItem ))
				{
					Speak( g_Cfg.GetDefaultMsg( "npc_pet_food_ty" ) );
					return( true );
				}
				else
				{
					Speak( g_Cfg.GetDefaultMsg( "npc_pet_food_no" ) );
					return( false );
				}
			}
			if ( ! CanCarry( pItem ))
			{
				Speak( g_Cfg.GetDefaultMsg( "npc_pet_weak" ) );
				return( false );
			}
			if ( pItem->IsType(IT_GOLD))
			{
			...
			Speak( g_Cfg.GetDefaultMsg( "npc_pet_drop" ) );
	...
	if ( ! NPC_WantThisItem( pItem ))
	{
		...
		pCharSrc->GetClient()->addObjMessage( g_Cfg.GetDefaultMsg( "npc_generic_dontwant" ), this );
		return( false );
	}
	...
	if ( pItem->IsType(IT_GOLD))
	{
		if ( m_pNPC->m_Brain == NPCBRAIN_BANKER )
		{
			...
			CGString sMsg;
			sMsg.Format( g_Cfg.GetDefaultMsg( "npc_banker_deposit" ), pItem->GetAmount());
			Speak( sMsg );
		...
		if ( NPC_CanSpeak())
		{
			Speak( g_Cfg.GetDefaultMsg( "npc_generic_thanks" ) );
		}
	...
	if ( NPC_IsVendor())
	{
		Speak( g_Cfg.GetDefaultMsg( "npc_vendor_sell" ) );
	...
	case NPCBRAIN_THIEF:
		if ( Food_CanEat(pItem) &&
			Use_Eat( pItem ))
		{
			Speak( g_Cfg.GetDefaultMsg( "npc_beggar_food_ty" ) );
			return( true );
		}
		if ( ! CanCarry( pItem ))
		{
			Speak( g_Cfg.GetDefaultMsg( "npc_owned_weak" ) );
			return( false );
		}
		if ( pItem->IsType(IT_GOLD) || Food_CanEat(pItem))
			Speak( g_Cfg.GetDefaultMsg( "npc_beggar_food_tal" ) );
		else
			Speak( g_Cfg.GetDefaultMsg( "npc_beggar_sell" ) );
		ItemEquip( pItem );
		pItem->Update();
		if (m_Act_Targ == pCharSrc->GetUID())
		{
			Speak( g_Cfg.GetDefaultMsg( "npc_beggar_ifonly" ) );

...
bool CClient::OnRxPing( const BYTE * pData, int iLen )
{
	...
	if ( iLen > 1 )	// this is not a ping. don't know what it is.
		return( false );
//Torfo (just changed to 4.0.3 source)
...
#if 0
	if (strlen(ppRequest[1]) > 250)
	{
		DEBUG_ERR(("Received extremely long line in HTTP Connection, possible exploit" DEBUG_CR));
		return( false );
	}
#endif
...
void CClient::xFlush()
{
	...
	int iLenRet;
	if ( GetConnectType() != CONNECT_GAME /* || m_Crypt.GetClientVer() >= 0x200040 */ )	// acting as a login server to this client.
	{
...
Do_Handle_Error:
	...
	else
	{
		...
		if ( m_Crypt.GetClientVer() >= 0x200040 ) // This seem active only on crypt clients
		{
...
void CClient::xSend( const void *pData, int length )
{
	...
	if ( GetConnectType() != CONNECT_HTTP )	// acting as a login server to this client.
	{
		...
		{
			DEBUG_ERR(( "%x:Client out TOO BIG %d!" DEBUG_CR, m_Socket.GetSocket(), length ));
			m_fClosed	= true;
			return;
		}
...
bool CClient::xRecvData() // Receive message from client
{
	...
	if ( ! m_Crypt.IsInit())
	{
		...
		g_Log.Event( LOGM_CLIENTS_LOG,
			"xRecvData() %x:Junk with non inited Crypt" DEBUG_CR, m_Socket.GetSocket());

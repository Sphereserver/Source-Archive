...
CResource::CResource()
{
	...
	m_sDisguiseVersion = GRAY_VERSION;
...
enum RC_TYPE
{
	...
	RC_DEBUGFLAGS,
	RC_DECAYTIMER,
	RC_DISGUISEVERSION,		// m_sDisguiseVersion
	RC_DISTANCETALK,
	RC_DISTANCEWHISPER,
	RC_DISTANCEYELL,
	RC_DUNGEONLIGHT,
...
const CAssocReg CResource::sm_szLoadKeys[RC_QTY+1] =
{
	...
	{ "AGREE",					{ ELEM_INT,		offsetof(CResource,m_iAgree) }},
	...
	{ "DISGUISEVERSION",		{ ELEM_CSTRING,	offsetof(CResource,m_sDisguiseVersion)	}},
	...
	{ "USEGODPORT",				{ ELEM_BOOL,	offsetof(CResource,m_fUseGodPort)	}},
...
bool CResource::r_LoadVal( CScript &s )
{
	...
	case RC_DECAYTIMER:
		...
		break;
	case RC_DISGUISEVERSION:
		m_sDisguiseVersion = s.GetArgStr();
		if ( m_sDisguiseVersion.IsEmpty() )
			m_sDisguiseVersion = GRAY_VERSION;
		break;
	case RC_GUARDLINGER:
		...
		if ( !strnicmp( pszKey, "CLIENT.",7))
		{
			...
			for ( CClient * pClient = g_Serv.GetClientHead(); pClient!=NULL; pClient = pClient->GetNext())
			{
				if (cli_num==i)
				{
					...
					if (!pSrc->GetChar()||!pSrc->GetChar()->IsClient())
					{
						pChar->r_WriteVal(pszKey,sVal,pSrc);
						return true;
					}
					else {
	...
	case RC_VERSION:
		{
			...
			sprintf(tVersionOut, g_szServerDescription, (LPCTSTR)g_Cfg.m_sDisguiseVersion.GetPtr());
...
bool CResource::IsSetSpecific()
{
	return IsSetEF( EF_Specific ) && IsSetOF( OF_Specific );
}

bool CResource::IsSetEF( EF_TYPE ef )
{
	return ( m_iExperimental & ef );
}

bool CResource::IsSetOF( OF_TYPE of )
{
	return ( m_iOptionFlags & of );
}
...
bool CResource::LoadResourceSection( CScript * pScript )
{
	...
	if ( restype < 0 )
	{
		...
		if ( isdigit( pszSection[0] ))
		{
			...
			if ( strlen( pszTitle ) > ilen )
			{
				...
				{
					restype = RES_ITEMDEF;
					rid = RESOURCE_ID( RES_ITEMDEF, ahextoi( pszSection ));
				}
				else if ( ! strnicmp( pszTitle, "char", 4 ))
				{
					restype = RES_CHARDEF;
					rid = RESOURCE_ID( RES_CHARDEF, ahextoi( pszSection ));
				}
				else if ( ! strnicmp( pszTitle, "template", 8 ))
				{
					restype = RES_TEMPLATE;
					rid = RESOURCE_ID( RES_TEMPLATE, atoi( pszSection ));
				}
				else if ( ! strnicmp( pszTitle, "spee", 4 ))
				{
					restype = RES_SPEECH;
					rid = RESOURCE_ID( RES_SPEECH, ahextoi( pszSection ));
				}
			}
		}
		else
	...
	case RES_DEFNAME:
		...
		{
			if ( fNewStyleDef )
				g_Exp.m_VarMsgs.SetStr( pScript->GetKey(), false, pScript->GetArgStr());
			else
				g_Exp.m_VarDefs.SetStr( pScript->GetKey(), false, pScript->GetArgStr());
		}
		return( true );
	case RES_LOCATION:
...
bool CGObSortArray<TYPE, KEY_TYPE>::TestSort() const
{
	...
	for ( int i=0; i<iQty-1; i++ )
	{
		...
		int iRet = CompareKey( GetAt(i)->GetResourceID(), GetAt(i+1), false );
		if ( iRet >= 0 )
		{
			DEBUG_ERR(( "Array is NOT sorted !" DEBUG_CR ));
			return( false );
...
//#ifndef _DEBUG
		if ( g_Cfg.m_iAgree == 0 )
		{
			g_Log.Event( LOGL_FATAL|LOGM_INIT,
				"You should understand that  this  is  NOT  final  software, and as such is not necessarily"		DEBUG_CR
				"suit to run in a live enviroment, unless you know what you're doing."					DEBUG_CR
				"Due to MANY people not understanding this, some word of warning became a necessity, hence this"	DEBUG_CR
				"message. If you agree with running this software   on  an  experimental  basis, set agree=1 in"	DEBUG_CR
				"sphere.ini and rerun the server. Thank you. :)"
				DEBUG_CR
				DEBUG_CR
				DEBUG_CR
				);
...
#define NUM_DEF_MESSAGES 485
	if ( !g_Cfg.IsSetOF( OF_SkipUndefMsgs ) && g_Exp.m_VarMsgs.GetCount() < NUM_DEF_MESSAGES )
	{
		g_Log.Event( LOGM_INIT| LOGL_ERROR, "Found %d default messages. Required for this version: %d.", g_Exp.m_VarMsgs.GetCount(), NUM_DEF_MESSAGES );
	}
...
{
	CVarDefBase *	pVar	= g_Exp.m_VarMsgs.GetKey( pszKey );
	if ( !pVar )
	{
		if ( g_Cfg.IsSetOF( OF_SkipUndefMsgs ) )
			return NULL;
		else
		{
			g_Exp.m_sTmp.Format( "Message string '%s' missing.", pszKey );
			return (LPCTSTR) g_Exp.m_sTmp.GetBuffer();
		}
	}
	pszKey	= pVar->GetValStr();
	return *pszKey ? pszKey : NULL;
}

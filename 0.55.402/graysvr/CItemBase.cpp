...
bool CItemBase::r_WriteVal( LPCTSTR pszKey, CGString & sVal, CTextConsole * pChar )
{
	...
	case IBC_SKILLMAKE:		// Print the resources need to make in nice format.
		{
			...
			if ( *pszKey == '.' )
			{
				...
				if ( fKeyOnly || fQtyOnly )
					m_SkillMake.WriteKeys( szTmp, index, fQtyOnly, fKeyOnly );
				else
					m_SkillMake.WriteNames( szTmp, index );
				if ( fQtyOnly && szTmp[0] == '\0' )
					strcpy( szTmp, "0" );
				sVal = szTmp;
			}
			else
	...
	case IBC_RESMAKE:
		...
		{
			TCHAR szTmp[ SCRIPT_MAX_LINE_LEN ];
			m_BaseResources.WriteNames( szTmp );
			sVal = szTmp;
		}
		break;
	case IBC_SPEED:

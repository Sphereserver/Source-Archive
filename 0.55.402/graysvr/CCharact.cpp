...
void CChar::Jail( CTextConsole * pSrc, bool fSet, int iCell )
{
	...
	else	// forgive.
	{
		...
		SysMessageDefault( "msg_forgiven" );
	}
}
...
void CChar::CancelAllTrades()
{
	...
	{
		if ( pItem == NULL ) return; //Torfo: just to be sure
		if ( pItem->IsType( IT_EQ_TRADE_WINDOW ))
		{
			pItem->Delete();
		}
		CItem* pItemNext = pItem->GetNext();
		if ( pItemNext == NULL ) return; //Torfo: just to be sure
		pItem=pItemNext;
	}
}
...
{
	int statMax = Stat_GetMax(STAT_STR); //Torfo
	if (statMax < 1) statMax=1; //Torfo
	CCommand cmd;
	cmd.StatChng.m_Cmd = XCMD_StatChngStr;
	cmd.StatChng.m_UID = GetUID();
	cmd.StatChng.m_max = 50;
	cmd.StatChng.m_val = ( (Stat_GetVal( STAT_STR ) * 50) / statMax); //Torfo (modified)
...
void CChar::UpdateMove( CPointMap pold, CClient * pExcludeClient, bool fFull )
{
	...
	for ( CClient * pClient = g_Serv.GetClientHead(); pClient!=NULL; pClient = pClient->GetNext())
	{
		...
		{
			// What do i now see ?
			pClient->addMapplane( pold.IsValidPoint() ? &pold : NULL );
...
bool CChar::ItemBounce( CItem * pItem )
{
	...
	{
		// if we can carry it
		pszWhere = g_Cfg.GetDefaultMsg( "msg_bounce_pack" );
		if ( pPack == NULL )
...
dropit:
		...
		pszWhere = g_Cfg.GetDefaultMsg( "msg_feet" );
		ItemDrop( pItem, GetTopPoint());
	}

	SysMessagef( g_Cfg.GetDefaultMsg( "msg_itemplace" ), pItem->GetName(), pszWhere );
	return( true );
}
...
void CChar::EatAnim( LPCTSTR pszName, int iQty )
{
	...
	sEmoteMessage.Format( g_Cfg.GetDefaultMsg( "msg_eatsome" ), (LPCTSTR) pszName );
	Emote(sEmoteMessage);
}
...
bool CChar::Reveal( DWORD dwFlags )
{
	...
	SysMessageDefault( "hiding_revealed" );
...
CItem * CChar::Make_Figurine( CGrayUID uidOwner, ITEMID_TYPE id )
{
	...
	//Torfo: Fix from 56b by Furio
	if ( Fight_IsActive() ) // Mount fight clear
		Fight_ClearAll();
...
bool CChar::Horse_Mount( CChar * pHorse ) // Remove horse char and give player a horse item
{
	...
	if ( ! CanTouch( pHorse ))
	{
		SysMessageDefault( "msg_mount_dist" );
		return( false );
	}
	...
	{
		SysMessageDefault( "msg_mount_unable" );
		return( false );
	}
	if ( ! pHorse->NPC_IsOwnedBy( this ) || pHorse->m_pPlayer )
	{
		SysMessageDefault( "msg_mount_dontown" );
		return( false );
	}
...
bool CChar::OnTickEquip( CItem * pItem )
{
	...
	case LAYER_HORSE:
		...
		// NOTE: What if my horse dies (poisoned?)
		{
			...
			if ( pHorse == NULL )
				return( false );
//Torfo start (Fix from 56b by Furio)
...
void CChar::SleepStart( bool fFrontFall )
{
	...
	{
		SysMessageDefault( "msg_cantsleep" );
		return;
	}
...
CItemCorpse * CChar::MakeCorpse( bool fFrontFall )
{
		...
		GetDispID() != CREID_BLADES )
	{
		...
		CGString sName;
		sName.Format( g_Cfg.GetDefaultMsg( "msg_corpse_of" ), (LPCTSTR) GetName());
...
bool CChar::Death()
{
	...
	int iKillStrLen = sprintf( pszKillStr, g_Cfg.GetDefaultMsg( "msg_killed_by" ),
		(m_pPlayer)?'P':'N', GetName() );
	int iKillers = 0;
	...
	for ( ; pItem!=NULL; pItem=pItemNext )
	{
		...
		if ( pKiller != NULL )
		{
			...
			iKillStrLen += sprintf( pszKillStr+iKillStrLen, "%s%c'%s'",
				iKillers ? ", " : "",
				(pKiller->m_pPlayer)?'P':'N', pKiller->GetName() );
			iKillers ++;
		}
	...
	iKillStrLen += sprintf( pszKillStr+iKillStrLen, ( iKillers ) ? "." DEBUG_CR :
		       			"accident." DEBUG_CR );
	...
	if ( m_pPlayer )	// a logged out player.
	{
...
bool CChar::OnFreezeCheck()
{
	...
	if ( pFlag == NULL )	// stuck for some other reason i guess.
	{
		SysMessage(( IsStatFlag( STATF_Sleeping )) ?
			g_Cfg.GetDefaultMsg( "msg_unconscious" ) :
			g_Cfg.GetDefaultMsg( "msg_frozen" ) );
	}
	else
	{
		// IT_EQ_STUCK
...
CRegionBase * CChar::CanMoveWalkTo( CPointBase & ptDst, bool fCheckChars, bool fCheckOnly, DIR_TYPE dir )
{
	...
	{
		SysMessageDefault( "msg_overload" );
		return( NULL );
	}
	...
	{
		SysMessageDefault( "msg_mount_ceiling" );
		return( NULL );
	}
	...
	if ( ! IsStatFlag( STATF_DEAD | STATF_Sleeping | STATF_Insubstantial ) && fCheckChars )
	{
		...
		while (true)
		{
				...
				pChar->IsDisconnected())
			{
				...
				{
					SysMessageDefault( "msg_tingling" );
				}
				continue;
			}
			...
			{
				sMsg = g_Cfg.GetDefaultMsg( "msg_invisible" );
				continue;
			}
			else if ( pChar->IsStatFlag( STATF_Hidden ))
			{
				// reveal hidden people ?
				sMsg.Format( g_Cfg.GetDefaultMsg( "hiding_stumble" ), (LPCTSTR) pChar->GetName());
				pChar->Reveal(STATF_Hidden);
			}
			else if ( pChar->IsStatFlag( STATF_Sleeping ))
			{
				sMsg.Format( g_Cfg.GetDefaultMsg( "msg_stepon_body" ), (LPCTSTR) pChar->GetName());
				// ??? damage.
			}
			else if ( iStamReq < 0 || Stat_GetVal(STAT_DEX) < iStamReq + 1 )
			{
				sMsg.Format( g_Cfg.GetDefaultMsg( "msg_cantpush" ), (LPCTSTR) pChar->GetName());
				if ( ! IsPriv( PRIV_GM ))
				{
					SysMessage( sMsg );
					return NULL;
			...
			else
			{
				...
				sMsg.Format( g_Cfg.GetDefaultMsg( "msg_push" ), (LPCTSTR) pChar->GetName());
			}
...
bool CChar::CheckLocation( bool fStanding )
{
	...
	if ( ! fStanding )
	{
		...
		case SKILL_ARCHERY:
			m_atFight.m_fMoved	= 1;
			if ( !g_Cfg.IsSetOF( OF_Archery_CanMove ) )
			{
	...
	while (true)
	{
		...
		case IT_DREAM_GATE:
			...
			if ( IsClient())
			{
				CItemServerGate* pItemGate = dynamic_cast <CItemServerGate*>(pItem);
				DEBUG_CHECK(pItemGate);
				if ( pItemGate == NULL )
					continue;
				CServerRef pServ = pItemGate->GetServerRef();
				if ( pServ == NULL )
					continue;
	...
	if ( ! g_Serv.IsLoading())
	{
		...
		if ( IsClient() && pNewArea )
		{
				...
				! pNewArea->IsInside2d( GetTopPoint()))	// new area.
			{
				...
				SysMessagef( g_Cfg.GetDefaultMsg( "msg_region_enter" ), ( pVarStr ) ? (LPCTSTR) pVarStr->GetValStr() :
			...
			else if ( m_pArea != NULL && ! IsStatFlag( STATF_DEAD ))
			{
				if ( pNewArea->IsGuarded() != m_pArea->IsGuarded())
				{
                  ...
                  // this->GetClient()->addBarkSpeakTable( (SPKTAB_TYPE)112, NULL, 0x3b2, TALKMODE_ITEM, FONT_NORMAL );
						...
						SysMessagef( g_Cfg.GetDefaultMsg( "msg_region_guards_1" ),
							( pVarStr ) ? (LPCTSTR)pVarStr->GetValStr() : g_Cfg.GetDefaultMsg( "msg_region_guard_art" ) );
					}
					else
					{
                  // this->GetClient()->addBarkSpeakTable( (SPKTAB_TYPE)113, NULL, 0x3b2, TALKMODE_ITEM, FONT_NORMAL );
						...
						SysMessagef( g_Cfg.GetDefaultMsg( "msg_region_guards_2" ),
							( pVarStr ) ? (LPCTSTR)pVarStr->GetValStr() : g_Cfg.GetDefaultMsg( "msg_region_guard_art" ) );
					}
				}
...
bool CChar::MoveToChar( CPointMap pt )
{
	...
	SetTopPoint( pt );
	if ( prevPt.m_mapplane != pt.m_mapplane )
	{
		fSectorChange	= true;
	}
	...
	{
		CScriptTriggerArgs	Args( prevPt.m_x, prevPt.m_y, prevPt.m_z << 16 | prevPt.m_mapplane );
...
TRIGRET_TYPE CChar::OnTrigger( LPCTSTR pszTrigName, CTextConsole * pSrc, CScriptTriggerArgs * pArgs )
{
	...
	if ( m_pNPC )	// newbie script may have both NULL !!
	{
		...
		if ( g_Cfg.m_pEventsPetLink )
		{
			if ( g_Cfg.m_pEventsPetLink->HasTrigger( iAction ) )
			{
				CResourceLock s;
...
void CChar::OnTickFood()
{
	...
	if ( Stat_GetMax(STAT_FOOD) == 0 )	// No need for food.
  		return;
    	if ( Stat_GetVal(STAT_FOOD) > 0 )
	{
		Stat_SetVal( STAT_FOOD, Stat_GetVal(STAT_FOOD) -1 );
	}
	else
	{
		Stat_SetVal( STAT_FOOD, Stat_GetVal(STAT_FOOD) +1 );
	...
	if ( nFoodLevel < 40 )	// start looking for food at 40%
 	{
	    	...
	    	SysMessagef( g_Cfg.GetDefaultMsg( "msg_hunger" ), (LPCTSTR) Food_GetLevelMessage( false, false ));
		if ( ! m_pPlayer )
		{
		   	ASSERT(m_pNPC);
...
bool CChar::OnTick()
{
	...
	if ( iTimeDiff >= TICK_PER_SEC )	// don't bother with < 1 sec times.
	{
		...
		for ( int i=0; i< STAT_BASE_QTY+1; i++ )	// allow food
		{
			...
			case STAT_DEX:
				break;
			case STAT_INT:
				if ( g_Cfg.IsSetSpecific() )
				{
					if ( Skill_GetActive() == SKILL_MEDITATION )
					{
						iRate	-= (iRate * Skill_GetAdjusted( SKILL_MEDITATION )) / 2000;
						if ( iRate < 5 )
							iRate = 5;
					}
				}
	...
	if ( IsTimerExpired() && IsTimerSet())
	{
		...
		switch ( Skill_Done())
		{
		case -SKTRIG_ABORT: Skill_Fail( true ); break;	// fail with no message or credit.
		case -SKTRIG_FAIL: Skill_Fail( false ); break;
		case -SKTRIG_QTY: Skill_Cleanup(); break;
		}
		if ( ! m_pPlayer )
		{
			// What to do next ?

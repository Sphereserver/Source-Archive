		...
		if ( ! m_pChar->CanUse( pItem, false ))
		{
			if ( ! m_pChar->CanTouch( pItem ))
			{
				SysMessage(( m_pChar->IsStatFlag( STATF_DEAD )) ?
					g_Cfg.GetDefaultMsg( "reach_ghost" ) :
					g_Cfg.GetDefaultMsg( "reach_fail" ) );
				return false;
			}

			SysMessageDefault( "reach_unable" );
			return false;
		}
	}
...
mustequipthis:
	...
	case IT_TRACKER:
		{
			...
			if ( ! m_pChar->Skill_Tracking( pItem->m_uidLink, dir ))
			{
				...
				{
					SysMessageDefault( "tracking_unable" );
				}
				m_Targ_UID = pItem->GetUID();
				addTarget( CLIMODE_TARG_LINK, g_Cfg.GetDefaultMsg( "itemuse_tracker_attune") );
			}
		}
		return true;
	...
	case IT_FISH_POLE:	// Just be near water ?
		...
		addTarget( CLIMODE_TARG_USE_ITEM, g_Cfg.GetDefaultMsg( "fishing_promt" ), true );
		return true;
	...
	case IT_SHIP_HOLD_LOCK:
		...
		{
			SysMessageDefault( "itemuse_locked");
			if ( ! IsPriv( PRIV_GM ))
				return false;
		}
	...
	case IT_GAME_BOARD:
		if ( ! pItem->IsTopLevel())
		{
			SysMessageDefault( "itemuse_gameboard_fail" );
			return false;
		}
		{
	...
	case IT_EQ_NPC_SCRIPT:
		...
		{
			SysMessageDefault( "itemuse_book_fail" );
		}
		return true;
	...
	case IT_POTION:
		...
		{
			SysMessageDefault( "itemuse_potion_fail" );
			return false;
		}
		if ( RES_GET_INDEX(pItem->m_itPotion.m_Type) == SPELL_Poison )
		{
			...
			addTarget( CLIMODE_TARG_USE_ITEM, g_Cfg.GetDefaultMsg( "target_promt" ), false, true );
			return true;
		}
	...
	case IT_SPAWN_CHAR:
		...
		if ( pItem->m_itSpawnChar.m_current )
			SysMessageDefault( "itemuse_spawnchar_neg" );
		else
		{
			SysMessageDefault( "itemuse_spawnchar_rset" );
			fReset	= true;
		}
	...
	case IT_SPAWN_ITEM:
		SysMessageDefault( "itemuse_spawnitem_trig" );
	...
	case IT_SHRINE:
		if ( m_pChar->OnSpellEffect( SPELL_Resurrection, m_pChar, 1000, pItem ))
			return true;
		SysMessageDefault( "itemuse_shrine" );
		return true;

	case IT_SHIP_TILLER:
		// dclick on tiller man.
		pItem->Speak( g_Cfg.GetDefaultMsg( "itemuse_tillerman" ), 0, TALKMODE_SAY, FONT_NORMAL );
		return true;
	...
	case IT_RUNE:
		...
		addPromptConsole( CLIMODE_PROMPT_NAME_RUNE, g_Cfg.GetDefaultMsg( "itemuse_rune_name" ) );
		return true;
	...
	case IT_FORGE:
		addTarget( CLIMODE_TARG_USE_ITEM, g_Cfg.GetDefaultMsg ("itemuse_forge") );
		return true;
	case IT_ORE:
	...
	case IT_KEYRING:
		...
		{
			SysMessageDefault( "itemuse_key_fail" );
			return false;
		}
		addTarget( CLIMODE_TARG_USE_ITEM, g_Cfg.GetDefaultMsg( "itemuse_key_promt" ), false, true );
		return true;

	case IT_BANDAGE:		// SKILL_HEALING, or SKILL_VETERINARY
		addTarget( CLIMODE_TARG_USE_ITEM, g_Cfg.GetDefaultMsg( "itemuse_bandage_promt" ), false, false );
		return true;
	...
	case IT_WEAPON_MACE_SMITH:	// Can be used for smithing ?
		addTarget( CLIMODE_TARG_USE_ITEM, g_Cfg.GetDefaultMsg( "itemuse_weapon_promt" ), false, true );
		return true;

	case IT_MEAT_RAW:
	case IT_FOOD_RAW:
		addTarget( CLIMODE_TARG_USE_ITEM, g_Cfg.GetDefaultMsg( "itemuse_foodraw_promt" ) );
		return true;
	case IT_FISH:
		SysMessageDefault( "itemuse_fish_fail" );
		return true;
	case IT_TELESCOPE:
		// Big telescope.
		SysMessageDefault( "itemuse_telescope" );
		return true;
	case IT_MAP:
	case IT_MAP_BLANK:
		...
		{
			SysMessageDefault( "itemuse_map_fail" );
		}
		else
		{
	...
	case IT_CANNON_BALL:
		{
			CGString sTmp;
			sTmp.Format( g_Cfg.GetDefaultMsg( "itemuse_cball_promt" ), (LPCTSTR) pItem->GetName());
	...
	case IT_CANNON_MUZZLE:
		...
		{
			addTarget( CLIMODE_TARG_USE_ITEM, g_Cfg.GetDefaultMsg( "itemuse_cannon_powder" ) );
			return true;
		}
		if ( ! ( pItem->m_itCannon.m_Load & 2 ))
		{
			addTarget( CLIMODE_TARG_USE_ITEM, g_Cfg.GetDefaultMsg( "itemuse_cannon_shot" ) );
			return true;
		}
		addTarget( CLIMODE_TARG_USE_ITEM, g_Cfg.GetDefaultMsg( "itemuse_cannon_targ" ), false, true );
		return true;
	...
	case IT_WEAPON_MACE_CROOK:
		addTarget( CLIMODE_TARG_USE_ITEM, g_Cfg.GetDefaultMsg( "itemuse_crook_promt" ), false, true );
		return true;
	...
	case IT_PITCHER_EMPTY:
		{ // not a crime.
			CGString sTmp;
			sTmp.Format( g_Cfg.GetDefaultMsg( "itemuse_pitcher_targ" ), (LPCTSTR) pItem->GetName());
	...
	case IT_WEAPON_MACE_PICK:
		{	// Mine at the location. (possible crime?)
			CGString sTmp;
			sTmp.Format( g_Cfg.GetDefaultMsg( "itemuse_macepick_targ" ), (LPCTSTR) pItem->GetName());
	...
	case IT_HAIR_DYE:
		...
		{
			SysMessageDefault( "itemuse_dye_nohair" );
			return true;
		}
		Dialog_Setup( CLIMODE_DIALOG_HAIR_DYE, g_Cfg.ResourceGetIDType( RES_DIALOG, "d_HAIR_DYE" ), 0, m_pChar );
		return true;
	case IT_DYE:
		addTarget( CLIMODE_TARG_USE_ITEM, g_Cfg.GetDefaultMsg( "itemuse_dye_vat" ) );
		return true;
	case IT_DYE_VAT:
		addTarget( CLIMODE_TARG_USE_ITEM, g_Cfg.GetDefaultMsg( "itemuse_dye_targ"), false, true );
		return true;

	case IT_MORTAR:
		// If we have a mortar then do alchemy.
		addTarget( CLIMODE_TARG_USE_ITEM, g_Cfg.GetDefaultMsg( "itemuse_mortar_promt" ) );
		return true;
	case IT_POTION_EMPTY:
		if ( ! m_pChar->ContentFind(RESOURCE_ID(RES_TYPEDEF,IT_MORTAR)))
		{
			SysMessageDefault( "itemuse_no_mortar" );
			return( false );
		}
		addTarget( CLIMODE_TARG_USE_ITEM, g_Cfg.GetDefaultMsg( "itemuse_mortar_promt" ) );
		return true;
	...
	case IT_SEWING_KIT:	// IT_SEWING_KIT Sew with materials we have on hand.
		{
			CGString sTmp;
			sTmp.Format( g_Cfg.GetDefaultMsg( "itemuse_sewkit_promt" ), (LPCTSTR) pItem->GetName());
			addTarget( CLIMODE_TARG_USE_ITEM, sTmp );
		}
		return true;
	...
	SysMessageDefault( "itemuse_cantthink" );
	return( false );
}
...
bool CClient::Cmd_Skill_Menu( RESOURCE_ID_BASE rid, int iSelect )
{
	...
	if ( ! iShowCount )
	{
		SysMessageDefault( "cant_make_res" );
		return( false );
	}
...
bool CClient::Cmd_Skill_Tracking( int track_sel, bool fExec )
{
	...
	if ( track_sel < 0 )
	{
		...
		{
			SysMessageDefault( "tracking_spyglass" );
			return( false );
		}
	...
	SysMessage( g_Cfg.GetDefaultMsg( sm_Track_FailMsg[track_sel] ) );
	return( false );
}
...
bool CClient::Cmd_Skill_Smith( CItem * pIngots )
{
	...
	{
		SysMessageDefault( "smithing_hammer" );
		return( false );
	}
	...
	{
		SysMessageDefault( "smithing_forge" );
		return( false );
	}
...
bool CClient::Cmd_Skill_Inscription()
{
	...
	if ( pBlank == NULL )
	{
		SysMessageDefault( "inscription_fail" );
		return( false );
	}
...
bool CClient::Cmd_Skill_Alchemy( CItem * pReag )
{
	...
	if ( ! pReag->IsType(IT_REAGENT))
	{
		...
		SysMessageDefault( "alchemy_not_reg" );
		return( false );
	}
	...
	{
		SysMessageDefault( "alchemy_nobottles" );
		return( false );
	}
...
bool CClient::Cmd_Skill_Cartography( int iLevel )
{
	...
	{
		SysMessageDefault( "cartography_fail" );
		return( false );
	}

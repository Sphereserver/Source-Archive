...
public:
	...
	bool Write( const void * pData, size_t iLen ) const
	{
...
#ifndef _WIN32
		fflush( m_pStream );	// This make no sense !?
#endif
		return ( iStatus == 1 );
	}
	...
	size_t VPrintf( LPCTSTR pFormat, va_list args )
	{
		...
		size_t lenret = fwrite( szTemp, 1, len, m_pStream );
		if ( lenret != len )
			return( (size_t) -1 );
		return( lenret );
	}
...
protected:
	...
	{
		ASSERT(pInfoChk);
		return( mmioCreateChunk( m_hmmFile, pInfoChk, dwFlags ));
	}
	bool OpenBase( void * pExtra = NULL )
	{
		LPCTSTR pszName;
		...
		{
			pszName = m_strFileName;
		}
		m_hmmFile = mmioOpen( (LPSTR)(LPCSTR) pszName, (LPMMIOINFO) pExtra, GetMode() );
		return( m_hmmFile != NULL );
	}
	void CloseBase()
	{
		MMRESULT mRes = mmioClose( m_hmmFile, 0 );
		m_hmmFile = NULL;
	}
...
public:
	...
	{
		// NOTE: pInfoRIFF may be NULL
		ASSERT(pInfoChk);
		return( mmioDescend( m_hmmFile, pInfoChk, pInfoRIFF, dwFlags ));
	}
	DWORD Ascend( MMCKINFO * pInfo ) const
	{
		ASSERT(pInfo);
		return( mmioAscend( m_hmmFile, pInfo, 0 ));
	}

	virtual DWORD GetPosition() const
	{
		return( mmioSeek( m_hmmFile, 0, SEEK_CUR ));
	}
	virtual LONG Seek( LONG lOffset, UINT iOrigin )
	{
		return( mmioSeek( m_hmmFile, lOffset, iOrigin ));
	}
	virtual size_t Read( void * pData, size_t dwLength ) const
	{
		ASSERT(pData);
		return( mmioRead( m_hmmFile, (HPSTR) pData, (LONG) dwLength ));
	}
	virtual bool Write( const void * pData, size_t dwLength ) const
	{
		ASSERT(pData);
		return( mmioWrite( m_hmmFile, (HPSTR) pData, (LONG) dwLength ) == (LONG) dwLength );
	}

...
bool CDIBitmap::Set( LPBITMAPINFO pDIB )
{
	...
	m_pPixelData = ((BYTE FAR *) GetColors()) + GetColorSize();
	return( true );
}
...
bool CDIBitmap::Read( CFile * pFile, bool fHeader )
{
	//@------------------------------------------------------------------------
	// PURPOSE:
...
#define DIB_HEADER_MARKER     ((WORD) ('M' << 8) | 'B')
	...
	//
	lSize = GetImageSize();
	if ( pFile->Read( GetPixelData(), lSize ) != 1 )
	{
		return( false );
	}
...
bool CDIBitmap::Write( CFile * pFile, bool fHeader )
{
	//
...
bool CGBitmap::Set( HBITMAP hBitmap )
{
	Empty();
	if ( hBitmap != NULL )
	{
		...
		m_hBitmap = NULL;
		DEBUG_ERR(( "CGBitmap:Set BAD %xh", hBitmap ));
	}
	return( false );
}

bool CGBitmap::SetDIB( LPBITMAPINFO pDIB, const void FAR * pBits )
{
	//
...
bool CGBitmap::Fill_DIB_Header( LPBITMAPINFO pDIB, DWORD dwStyle, WORD wBits ) const
{
	//
	// PURPOSE>
	//  Make the DIB header.
	...
	if ( ! GetObject( &bm ))
	{
		DEBUG_ERR(( "CGBitmap: Bad %xh", m_hBitmap ));
		return( false );
	}
...
LONG CGBitmap::GetDIBSize( void ) const
{
	//@------------------------------------------------------------------------
	// PURPOSE:
...
bool CGBitmap::Read( CFile * pFile, bool fHeader )
{
	// RETURN:
	//  true = Success.
	//  false = ERROR.
...
bool CGBitmap::Load( LPCSTR pszFileName )
{
	// RETURN:
	//  true = Success.
	//  false = ERROR.
...
void CGBitmap::Copy( HBITMAP hBitmap )
{	// copy in
	Set( Bitmap_MakeCopy( hBitmap ));
}

HBITMAP CGBitmap::MakeCopy() const
{	// copy out
	return( Bitmap_MakeCopy( m_hBitmap ));
}

CDIBitmap * CGBitmap::CreateDIB( DWORD dwStyle, WORD wBits, HPALETTE hPal ) const
{
	//@------------------------------------------------------------------------
	// PURPOSE:
...
bool CGBitmap::Write( CFile * pFile, bool fHeader ) const
{
	//@------------------------------------------------------------------------
	// PURPOSE:
...
bool CGBitmap::Save( LPCSTR pszFileName ) const
{
	//@------------------------------------------------------------------------
	// PURPOSE:
...
COLORREF CGBitmap::BltCopy( HDC hDC, int x, int y ) const
{
...
void CGBitmap::BltStretch( HDC hDC, int cx, int cy, int x, int y ) const
{
	HDC hDCBit = CreateCompatibleDC( hDC );
	if ( hDCBit != NULL )
	{
...
HBITMAP CGBitmap::CreateAnd( void ) const
{
	// Prepare DC
...
void CGBitmap::Paint( HDC hDC, HBITMAP hbmpAnd,

    ...
    static int CvtUNICODEToSystem( TCHAR * pOut, int iSizeOutBytes, WCHAR wChar )
    {
    	...
    	for ( ; iOut > 0; iOut-- )
    	{
    		pOut = 0x80 | ( wChar & ((1<<6)-1));
    		wChar >>= 6;
    	}
    
    	ASSERT( wChar < (1<<iStartBits));
    	pOut = ( 0xfe << iStartBits ) | wChar;
    ...
    int CvtNUNICODEToSystem( TCHAR * pOut, int iSizeOutBytes, const NCHAR * pInp, int iSizeInChars )
    {
    		...
    		posInfo->dwMajorVersion > 4 )
    	{
    		...
    		WCHAR szBuffer;
    		for ( ; pInp && iInp < COUNTOF(szBuffer)-1 && iInp < iSizeInChars; iInp++ )
    		{
    			szBuffer = pInp;
    		}
    		szBuffer = '\0';
    		...
    		if ( iOut < 0 )
    		{
    			pOut = 0;	// make sure it's null terminated
    			return( 0 );
    		}
    	}
    	else

    ...
    #define PACKWORD(p,w)       (p)=HIBYTE(w);(p)=LOBYTE(w)
    #define UNPACKWORD(p)       MAKEWORD((p),(p))       // low,high
    ...
    #define PACKDWORD(p,d)      (p)=((d)>>24)&0xFF;(p)=((d)>>16)&0xFF;(p)=HIBYTE(d);(p)=LOBYTE(d)
    #define UNPACKDWORD(p)      MAKEDWORD( MAKEWORD((p),(p)), MAKEWORD((p),(p)))
    ...
    private:
        ...
        // terminate with a 0
        // 0 = english default.
        char m_codes; // UNICODE language pref. ('ENU'=english)
    public:
        CLanguageID()
        {
                m_codes = 0;
    ...
    extern const WORD g_Packet_Lengths; // from client 2.0.0 @offset 010e700
    ...
    enum EXTDATA_TYPE
    {
        ...
        EXTDATA_Party_Enable    = 0x08, // send to the client (? not sure)
    ...
    union CExtData
    {
        BYTE Default;                   // Unknown stuff.
        NDWORD WalkCode;        // EXTDATA_WalkCode_Prime or EXTDATA_WalkCode_Add
        ...
        struct
        {
                ...
                NCHAR m_msg;    // text of the msg. var len
    ...
    enum CHATMSG_TYPE   // Chat system messages.
    {
        ...
        CHATMSG_MustBeInAConference,            // b - You must be in a conference to do this. To join a conference, select one 
from the Conference menu
        ...
        CHATMSG_PlayerIsIgnoring,                       // f - <name> has chosen to ignore you. None of your messages to them wi
ll get through
        CHATMSG_RevokedSpeaking,                        // 10 - The moderator of this conference has not given you speaking priv
eledges.
        CHATMSG_ReceivingPrivate,                       // 11 - You can now receive private messages
        CHATMSG_NoLongerReceivingPrivate,       // 12 - You will no longer receive private messages. Those who send you a messag
e will be notified that you are blocking incoming messages
        CHATMSG_ShowingName,                            // 13 - You are now showing your character name to any players who inqui
re with the whois command
        CHATMSG_NotShowingName,                         // 14 - You are no long showing your character name to any players who i
nquire with the whois command
        ...
        CHATMSG_ModeratorHasKicked,                     // 19 - <name>, a conference moderator, has kicked you out of the confer
ence
        ...
        CHATMSG_ModeratorRemovedSpeaking,       // 21 - <name>, a channel moderator, has removed your speaking priveledges for t
his conference.
        CHATMSG_ModeratorGrantSpeaking,         // 22 - <name>, a channel moderator, has granted you speaking priveledges for th
is conference.
        CHATMSG_SpeakingByDefault,                      // 23 - From now on, everyone in the conference will have speaking privi
ledges by default.
        CHATMSG_ModeratorsSpeakDefault,         // 24 - From now on, only moderators in this conference will have speaking privi
ledges by default.
        CHATMSG_PlayerTalk,                                     // 25 - <name>:
        CHATMSG_PlayerEmote,                            // 26 - *<name>*
        CHATMSG_PlayerPrivate,                          // 27 - :
        CHATMSG_PasswordChanged,                        // 28 - The password to the conference has been changed
        CHATMSG_NoMorePlayersAllowed,           // 29 - Sorry--the conference named <name> is full and no more players are allow
ed in.
        ...
        CHATMSG_SendPlayerName =        0x03ee, // Message to add a player out to client (prefixed with a "1" if they are the mo
derator or a "0" if not
        CHATMSG_RemoveMember =          0x03ef, // Message to remove a player from clients channel member list
        CHATMSG_ClearMemberList =       0x03f0, // This message clears the list of channel participants (for when leaving a chan
nel)
    ...
    enum DELETE_ERR_TYPE
    {
        ...
        DELETE_ERR_NOT_OLD_ENOUGH, // 3 That character is not old enough to delete. The character must be 7 days old before it c
an be deleted.
    ...
    #define MAX_ACCOUNT_PASSWORD_ENTER 16       // client only allows n chars.
        char m_charname;
        char m_charpass;        // but the size of the structure is 30 (go figure)
    };
    ...
    #define MAX_CHARS_PER_ACCT  5
        ...
        union
        {
                ...
                struct  // size = >3    // user typed in text.
                {
                        ...
                        char m_text;    // 8=var size MAX_TALK_BUFFER
                } Talk;
                ...
                struct  // size = 11    // plot course on map.
                {
                        ...
                        BYTE m_action;  // 1, MAPCMD_TYPE 1: add pin, 5: delete pin, 6: toggle edit/noedit or cancel while in ed
it
                        BYTE m_pin;
                        NWORD m_pin_x;
                        NWORD m_pin_y;
    ...
    #define MAX_BOOK_LINES 8    // max lines per page.
                ...
                struct // size = var (0x0c)     // Bulletin request
                {
                        ...
                        BYTE m_flag;    // 3= BBOARDF_TYPE 0=board name, 5=new message, 4=associate message, 1=message header, 2
=message body
                        NDWORD m_UID;   // 4-7 = UID for the bboard.
                        NDWORD m_UIDMsg; // 8- = name or links data.
                        BYTE m_data;    // submit new message data here.
                } BBoard;
                ...
                struct  // size = 65    // request to list the chars I can play.
                {
                        ...
                        char m_acctname;        // This is corrupted or encrypted seperatly ?
                        char m_acctpass;
                } CharListReq;
                ...
                struct // size = 16+var = console Prompt response.
                {
                        ...
                        NWORD m_len;
                        BYTE m_unk3;
                        char m_text;    // null terminated text.
                } Prompt;
                ...
                struct  // sizeof = 149 // not sure what this is for.
                {
                        ...
                        WCHAR m_wDirectory;     // @4 = 00 00
                        BYTE m_szVideoCardDescrip2; // @24 = AA 7E 3F 04 00
                        BYTE m_szModemManufacturer;     // @44 = 40 F8
                        BYTE m_Unk54;
                        BYTE m_bUnk84;                          // @84 = 0
                ...
                struct // size = var // Unicode Speech Request
                {
                        ...
                        char m_lang;    // lang (null terminated, "ENU" for US english.) CLanguageID
                        NCHAR m_utext;  // NCHAR text (null terminated, ?=(msgsize-12)/2) MAX_TALK_BUFFER
                } TalkUNICODE;
                ...
                struct // size = var // Get gump button change
                {
                        ...
                        NDWORD m_checkIds;      // 19 - ?? id of a checkbox or radio button that is checked
                        ...
                        struct
                        {
                                ...
                                NCHAR m_utext;  // character data (in unicode!):
                        } m_texts;
                } GumpDialogRet;
                ...
                struct // size = various
                {
                        ...
                        char m_lang;    //      3-6 = unicode language code CLanguageID
                        NCHAR m_utext;  //      7-? = text from client
                } ChatText;
                ...
                struct // size = 64     // response to pressing the chat button
                {
                        BYTE m_Cmd;             // 0 = 0xb5
                        BYTE m_unk1;    // 1 = ???
                        NCHAR m_uname;  // 2-63 - unicode name
                } Chat;
                ...
                struct  // we get walk codes from the > 1.26.0 server to be sent back in the Walk messages.
                {
                        ...
                        NWORD m_type;   // 3 - 4 EXTDATA_TYPE 1=prime LIFO stack, 2=add to beginning of LIFO stack, 6=add to par
ty
                        CExtData m_u;
                } ExtData;
    ...
    len: 1
                ...
                char m_charname;        // 7-36= character name (30 bytes) ( 00 padded at end)
                        NWORD m_health;         // 37-38 = current health
                NWORD m_maxhealth;      // 39-40 = max health
                        BYTE m_perm;            // 41 = permission to change name ? 0=stats invalid, 1=stats valid., 0xff = i ca
n change the name
    ...
    #define CHARMODE_INVIS              0x80
                ...
                struct // size = 14 + 30 + var
                {
                        ...
                        char m_charname;        // 14
                        char m_text;            // var size. MAX_TALK_BUFFER
                } Speak;
                ...
                struct  // size = 11
                {
                        BYTE m_Cmd; // 0 = 0x56
                        NDWORD m_UID;
                        BYTE m_Mode; // 1: Add a pin location, 5: Display map, pins to follow 7: Editable, no pins follow 8: rea
d only
                ...
                struct // size = var (38)       // Bulletin Board stuff
                {
                        ...
                        NDWORD m_UID;   // 4-7 = UID for the bboard.
                        BYTE m_data;    // 8- = name or links data.
                } BBoard;
                ...
                struct // size = var // put up a menu of items.
                {
                        ...
                        NWORD m_context;                // = context info passed from server
                        BYTE m_lenname;
                        char m_name;            // var len. (not null term)
                        BYTE m_count;
                        struct          // size = var
                        {
    ...
    #define MAX_ITEMS_MULTI (MAX_BUFFER - 11) / 12/*1279*/
                ...
                struct  // Needs God client ?
                {
                        ...
                        char m_skillname;// 9-? = zero term skill name
                } EditSkillsMul;
                ...
                struct // size = 16 // console prompt request.
                {
                        ...
                        BYTE m_unk3;
                        char m_text;    // 3 = null terminated text? doesn't seem to work.
                } Prompt;
    ...
    #define MAX_SERVER_NAME_SIZE MAX_NAME_SIZE
                        ...
                        struct  // size=40
                        {
                                ...
                                BYTE m_ip;              // 36-39 = struct in_addr
                        } m_serv;
                } ServerList;
                ...
                struct // size = 13 + var // Send a gump text entry dialog.
                {
                        ...
                        char m_text1;   // line 1 variable text length
                        BYTE m_cancel;          // 0 = disable, 1=enable
                        BYTE m_style;           // 0 = disable, 1=normal, 2=numerical
                        NDWORD m_mask;          // format = max length of textbox input, format = max value of textbox input
                        NWORD m_textlen2;       // length of string #2
                        char m_text2;   // line 2 text
                } GumpInpVal;
                ...
                struct // size = 14 + 30 + var // Unicode speech
                {
                        ...
                        NWORD m_wHue;           // 10-11 = HUE_TYPE.
                        NWORD m_font;           // 12-13 = FONT_TYPE
                        char m_lang;            // language (same as before) CLanguageID
                        char m_charname;        //
                        NCHAR m_utext;          // text (null terminated, ?=(msgsize-48)/2 MAX_TALK_BUFFER
                } SpeakUNICODE;
                ...
                struct // size = 21 + var // Send a gump menu dialog.
                {
                        ...
                        char m_commands;        // Format commands (var len) { %s }
                        BYTE m_zeroterm;
                        NWORD m_textlines;      // How many lines of text to follow.
                        struct
                        {
                                NWORD m_len;    // len of this line.
                                NCHAR m_utext;  // UNICODE text. (not null term)
                        } m_texts;
                } GumpDialog;
                ...
                struct
                {
                        ...
                        char m_lang;// 5-8 = unicode language code (null term....default = 'enu' (english?) CLanguageID
                        NCHAR m_uname;          // 9-? = name in unicode (zero terminated) (may have  prefix, moderator, etc)
                        NCHAR m_uname2; // ?-? = name in unicode (also used for other things (passworded, etc)
                } ChatReq;
    ...
    #define MAX_ITEMS_PREVIEW (MAX_BUFFER - 16) / 10
                ...
                struct // size = variable       // Put up a tooltip
                {
                        ...
                        NWORD m_len;                    // 1-2
                        NDWORD m_UID;                   // 3-6
                        NCHAR m_utext;          // zero terminated UNICODE string
                } ToolTip;
                ...
                struct  // size = 8 // Show Character Profile.
                {
                        ...
                        char m_title;   // Description 1 (not unicode!, name, zero terminated)
                        NCHAR m_utext1; // Static Description 2 (unicode!, zero terminated)
                        NCHAR m_utext2; // Player Description 3 (unicode!, zero terminated)
                } CharProfile;
                ...
                struct  // Display a msg from table. (already present at the client side)
                {
                        ...
                        NDWORD m_UID;           // 3-6 = UID num of speaker. 01010101 = system
                        NWORD m_id;                     // 7-8 = CREID_TYPE of speaker. Used for text hieight (doesn't the damn 
client already know this?)
                        BYTE m_mode;            // 9 = TALKMODE_TYPE
                        NWORD m_wHue;           // 10-11 = HUE_TYPE.
                        NWORD m_font;           // 12-13 = FONT_TYPE
                        NDWORD m_index;         // = predefined message type (SPKTAB_TYPE)
                        char m_charname; // Name or other label (does OSI have this part actually working?)
                } SpeakTable;

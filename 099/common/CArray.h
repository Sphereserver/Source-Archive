    ...
    public:
        ...
        {       // Bump the existing entry here forward.
                ...
                memmove( &m_pData, &m_pData, sizeof(TYPE)*(m_nCount-nIndex-1));
                m_pData = newElement;
        }
    ...
    template<class TYPE, class KEY_TYPE>
    int CGObSortArray<TYPE, KEY_TYPE>::FindKeyNear( KEY_TYPE key, int & iCompareRes ) const
    {
    ...
    template<class TYPE>
    struct CGPtrSortArray : public CGPtrTypeArray<TYPE>
    {
    protected:
        virtual int QCompare( int i, TYPE pServ ) = 0;
    ...
    template<class TYPE>
    void  CGPtrSortArray<TYPE>::QSort( int left, int right )
    {
        ASSERT( left < right );
        int j = left;
        int i = right;
    
        TYPE pServRef = GetAt( (left+right) / 2 );

    ...
    const BYTE CCrypt::sm_key_table =
    {
    	// @ 0f1420 in v2.0.3
    ...
    const BYTE CCrypt::sm_seed_table =
    {
    ...
    static DWORD sm_dwCodingData;
    
    static const DWORD sm_dwInitData =
    {
    	// @0106248 in v2.0.3
    ...
    BYTE CCrypt::EnCryptByte(BYTE data)
    {
    	...
    	if ( GetClientVer() < 0x200040 )
    	{
    		...
    		m_gameBlockPos--;
    		result = data ^ m_Key.u_cKey;
    
    		m_Key.u_cKey = result;
    	}
    	if ( GetClientVer() >= 0x200003 )
    	{
    ...
    void CCrypt::Decrypt( BYTE * pOutput, const BYTE * pInput, int iLen )
    {
    	...
    	if ( m_ConnectType == CONNECT_LOGIN || GetClientVer() < 0x126000 || GetClientIgnition())
    	{
    	...
    	if ( GetClientVer() < 0x200040 && m_gameStreamPos + iLen > CRYPT_GAMETABLE_TRIGGER)
    	{
    ...
    {
    	const DWORD *pCodes = sm_dwCodingData;
    	bool fUnknown = true;
    	if(fUnknown)		// ??? seems to ALWAYS be TRUE ???
    	{
    		...
    		for(int i=0; i<8; i++)
    		{
    			key.u_iKey ^= pCodes^(((pCodes]+pCodes+0x100])^pCodes+0x200])+pCodes+0x300]);
    			key.u_iKey ^= pCodes^(((pCodes]+pCodes+0x100])^pCodes+0x200])+pCodes+0x300]);
    		}
    		key.u_iKey ^= pCodes;
    	}
    	...
    	DWORD tmp = key.u_iKey;
    	key.u_iKey = key.u_iKey;
    	key.u_iKey = tmp;
    }
    
    void CCrypt::InitSeed( int iTable )
    {
    	const BYTE * pKey = sm_seed_table;
    ...
    void CCrypt::InitTables()		// static
    {
    	...
    	{
    		memcpy(sm_dwCodingData, sm_dwInitData, sizeof(sm_dwInitData));
    
    		DWORD code;
    		code = (sm_key_table << 24) + (sm_key_table << 16) + (sm_key_table << 8) + sm_key_table;
    		code = (sm_key_table << 24) + (sm_key_table << 16) + (sm_key_table << 8) + sm_key_table;
    		code = (sm_key_table << 24) + (sm_key_table << 16) + (sm_key_table << 8) + sm_key_table;
    		for(int j=0; j<18; j++)
    		{
    			sm_dwCodingData ^= code;
    		}
    
    		CCryptKey tmpKey;
    		tmpKey.u_iKey = tmpKey.u_iKey = 0;
    
    		for(j=0; j<0x412; j+=2)
    		{
    			PrepareKey( tmpKey, i );
    			sm_dwCodingData = tmpKey.u_iKey;
    			sm_dwCodingData = tmpKey.u_iKey;
    		}
    	}
    ...
    void CCrypt::Init( DWORD dwIP, CONNECT_TYPE type )
    {
    	CCryptBase::Init( dwIP );
    	m_ConnectType = type;
    
    	if ( type == CONNECT_LOGIN )
    		return;

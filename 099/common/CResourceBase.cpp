    ...
    LPCTSTR const CResourceBase::sm_szResourceBlocks =	// static
    {
    	...
    	"PLOTITEM",		// (Phase this out in favor of better @trigger TEVENT= and EVENT= support.)
    ...
    bool CResourceBase::LoadResources( CResourceScript * pScript )
    {
    	...
    	if ( ! pScript->Open())
    	{
    		g_pLog->Event( LOGL_CRIT|LOGM_INIT, " '%s' not found...\n", (LPCTSTR) pScript->GetFilePath());
    		return( false );
    	}
    ...
    bool CResourceDef::SetResourceName( LPCTSTR pszKey )
    {
    	ASSERT(pszKey);
    
    	// This is the global def for this item.
    	for ( int i=0; pszKey; i++ )
    	{
    	...
    	CVarDefBase * pVarKey = g_Exp.m_VarDefs.FindKeyPtr( pszKey );
    	if ( pVarKey )
    	{
    		...
    		{
    			DEBUG_WARN(( "The DEFNAME=%s has a strange type mismatch? 0%x!=0%x\n", (LPCTSTR) pszKey, pVarKey->GetValNum(), GetResourceID().GetPrivateUID() ));
    		}
    		else
    		{
    			DEBUG_WARN(( "The DEFNAME=%s already exists! 0%x!=0%x\n", (LPCTSTR) pszKey, RES_GET_INDEX(pVarKey->GetValNum()), GetResourceID().GetResIndex() ));
    		}
    
    		iVarNum = g_Exp.m_VarDefs.SetNum( pszKey, GetResourceID().GetPrivateUID() );
    	}
    	else
    	{
    		iVarNum = g_Exp.m_VarDefs.SetNumNew( pszKey, GetResourceID().GetPrivateUID() );
    	}
    ...
    bool CResourceLink::ResourceLock( CResourceLock &s )
    {
    	...
    	LPCTSTR pszKey = GetResourceName();
    ...
    bool CResourceRefArray::r_LoadVal( CScript & s, RES_TYPE restype )
    {
    	...
    	TCHAR * ppBlocks;	// max is arbitrary
    ...
    void CResourceRefArray::WriteResourceRefList( CGString & sVal ) const
    {
    	...
    	for ( int j=0;j<GetCount(); j++ )
    	{
    		...
    		len += strcpylen( szVal+len, GetResourceName( j ));
    ...
    bool CResourceQtyArray::IsResourceMatchAll( CObjBase * pObj ) const
    {
    	...
    	{
    		RESOURCE_ID ridtest = GetAt(i).GetResourceID();
    		if ( ! pObj->IsResourceMatch( ridtest, GetAt(i).GetResQty()))
    			return( false );
    	}
    ...
    void CResourceQtyArray::WriteKeys( TCHAR * pszArgs ) const
    {
    	for ( int i=0; i<GetCount(); i++ )
    	{
    		if ( i )
    		...
    		pszArgs += GetAt(i).WriteKey( pszArgs );
    	}
    	*pszArgs = '\0';
    }
    
    void CResourceQtyArray::WriteNames( TCHAR * pszArgs ) const
    {
    	for ( int i=0; i<GetCount(); i++ )
    	{
    		if ( i )
    ...
    bool CResourceQtyArray::operator == ( const CResourceQtyArray & array ) const
    {
    	...
    	for ( int i=0; i<GetCount(); i++ )
    	{
    		for ( int j=0; true; j++ )
    		{
    			if ( j>=array.GetCount())
    				return( false );
    			if ( ! ( GetAt(i).GetResourceID() == array.GetResourceID() ))
    				continue;
    			if ( GetAt(i).GetResQty() != array.GetResQty() )
    				continue;
    			break;
    		}
    	}

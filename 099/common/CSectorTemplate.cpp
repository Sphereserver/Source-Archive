    ...
    void CCharsActiveList::SetWakeStatus( int iMapPlane )
    {
    	m_timeLastClient = CServTime::GetCurrentTime();	// mark time in case it's the last client
    	m_dwMapPlaneClients |= 1 << ( iMapPlane & 0x1f );
    }
    ...
    void CSectorTemplate::CheckMapBlockCache( int iTime )
    {
    	...
    	{
    		CGrayMapBlock * pMapBlock = STATIC_CAST <CGrayMapBlock *>(m_MapBlockCache);
    		ASSERT(pMapBlock);
    ...
    const CGrayMapBlock * CSectorTemplate::GetMapBlock( const CPointMap & pt )
    {
    	...
    	if ( i >= 0 )
    	{
    		pMapBlock = STATIC_CAST <CGrayMapBlock *>(m_MapBlockCache);
    		ASSERT(pMapBlock);
    ...
    CRegionBase * CSectorTemplate::GetRegion( const CPointBase & pt, DWORD dwType ) const
    {
    	...
    	for ( int i=0; i<iQty; i++ )
    	{
    		CRegionBase * pRegion = m_RegionLinks;
    		ASSERT(pRegion);
    ...
    CTeleport * CSectorTemplate::GetTeleport2d( const CPointMap & pt ) const
    {
    	...
    	if ( i < 0 )
    		return( NULL );
    	return STATIC_CAST <CTeleport *>( m_Teleports);
    }

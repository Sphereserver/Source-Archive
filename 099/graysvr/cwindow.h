    ...
    public:
        ...
        HICON GetIcon( int fType = ICON_SMALL ) const
        {
                // ICON_BIG vs ICON_SMALL
                return( (HICON)(DWORD) SendMessage( WM_GETICON, (WPARAM)fType ));
        }
        HICON SetIcon( HICON hIcon, int fType = ICON_SMALL )
        {
                // ICON_BIG vs ICON_SMALL
    ...
    public:
        ...
        LPSTR           m_lpCmdLine;    // Points to a null-terminated string that specifies the command line for the applicatio
n.
        CWindow *       m_pMainWnd;             // Holds a pointer to the application's main window. For an example of how to in
itialize m_pMainWnd, see InitInstance.
    ...
    public:
        ...
        {
                return( ::GetPrivateProfileString( pszSection, pszEntry, pszDefault, pszReturnBuffer, cbReturnBuffer, m_pszProfi
leName ));
        }
    ...
    // Operations
        ...
        {
                ASSERT(IsWindow());
                return (HICON) SendMessage( STM_GETICON );
        }
        ...
        {
                ASSERT(IsWindow());
                return (HENHMETAFILE) SendMessage( STM_GETIMAGE, IMAGE_ENHMETAFILE );
        }
    ...
    // Operations
        ...
        int LineFromChar( ich)         ((int)(DWORD)SendMessage( EM_LINEFROMCHAR, (WPARAM)(int)(ich), 0L))
        int LineIndex( line)           ((int)(DWORD)SendMessage( EM_LINEINDEX, (WPARAM)(int)(line), 0L))
        int LineLength( line)          ((int)(DWORD)SendMessage( EM_LINELENGTH, (WPARAM)(int)(line), 0L))
    
        void Scroll( dv, dh)            ((void)SendMessage( EM_LINESCROLL, (WPARAM)(dh), (LPARAM)(dv)))
    
        BOOL CanUndo() const                   ((BOOL)(DWORD)SendMessage( EM_CANUNDO, 0L, 0L))
        BOOL Undo()                      ((BOOL)(DWORD)SendMessage( EM_UNDO, 0L, 0L))
        void EmptyUndoBuffer()           ((void)SendMessage( EM_EMPTYUNDOBUFFER, 0L, 0L))
    
        void SetPasswordChar(ch)       ((void)SendMessage( EM_SETPASSWORDCHAR, (WPARAM)(UINT)(ch), 0L))
    
        BOOL FmtLines(fAddEOL)         ((BOOL)(DWORD)SendMessage( EM_FMTLINES, (WPARAM)(BOOL)(fAddEOL), 0L))
    ...
    public:
        ...
        void ReplaceSel( LPCTSTR lpszNewText, BOOL bCanUndo = FALSE )
        {
                ASSERT(IsWindow());
                SETTEXTEX settext;
                settext.flags = ( bCanUndo ) ? (ST_SELECTION|ST_KEEPUNDO): (ST_SELECTION);
            settext.codepage = CP_ACP; 
                int iBytesMoved = (int) SendMessage( EM_SETTEXTEX, (WPARAM) &settext, (LPARAM) lpszNewText );
        }
    #endif

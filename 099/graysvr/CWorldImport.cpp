    ...
    bool CImportFile::ImportSCP( CScript & s, WORD wModeFlags )
    {
        ...
        while ( s.FindNextSection())
        {
                ...
                else if ( s.IsSectionType( "WORLDCHAR" ))
                {
                        ...
                        {
                                m_pCurObj = CChar::CreateBasic( (CREID_TYPE) g_Cfg.ResourceGetIndexType( RES_CHARDEF, s.GetArgSt
r()));
                        }
                }
                else if ( s.IsSectionType( "WORLDITEM" ))
                {
                        ...
                        {
                                m_pCurObj = CItem::CreateTemplate( (ITEMID_TYPE) g_Cfg.ResourceGetIndexType( RES_ITEMDEF, s.GetA
rgStr()));
                        }
                }
                else
    ...
    bool CImportFile::ImportWSC( CScript & s, WORD wModeFlags )
    {
        ...
        while ( s.ReadTextLine(true))
        {
                ...
                if ( mode == IMPFLAGS_ITEMS )   // CItem.
                {
                        ...
                        fprintf(wscfile, "SPAWN %i\n", (items.spawn1*16777216)+(items.spawn2*65536)+(items.spawn3*256)+items.spa
wn4);
                        fprintf(wscfile, "CORPSE %i\n", items.corpse);
                        fprintf(wscfile, "OWNER %i\n", (items.owner1*16777216)+(items.owner2*65536)+(items.owner3*256)+items.own
er4);
    #endif
                }
                if ( mode == IMPFLAGS_CHARS )
                {
                        ...
                        {
                                SKILL_TYPE skill = (SKILL_TYPE) atoi( &(s.GetKey()));
                        ...
                        fprintf(wscfile, "SPAWN %i\n", (chars.spawn1*16777216)+(chars.spawn2*65536)+(chars.spawn3*256)+chars.spa
wn4);
                        fprintf(wscfile, "WAR %i\n", chars.war);
                        fprintf(wscfile, "OWN %i\n", (chars.own1*16777216)+(chars.own2*65536)+(chars.own3*256)+chars.own4);
    
                        fprintf(wscfile, "ALLMOVE %i\n", chars.priv2);
                        fprintf(wscfile, "PACKITEM %i\n", chars.packitem);
                        fprintf(wscfile, "RESERVED1 %i\n", chars.cell);
                        fprintf(wscfile, "NPCWANDER %i\n", chars.npcWander);
                        fprintf(wscfile, "FOLLOWTARGET %i\n", (chars.ftarg1*16777216)+(chars.ftarg2*65536)+(chars.ftarg3*256)+ch
ars.ftarg4);
                        ...
                        fprintf(wscfile, "ROBE %i\n", (chars.robe1*16777216)+(chars.robe2*65536)+(chars.robe3*256)+chars.robe4);
    #endif
                        continue;
                }
        }

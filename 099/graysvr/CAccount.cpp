    ...
    CAccountRef CAccounts::Account_FindChecked( LPCTSTR pszName )
    {
    ...
    CAccountRef CAccounts::Account_FindCheck( LPCTSTR pszName )
    {
        TCHAR szName;
        ...
        if ( strchr( szName, '@' ))
        {
                // Find by email address alternatively ?
                return( Account_FindEmail( szName ));
        }
        return Account_FindChecked( szName );
    }
    ...
    bool CAccounts::Account_Load( LPCTSTR pszNameRaw, CScript & s, bool fChanges )
    {
        ...
        CAccountRef pAccount = Account_FindChecked( szName );
        if ( pAccount )
        {
...
,
    ...
    CAccountRef CAccounts::Account_Get( int index )
    {
        ...
        if ( index >= m_Accounts.GetCount())
                return( NULL );
        return( CAccountRef( STATIC_CAST <CAccount *>( m_Accounts)));
    }
    ...
    CAccountRef CAccounts::Account_FindEmail( LPCTSTR pszEmail )
    {
    ...
    CAccountRef CAccounts::Account_FindIP( CSocketAddressIP dwIP )
    {
    ...
    CAccountRef CAccounts::Account_Create( LPCTSTR pszName )
    {
    ...
    CAccountRef CAccounts::Account_CreateNew( CTextConsole * pSrc, LPCTSTR pszName, LPCTSTR pszPassword )
    {
        CAccountRef pAccount = Account_FindCheck( pszName );
        if ( pAccount != NULL )
        {
        ...
        pSrc->SysMessagef( "Account '%s' created" DEBUG_CR, (LPCTSTR) pszName );
        return( pAccount );
    }
    
    CAccountRef CAccounts::Account_CreateEmail( CTextConsole * pSrc, LPCTSTR pszName, LPCTSTR pszEmail )
    {
        ...
        if ( ! CAccount::NameStrip( szName, pszName ))
        {
                pSrc->SysMessagef( "Account name '%s' is not valid.", (LPCTSTR) pszName );
                return( NULL );
        }
        ...
        if ( pAccount != NULL )
        {
                pSrc->SysMessagef( "Account '%s' already exists", (LPCTSTR) szName );
                return( NULL );
        }
        ...
        if ( pAccount != NULL )
        {
                pSrc->SysMessagef( "An account with email '%s' already exists.", (LPCTSTR) pszEmail );
                return( NULL );
        }
        ...
        if ( pSrc->GetPrivLevel() < PLEVEL_Admin )
        {
                ...
                if (pClient)
                {
                        pAccount = Account_FindIP( pClient->m_Socket.GetPeerName() );
                        if ( pAccount )
                        {
                                ...
                                if ( datetime.GetTime() - pAccount->m_dateLastConnect.GetTime() 
                                        < 2*24*60*60 )
                                {
                                        pSrc->SysMessage( "You already have an account." );
                                        return( NULL );
                                }
                        }
                }
        ...
        {
                pSrc->SysMessage( "This email address is not allowed or may already be used." );
        ...
        {
                pSrc->SysMessagef( "Scheduling password mailing to '%s' failed", (LPCTSTR) pszEmail );
                Account_Delete( pAccount );
                return( NULL );
        }
    
        // This counts as being on line.
        pSrc->SysMessagef( "Password has been mailed to '%s'.", (LPCTSTR) pszEmail );
        return( pAccount );
    }
    
    bool CAccounts::Account_PasswordEmail( CTextConsole * pSrc, LPCTSTR pszEmail )
    {
        // Mail an accounts password to them. recovery
        // Find by email
        ASSERT(pSrc);
    
        CAccountRef pAccount = Account_FindEmail( pszEmail );
        if ( pAccount == NULL )
        {
                // IS it just the account name ?
                pAccount = Account_FindChecked( pszEmail );
                if ( pAccount == NULL )
                {
                        pSrc->SysMessagef( "Email address '%s' does not have an account.", (LPCSTR) pszEmail );
                        return( false );
                }
        }
        ...
        if ( pSrc->GetPrivLevel() <= PLEVEL_Player )
        {
                ...
                if ( datetime.GetTime() - pAccount->m_dateLastConnect.GetTime() 
                        < 24*60*60 )
                {
                        pSrc->SysMessage( "Try again tomorrow." );
                        return( false );
                }
        }
    ...
    bool CAccounts::Account_OnCmd( TCHAR * pszArgs, CTextConsole * pSrc )
    {
        ...
        {
                index = (VACS_TYPE) FindTableSorted( ppCmd, sm_szVerbKeys, COUNTOF( sm_szVerbKeys )-1 );
        }
        ...
        {
        case VACS_ADD:
                Account_CreateNew( pSrc, ppCmd, ppCmd );
                return true;
    
        case VACS_ADDEMAIL:
                Account_CreateEmail( pSrc, ppCmd, ppCmd );
                return true;
    
        case VACS_EMAILPASS:
                Account_PasswordEmail( pSrc, ppCmd );
                return true;
        ...
        case VACS_UNUSED:
                return Cmd_ListUnused( pSrc, ppCmd, ppCmd, ppCmd );
    
    #if 0
        case VACS_LIST:
                {
                // List all the accounts. 10 at a time.
                int iPage = pVerb ? atoi(ppCmd):0;
        ...
        CAccountRef pAccount = Account_FindCheck( ppCmd );
        if ( pAccount == NULL )
        {
                pSrc->SysMessagef( "Account '%s' does not exist" DEBUG_CR, ppCmd );
                return false;
        }
        ...
        return pAccount->r_Verb( CScript( ppCmd, ppCmd ), pSrc );
    }
    ...
    int CAccount::NameStrip( TCHAR * pszNameOut, LPCTSTR pszNameInp ) // static
    {
        ...
        int len = Str_GetBare( pszNameOut, pszNameInp, MAX_ACCOUNT_NAME_SIZE, " !\"#$%&()*,/:;<=>?^{|}~" ); // "'`-.+" // seems 
people are using these.
    ...
    static LPCTSTR const sm_szPrivLevels =
    {
    ...
    int CAccount::AttachChar( CChar * pChar )
    {
        ...
        if ( i >= 0 )
        {
                ...
                {
                        g_Log.Event( LOGM_ACCOUNTS|LOGL_ERROR, "Account '%s' has %d characters" DEBUG_CR, (LPCTSTR) GetName(), i
Qty );
                }
        }
    ...
    void CAccount::TogPrivFlags( WORD wPrivFlags, LPCTSTR pszArgs )
    {
        ...
        if ( pszArgs == NULL || pszArgs == '\0' )       // toggle.
        {
    ...
    bool CAccount::Kick( CTextConsole * pSrc, bool fBlock )
    {
        ...
        g_Log.Event( LOGL_EVENT|LOGM_GM_CMDS, "A'%s' was %sed by '%s'" DEBUG_CR, (LPCTSTR) GetName(), pszAction, (LPCTSTR) pSrc-
>GetName());
        return( true );
    }
    ...
    bool CAccount::EmailPassword()
    {
        ...
        m_dateLastConnect = CGTime::GetCurrentTime();
    ...
    void CAccount::SetNewPassword( LPCTSTR pszPassword )
    {
        if ( pszPassword == NULL || pszPassword == '\0' )
        {
                ...
                for ( int i=0; i<8 && i<MAX_ACCOUNT_PASSWORD_ENTER-1; i++ )
                {
    ...
    bool CAccount::SetEmailAddress( LPCTSTR pszEmail, bool fBlockHost )
    {
        ...
        // Not already used ?
        CAccountRef pAccount = g_Accounts.Account_FindEmail( pszEmail );
    ...
    bool CAccount::ScheduleEmailMessage( RESOURCE_ID iEmailMessage )
    {
        ...
        {
                if ( m_EMailSchedule == iEmailMessage )
                        return( false );
        }
    ...
    LPCTSTR const CAccount::sm_szLoadKeys = // static
    {
        "ACCOUNT",
        "BLOCK",
        "CHARS",
    ...
    bool CAccount::r_WriteVal( LPCTSTR pszKey, CGString &sVal, CTextConsole * pSrc )
    {
        ...
        case AC_CHATNAME:
                ...
                break;
        case AC_COMMENT:
                sVal = m_TagDefs.FindKeyStr( "COMMENT" );
                break;
        case AC_EMAIL:
                sVal = m_sEMail;
                break;
        ...
        case AC_EMAILMSG:
                ...
                {
                        sVal = g_Cfg.ResourceGetName( m_EMailSchedule );
                }
                break;
        case AC_EMAILQ:
    ...
    bool CAccount::r_LoadVal( CScript & s )
    {
        ...
        case AC_CHARUID:
                ...
                if ( ! g_Serv.IsLoading())
                {
                        ...
                        {
                                DEBUG_ERR(( "CHARUID 0%x (%s) not attached to account '%s'" DEBUG_CR, (DWORD) uid, (LPCTSTR) pCh
ar->GetName(), (LPCTSTR) GetName()));
                                return( false );
                        }
                        AttachChar(pChar);
                }
    ...
    void CAccount::r_Write( CScript & s, bool fTitle )
    {
        ...
        {
                s.WriteKey( "PLEVEL", sm_szPrivLevels );
        }
        ...
        for ( int i=0; i<m_EMailSchedule.GetCount(); i++ )
        {
                s.WriteKey( "EMAILMSG", g_Cfg.ResourceGetName( m_EMailSchedule ));
        }
    ...
    {
        ASSERT(pSrc);
        PLEVEL_TYPE plevel = pSrc->GetPrivLevel();
        if ( pSrc->GetAccount() != this &&
                plevel <= GetPrivLevel())       // can't change accounts higher than you in any way
        {
                // Priv Level too low
        ...
        switch ( i )
        {
        // case AV_SUMMON:      // move all the offline chars here ?
        ...
        case AV_EMAIL:
                ...
                if ( s.HasArgs())
                {
                        ...
                        {
                                pSrc->SysMessagef( "Email address '%s' is not allowed or is already used.", (LPCTSTR) s.GetArgSt
r());
                                return true;
                        }
                }
        ...
        case AV_PASSWORD:
                ...
                {
                        ClearPassword();
                        pSrc->SysMessagef( "Password for '%s' has been cleared.", (LPCTSTR) GetName() );
                        pSrc->SysMessage( "Log out, then back in to set the new password." DEBUG_CR );
                        g_Log.Event( LOGM_ACCOUNTS|LOGL_EVENT, "Account '%s', password cleared", (LPCTSTR) GetName());
                }
                else
                {
                        SetPassword( s.GetArgStr());
                        pSrc->SysMessage( "Password for '%s' has been set" DEBUG_CR );
                        g_Log.Event( LOGM_ACCOUNTS|LOGL_EVENT, "Account '%s', password set to '%s'", (LPCTSTR) GetName(), (LPCTS
TR) s.GetArgStr());
                }
                return( true );
    ...
    bool CAccount::SendOutgoingMail( RESOURCE_ID rid )
    {
        ...
        {
                DEBUG_ERR(( "Failed to send mail to '%s' '%s', reason '%s'" DEBUG_CR, (LPCTSTR) sName, (LPCTSTR) m_sEMail, (LPCT
STR) smtp.GetLastResponse() ));
                return( false );
        }
    ...
    void CAccount::SendAllOutgoingMail()
    {
        ...
        while ( m_EMailSchedule.GetCount())
        {
                ...
                if ( g_Serv.IsLoading())        // don't do this now. multi thread problems.
                        return;
                RESOURCE_ID iMsg = m_EMailSchedule;

    ...
    void CCharRefArray::DeleteChars()
    {
        ...
        {
                CChar * pChar = m_uidCharArray.CharFind();
                if ( pChar )
                {
    ...
    void CCharRefArray::WritePartyChars( CScript & s )
    {
        ...
        {
                s.WriteKeyHex( "CHARUID", m_uidCharArray );
        }
    }
    ...
    void CPartyDef::SendAddList( CGrayUID uid, CChar * pCharDest )
    {
        ...
        {
                // Send just this one.
                ExtData.Party_Msg_Data.m_uids = uid;
                iQty = 1;
        }
        else
    #endif
        {
                ...
                for ( int i=0; i<iQty; i++ )
                {
                        ExtData.Party_Msg_Data.m_uids = m_Chars.GetChar(i);
                }
        }
    ...
    void CPartyDef::SendRemoveList( CChar * pCharRemove, CGrayUID uidAct )
    {
        ...
        if ( pCharRemove )
        {
                ...
                ExtData.Party_Msg_Data.m_uids = pCharRemove->GetUID();
                iQty = 1;
        }
        else
        {
        ...
        ExtData.Party_Msg_Data.m_uids = uidAct;

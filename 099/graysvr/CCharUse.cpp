    ...
    bool CChar::Use_MultiLockDown( CItem * pItemTarg )
    {
        ...
        if ( ! pItemTarg->m_uidLink.IsValidUID())
        {
                ...
                {
                        SysMessage( "Locking items using this method is not allowed." );
                }
                return( true );
        }
        if ( pItemTarg->m_uidLink == m_pArea->GetResourceID() )
        {
                ...
                {
                        SysMessage( "Unlocking items using this method is not allowed." );
                }
                return( true );
        }
    ...
    void CChar::Use_CarveCorpse( CItemCorpse * pCorpse )
    {
        ...
        if ( pCorpseDef != NULL )
        {
                ...
                {
                        CItem * pBlood = CItem::CreateBase( (ITEMID_TYPE) sm_Item_Blood );
                        ASSERT(pBlood);
        ...
        {
                int iQty = pCorpseDef->m_BaseResources.GetResQty();
                RESOURCE_ID rid = pCorpseDef->m_BaseResources.GetResourceID();
    ...
    void CChar::Use_MoonGate( CItem * pItem )
    {
        ...
        if ( pItem->IsType(IT_MOONGATE))
        {
                ...
                for ( ;true; i++ )
                {
                        if ( i>=iCount)
                                return;
                        int iDist = GetTopPoint().GetDist( g_Cfg.m_MoonGates );
        ...
        if ( ! m_pPlayer )
        {
                ...
                if ( m_pNPC->m_Brain == NPCBRAIN_GUARD )
                {
                        ...
                        CRegionWorld * pArea = dynamic_cast <CRegionWorld *> ( ptTeleport.GetRegion(REGION_TYPE_MULTI|REGION_TYP
E_AREA));
...
E_AREA));
    ...
    static LPCTSTR const sm_Txt_ArcheryButte_Success =
    {
        ...
        if (!fSuccess)
        {
                ...
                Emote( sm_Txt_ArcheryButte_Failure );
        }
        else
        {
                ...
                Emote( sm_Txt_ArcheryButte_Success );
        }
        // Update the target
        pButte->m_itArcheryButte.m_AmmoType = AmmoID;
        ...
        case IT_DOOR_LOCKED:
                if ( ! ContentFindKeyFor( pItem ))
                {
                        ...
                        if ( pItem->IsAttr(ATTR_MAGIC))
                        {
                                ...
                                ITEMID_TYPE id = ( GetDispID() & DOOR_NORTHSOUTH ) ? ITEMID_DOOR_MAGIC_SI_NS : ITEMID_DOOR_MAGIC
_SI_EW;
        ...
        case IT_SPY_GLASS:
                ...
                static LPCTSTR const sm_sPhases =
                {
                        "new moon",
                        "waxing crescent",
                        "first quarter",
                ...
                SysMessagef( "Trammel is in the %s phase.", sm_sPhases);
                SysMessagef( "Felucca is in the %s phase.", sm_sPhases);
                }
    ...
    bool CChar::ItemEquipArmor( bool fForce )
    {
        ...
        if ( ! fForce )
        {
                ...
                {
                        pBestArmor = LayerFind((LAYER_TYPE)i);
                        if ( pBestArmor != NULL )
                        {
                                iBestScore = INT_MAX;
                        }

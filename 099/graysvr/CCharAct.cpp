    ...
    void CChar::LayerAdd( CItem * pItem, LAYER_TYPE layer )
    {
        ...
        if ( layer == LAYER_NONE )
        {
                ...
                {
                        DEBUG_MSG(( "ContentAdd id=%s '%s', LAYER_NONE is strange" DEBUG_CR, (LPCTSTR) pItem->GetResourceName(),
 (LPCTSTR) pItem->GetName()));
    ...
    void CChar::UpdateStatVal( STAT_TYPE type, int iChange, int iLimit )
    {
        ...
        int iVal = m_StatVal.m_val;
    
        if ( iChange )
        {
                if ( ! iLimit )
                {
                        iLimit = Stat_Get(type);
                }
                if ( iChange < 0 )
                {
                        iVal += iChange;
...
()));
    ...
    int CChar::ItemPickup( CItem * pItem, int amount )
    {
        ...
        CChar * pChar;  // the char that possesses this.
        if ( IsTakeCrime( pItem, &pChar ))
        {
        ...
        CItem * pItemPrev = LayerFind( LAYER_DRAGGING );
        if ( pItemPrev != NULL )
        {
    ...
    dropit:
                if ( ! GetTopPoint().IsValidPoint())
                {
                        ...
                        if (pszWhere)
                        {
                                DEBUG_ERR(( "No pack to place loot item '%s' for NPC '%s'" DEBUG_CR, pItem->GetResourceName(), G
etResourceName()));
                        }
                        else
                        {
                                DEBUG_ERR(( "Loot item %s too heavy for NPC %s" DEBUG_CR, pItem->GetResourceName(), GetResourceN
ame()));
                        }
                        pItem->Delete();
                        return false;
        ...
        SysMessagef( "You put the %s %s.", pItem->GetName(), pszWhere );
        return( true );
    }
    ...
    CItem * CChar::Make_Figurine( CGrayUID uidOwner, ITEMID_TYPE id )
    {
        ...
        CItem * pItem = CItem::CreateScript( ( id == ITEMID_NOTHING ) ? pCharDef->m_trackID : id );
        ASSERT(pItem);
    ...
    const WORD g_Item_Horse_Mounts = // extern
    {
    ...
    bool CChar::Horse_Mount( CChar * pHorse ) // Remove horse char and give player a horse item
    {
        ...
        for ( int i=0; true; i++ )
        {
                ...
                if ( pHorse->GetDispID() == g_Item_Horse_Mounts )
                {
                        id = (ITEMID_TYPE) g_Item_Horse_Mounts;
                        break;
                }
        }
    ...
    CItemCorpse * CChar::MakeCorpse( bool fFrontFall )
    {
        ...
        bool fLootable = ! IsStatFlag( STATF_Conjured );
    ...
    bool CChar::Death()
    {
        ...
        if ( m_pPlayer )
        {
                ...
                LayerAdd( CItem::CreateScript( ITEMID_DEATHSHROUD ), LAYER_QTY );
        ...
        else if ( m_pPlayer )   // a logged out player.
        {
    ...
    CRegionBase * CChar::CanMoveWalkTo( CPointBase & ptDst, bool fCheckChars )
    {
        ...
        if ( IsStatFlag(STATF_OnHorse) &&
                ...
                ! IsPriv(PRIV_GM))
        {
                SysMessage( "The ceiling is too low for you to be mounted!" );
                return( NULL );
        }
        ...
        while (true)
        {
                ...
                case IT_FIRE:
                        ...
                        // pItem->m_itSpell.m_spelllevel = 0-1000 = heat level.
                        {
                                ...
                                OnTakeDamage( g_Cfg.GetSpellEffect( SPELL_Fire_Field, iSkillLevel ), NULL, DAMAGE_FIRE | DAMAGE_
GENERAL );
                        }
    ...
    bool CChar::MoveToRegion( CRegionWorld * pNewArea, bool fAllowReject )
    {
        ...
        if ( ! g_Serv.IsLoading())
        {
                ...
                if ( IsClient() && pNewArea )
                {
                        ...
                        {
                                CVarDefStr * pVarStr = dynamic_cast <CVarDefStr *>( pNewArea->m_TagDefs.FindKeyPtr("ANNOUNCEMENT
"));
                                SysMessagef( "You have entered %s", ( pVarStr ) ? (LPCTSTR) pVarStr->GetValStr() :
                        ...
                        else if ( m_pArea != NULL && ! IsStatFlag( STATF_DEAD ))
                        {
                                if ( pNewArea->IsGuarded() != m_pArea->IsGuarded())
                                {
                                        ...
                                        {
                                                CVarDefStr * pVarStr = dynamic_cast <CVarDefStr *>( pNewArea->m_TagDefs.FindKeyP
tr("GUARDOWNER"));
                                                SysMessagef( "You are now under the protection of %s guards",
                                                        ( pVarStr ) ? (LPCTSTR)pVarStr->GetValStr() : "the");
                                        }
                                        else
                                        {
                                                CVarDefStr * pVarStr = dynamic_cast <CVarDefStr *>( m_pArea->m_TagDefs.FindKeyPt
r("GUARDOWNER"));
                                                SysMessagef( "You have left the protection of %s guards",
                                                        ( pVarStr ) ? (LPCTSTR)pVarStr->GetValStr() : "the" );
                                        }
                                }
                                if ( pNewArea->IsFlag(REGION_FLAG_NO_PVP) != m_pArea->IsFlag(REGION_FLAG_NO_PVP))
                                {
                                        SysMessage(( pNewArea->IsFlag(REGION_FLAG_NO_PVP)) ?
    ...
    bool CChar::MoveTo( CPointMap pt )
    {
        ...
        {
                OnTrigger( CTRIG_EnvironChange, this );
        }
    ...
    bool CChar::MoveToValidSpot( DIR_TYPE dir, int iDist, int iDistStart )
    {
        ...
        for ( int i=0; i<iDist; i++ )
        {
                ...
                signed char z = g_World.GetHeightPoint( pt, wBlockFlags, pt.GetRegion( REGION_TYPE_MULTI ));
                if ( ! ( wBlockFlags &~ wCan ))
                {
    ...
    TRIGRET_TYPE CChar::OnTrigger( LPCTSTR pszTrigName, CTextConsole * pSrc, CScriptTriggerArgs * pArgs )
    {
        ...
        {
                // RES_EVENTS
                for ( i=0; i<pCharDef->m_Events.GetCount(); i++ )
                {
                        CResourceLink * pLink = pCharDef->m_Events;
                        ASSERT(pLink);
                        if ( ! pLink->HasTrigger( iAction ))
                                continue;
                        CResourceLock s;
    ...
    void CChar::OnTickFood()
    {
        ...
        if ( nFoodLevel < 40 )  // start looking for food at 40%
        {
                ...
                SysMessagef( "You are %s", (LPCTSTR) Food_GetLevelMessage( false, false ));
    ...
    bool CChar::OnTick()
    {
        ...
        if ( iTimeDiff >= TICK_PER_SEC )        // don't bother with < 1 sec times.
        {
                ...
                for ( int i=0; i<COUNTOF(m_StatVal); i++ )
                {
                        ...
                        else if ( m_StatVal.m_val != Stat_Get((STAT_TYPE)i))
                        {
        ...
        if ( IsTimerExpired() && IsTimerSet())
        {
                ...
                switch ( Skill_Done())
                {
                case -SKTRIG_ABORT: Skill_Fail( true ); break;  // fail with message but no credit.
        ...
        else
        {
                        ...
                        ( g_Cfg.m_wDebugFlags & DEBUGF_SKILL_TIME ))
                {
                        ...
                        if ( Fight_IsActive())
                        {
                                ...
                                {
                                case WAR_SWING_READY:           szTemp = '|'; break;
                                case WAR_SWING_EQUIPPING:       szTemp = '-'; break;
                                case WAR_SWING_SWINGING:        szTemp = '+'; break;
                                default: szTemp = '?'; break;
                                }
                        }
                        if ( IsStatFlag(STATF_Fly))
                        {

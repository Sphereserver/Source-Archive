    ...
    LPCTSTR const CWebPageDef::sm_szVerbKeys =
    {
    ...
    class CServersSortArray : public CGPtrSortArray<CServerRef>
    {
    private:
        WV_TYPE m_iType;        // ? SC_TYPE ?
    public:
        int QCompare( int left, CServerRef pServRef );
        bool SortByType( WV_TYPE iType );
    };
    
    int CServersSortArray::QCompare( int left, CServerRef pServRef )
    {
        // 1 = left > right
        ASSERT(pServRef);
        switch ( m_iType )
    ...
    public:
        ...
        virtual void SysMessage( LPCTSTR pszMessage, HUE_TYPE wHue = HUE_TEXT_DEF ) const
        {
    ...
    LPCTSTR const CWebPageDef::sm_szLoadKeys =
    {
        ...
        case WV_CLIENTLIST:
                {
                        for ( CClient * pClient = g_Serv.GetClientHead(); pClient!=NULL; pClient = pClient->GetNext())
                        {
                                ...
                                pChar->ParseText( Str_MakeFiltered( szTmp2 ), &g_Serv, 1 );
                                pSrc->SysMessage( szTmp2 );
                        }
                }
                break;
        ...
        case WV_SERVLIST:
                {
                        ...
                        {
                                CServerDef * pServ = Array;
                                if ( pServ == NULL )
                                        continue;
    
                                // NOTE: servers list onces per hour. if they have not registered in 8 hours then they are not v
alid.
                                ...
                                strcpy( szTmp2, pszArgs );
                                pServ->ParseText( Str_MakeFiltered( szTmp2 ), &g_Serv, 1 );
                                pSrc->SysMessage( szTmp2 );
                        }
                }
                break;
        ...
        case WV_GUILDLIST:
                {
                        ...
                        {
                                CItemStone * pStone = g_World.m_Stones;
                                if ( pStone == NULL )
                                        continue;
                                ...
                                strcpy( szTmp2, s.GetArgStr() );
                                pStone->ParseText( Str_MakeFiltered( szTmp2 ), &g_Serv, 1 );
                                pSrc->SysMessage( szTmp2 );
                        }
                }
                break;
        ...
        case WV_GMPAGELIST:
                {
                        ...
                        for ( ; pPage!=NULL; pPage = pPage->GetNext())
                        {
                                sm_iListIndex++;
                                strcpy( szTmp2, s.GetArgStr() );
                                pPage->ParseText( Str_MakeFiltered( szTmp2 ), &g_Serv, 1 );
                                pSrc->SysMessage( szTmp2 );
                        }
                }
                break;
    ...
    bool CWebPageDef::WebPageUpdate( bool fNow, LPCTSTR pszDstName, CTextConsole * pSrc )
    {
        ...
        {
                TCHAR szTmp;
                strcpy( szTmp, FileRead.GetKey());
    
                TCHAR * pszHead = strstr( szTmp, "<script language=\"Sphere\">" );
                if ( pszHead != NULL )
                {
                        // Deal with the stuff preceding the scripts.
                        *pszHead = '\0';
                        pszHead += 26;
                        ParseText( szTmp, pSrc, 1 );
                        FileOut.SysMessage( szTmp );
                        fScriptMode = true;
                }
                else
                {
                ...
                if ( fScriptMode )
                {
                        ...
                        if ( pszFormat != '\0' )
                        {
                                ...
                                if ( ! r_Verb( CScript( pszFormat ), &FileOut ))
                                {
                                        DEBUG_ERR(( "Web page source format error '%s'" DEBUG_CR, (LPCTSTR) szTmp ));
                                        continue;
                                }
                        }
    ...
    bool CWebPageDef::SetSourceFile( LPCTSTR pszName, CClient * pClient )
    {
        ...
        if ( pClient )
        {
                ...
                fMasterList = PeerName.IsSameIP( g_BackTask.m_RegisterIP );
        }
        ...
        if ( iType < 0 )
        {
                if ( pClient )
                {
                        g_Log.Event( LOGM_HTTP|LOGL_TRACE, "Bad HTTP file type request '%s' " DEBUG_CR, (LPCTSTR)pszName, PeerNa
me.GetAddrStr() );
                        if ( ! fMasterList )
                                return( false );
                }
    ...
    LPCTSTR const CWebPageDef::sm_szPageType =
    {
    ...
    LPCTSTR const CWebPageDef::sm_szTrigName =  // static
    {
        _TEXT("@AAAUNUSED"),
        "@Load",
        NULL,
    ...
    int CWebPageDef::ServPageRequest( CClient * pClient, LPCTSTR pszURLArgs, CGTime * pdateIfModifiedSince )
    {
        ...
        if ( HasTrigger(WTRIG_Load))
        {
                ...
                if ( ResourceLock(s))
                {
                        TRIGRET_TYPE iRet = CScriptObj::OnTriggerScript( s, sm_szTrigName, pClient, NULL );
                ...
                dateChange <= pdateIfModifiedSince->GetTime() )
        {
                ...
                // DEBUG_MSG(( "Web page '%s' not modified since '%s'" DEBUG_CR, (LPCTSTR) GetName(), ((CGTime)dateChange).Forma
tGmt(NULL) ));
                // "\r\n" = 0d0a
        ...
        int iLen = sprintf( szTmp,
                ...
                "Accept-Ranges: bytes\r\n",
                (LPCTSTR) sDate,
                (LPCTSTR) sm_szPageType // type of the file. image/gif, image/x-xbitmap, image/jpeg
                );
    ...
    bool CWebPageDef::ServPagePost( CClient * pClient, LPCTSTR pszURLArgs, TCHAR * pContentData, int iContentLength )
    {
        ...
        for ( int i=0; i<iArgs; i++ )
        {
                ...
                case 'T':
                        ...
                        {
                                TCHAR szData;
                                HtmlDeCode( szData, pszNum );
                                resp.AddText( iNum, szData );
                        }
                        break;
                }
        }
    ...
    bool CWebPageDef::ServPage( CClient * pClient, TCHAR * pszPage, CGTime * pdateIfModifiedSince )     // static
    {
        ...
        if ( iError == 404 )
        {
                ...
                g_Log.Event( LOGM_HTTP|LOGL_EVENT, "%x:HTTP Page Bad Request '%s'" DEBUG_CR, pClient->m_Socket.GetSocket(), (LPC
TSTR) szPageName );
        }

    ...
    CItemStone * CChar::Guild_Find( MEMORY_TYPE MemType ) const
    {
        ...
        if ( pMyStone == NULL )
        {
                ...
                const_cast <CChar*>(this)->Memory_ClearTypes( MemType );        // Make them forget they were ever in this guild
....again!
                return( NULL );
        }
        return( pMyStone );
...
....again!
    ...
    void CChar::Guild_Resign( MEMORY_TYPE MemType )
    {
        ...
        if ( pMember->IsPrivMember())
        {
                ...
                if ( pMemFight )
                {
                        ...
                        ASSERT(pMyStone);
                        SysMessagef( "You cannot quit your %s while in a fight", (LPCTSTR) pMyStone->GetTypeName() );
                }
        }
    ...
    {
        // Get my  if i have chosen to turn it on.
        LPCTSTR pszAbbrev = Guild_Abbrev(MemType);
        if ( pszAbbrev == NULL )
                return( NULL );
        TCHAR * pszTemp = Str_GetTemp();
        sprintf( pszTemp, " ", pszAbbrev );
        return( pszTemp );
    }
    ...
    int CChar::Noto_GetLevel() const
    {
        ...
        int i=0;
        int iKarma = Stat_Get(STAT_Karma);
        for ( ; i<COUNTOF( sm_KarmaLevel ) && iKarma < sm_KarmaLevel; i++ )
                ;
        ...
        for ( ; j<COUNTOF( sm_FameLevel ) && iFame > sm_FameLevel; j++ )
                ;
    ...
    void CChar::Noto_ChangeNewMsg( int iPrvLevel )
    {
        if ( iPrvLevel != Noto_GetLevel())
        {
                ...
                SysMessagef( "You are now %s", (LPCTSTR) Noto_GetTitle());
        }
    }
    ...
    int CChar::Skill_Snooping( SKTRIG_TYPE stage )
    {
        ...
        if ( CheckCrimeSeen( SKILL_SNOOPING, pCharMark, pCont, (stage == SKTRIG_FAIL)? "attempting to peek into" : "peeking into
" ))
        {
    ...
    int CChar::Skill_Stealing( SKTRIG_TYPE stage )
    {
        ...
        if ( pItem == NULL )    // on a chars head ? = random steal.
        {
                ...
                if ( pCharMark == NULL )
                {
                        SysMessage( "Nothing to steal here." );
                        return( -SKTRIG_QTY );
                }
    ...
    cantsteal:
        ...
        if ( pContainer ) 
        {
                ...
                if ( pCorpse )
                {
                        SysMessage( "You can't steal from corpses" );
                        return( -SKTRIG_ABORT );
                }
        }
        ...
        {
                SysMessage( "Just dclick this to practice stealing" );
                return -SKTRIG_QTY;
        }
        ...
        {
                SysMessage( "No stealing is possible here." );
                return( -SKTRIG_QTY );
        }
        ...
        if ( pCharMark != NULL )
        {
                ...
                {
                        SysMessage( "Can't harm other players here." );
                        return( -1 );
                }
        ...
        if ( CheckCrimeSeen( SKILL_STEALING, pCharMark, pItem, (stage == SKTRIG_FAIL)? "attempting to steal" : "stealing" ))
        {
    ...
    void CChar::CallGuards( CChar * pCriminal )
    {
        ...
        while (true)
        {
                ...
                {
                        if ( PrevGuardTarg.IsValidUID() && PrevGuardTarg == pGuard->m_Act_Targ )        // take him off this cas
e.
                                break;
        ...
        if ( pGuard == NULL )
        {
                // Spawn a new guard.
                RESOURCE_ID rid;
                CVarDefBase * pVar = m_pArea->m_TagDefs.FindKeyPtr("GUARDID");
                if ( pVar )
                {
...
;
    ...
    static const LAYER_TYPE sm_ArmorLayerBack = { LAYER_SHIRT, LAYER_CHEST, LAYER_TUNIC, LAYER_CAPE, LAYER_ROBE, LAYER_NONE };
// ARMOR_BACK,
    static const LAYER_TYPE sm_ArmorLayerChest = { LAYER_SHIRT, LAYER_CHEST, LAYER_TUNIC, LAYER_ROBE, LAYER_NONE };
// ARMOR_CHEST
    ...
    static const LAYER_TYPE sm_ArmorLayerLegs = { LAYER_PANTS, LAYER_SKIRT, LAYER_HALF_APRON, LAYER_ROBE, LAYER_LEGS, LAYER_NONE
 };     // ARMOR_LEGS,
    ...
    static const CArmorLayerType sm_ArmorLayers =       // layers covering the armor zone.
    {
    ...
    int CChar::CalcArmorDefense( void ) const
    {
        ...
        for ( CItem* pItem=GetContentHead(); pItem!=NULL; pItem=pItem->GetNext())
        {
                ...
                {
                case LAYER_HELM:                // 6
                        ArmorRegionMax = max( ArmorRegionMax, iDefense );
                        break;
        ...
        if ( iArmorCount )
        {
                ...
                {
                        iDefenseTotal += sm_ArmorLayers.m_wCoverage * ArmorRegionMax;
                }
        }
    ...
    {
        // Someone or something hit us. But there is no reciprocation here.
        // uType =
        ...
        if ( uType & ( DAMAGE_HIT_BLUNT | DAMAGE_HIT_PIERCE | DAMAGE_HIT_SLASH ))
        {
                ...
                if ( ! ( uType & DAMAGE_GENERAL ))
                {
                        ...
                        iDmg -= Calc_GetRandVal( pCharDef->m_defense );
                }
    ...
    effect_bounce:
        ...
        if ( uType & DAMAGE_FIRE )
        {
                if ( pCharDef->Can(CAN_C_FIRE_IMMUNE)) // immune to the fire part.
                {
                        ...
                        if ( ! ( uType & ( DAMAGE_HIT_BLUNT | DAMAGE_HIT_PIERCE | DAMAGE_HIT_SLASH | DAMAGE_POISON | DAMAGE_ELEC
TRIC )))
                                return( 0 );    // No effect.
                        iDmg /= 2;
                }
        ...
        if ( m_StatHealth <= 0 )
        {
                // We will die from this...make sure the killer is set correctly...if we don't do this, the person we are curren
tly
    ...
    int CChar::OnTakeDamageHitPoint( int iDmg, CChar * pSrc, DAMAGE_TYPE uType )
    {
                ...
                ! (uType & (DAMAGE_GOD|DAMAGE_ELECTRIC)))
        {
                ...
                if ( pShield != NULL && Skill_UseQuick( SKILL_PARRYING, Calc_GetRandVal((pSrc!=NULL) ? (pSrc->Skill_GetBase(SKIL
L_TACTICS)/10) : 100 )))
                {
                        // Damage the shield.
        ...
        {
                iHitRoll -= sm_ArmorLayers.m_wCoverage;
                if ( iHitRoll < 0 )
                        break;
                ...
                ! (uType & (DAMAGE_GENERAL|DAMAGE_GOD)))
        {
                ...
                static LPCTSTR const sm_Hit_Head1 =
                {
                ...
                static LPCTSTR const sm_Hit_Head2 =
                {
                ...
                static LPCTSTR const sm_Hit_Chest1 =
                {
                ...
                static LPCTSTR const sm_Hit_Chest2 =
                {
                ...
                {
                case ARMOR_HEAD:
                        ppMsg = (iDmg>10) ? sm_Hit_Head2 : sm_Hit_Head1 ;
                        break;
                case ARMOR_NECK:
                ...
                case ARMOR_BACK:
                        ...
                        break;
                case ARMOR_CHEST:
                        ppMsg = (iDmg>10) ? sm_Hit_Chest2 : sm_Hit_Chest1 ;
                        break;
                case ARMOR_ARMS:
                        ppMsg = sm_Hit_Arm;
                        break;
                ...
                if ( pSrc != this )
                {
                        ...
                        {
                                SysMessagef( "%s %s", ( pSrc == NULL ) ? "It" : (LPCTSTR) pSrc->GetName(), ppMsg );
                        }
                        if ( pSrc != NULL && pSrc->IsPriv(PRIV_DETAIL))
                        {
                                pSrc->SysMessagef( ppMsg, (LPCTSTR) GetName());
                        }
                }
        }
...
;
                ...
                const CArmorLayerType * pArmorLayer = &sm_ArmorLayers;
    
                for ( int i=0; pArmorLayer->m_pLayers != LAYER_NONE; i++ ) // layers covering the armor zone.
                {
                        if ( pArmorLayer->m_pLayers == layer )
                        {
    ...
    void CChar::Memory_Fight_Start( const CChar * pTarg )
    {
        ...
        if ( g_Log.IsLogged( LOGL_TRACE ))
        {
                DEBUG_MSG(( "Fight_Start '%s' attacks '%s', type 0%x" DEBUG_CR, (LPCTSTR) GetName(), (LPCTSTR) pTarg->GetName(),
 MemTypes ));
        }
    #endif
    ...
    int CChar::Fight_CalcDamage( CItem * pWeapon, SKILL_TYPE skill ) const
    {
        ...
        if ( skill != SKILL_ARCHERY )
                iDmgAdj = Calc_GetRandVal( Stat_Get(STAT_STR));
        else
                iDmgAdj = Calc_GetRandVal( Stat_Get(STAT_DEX));
        iDmg += iDmgAdj / 10;
    ...
    WAR_SWING_TYPE CChar::Fight_Hit( CChar * pCharTarg )
    {
        ...
        if ( skill == SKILL_ARCHERY )
        {
                ...
                if ( m_atFight.m_War_Swing_State == WAR_SWING_READY )
                {
                        ...
                        if ( OnTrigger( CTRIG_HitTry, pCharTarg ) == TRIGRET_RET_TRUE )
                                return( WAR_SWING_READY );
                ...
                pCharTarg->Effect( EFFECT_BOLT, (ITEMID_TYPE) pWeaponDef->m_ttWeaponBow.m_idAmmoX.GetResIndex(), this, 16, 0, fa
lse );
        }
        else
        {
        ...
        if ( m_Act_Difficulty < 0 )
        {
                if ( OnTrigger( CTRIG_HitMiss, pCharTarg ) == TRIGRET_RET_TRUE )
                        return( WAR_SWING_EQUIPPING );
                ...
                if ( IsPriv(PRIV_DETAIL))
                {
                        SysMessagef( "You Miss %s", (LPCTSTR) pCharTarg->GetName());
                }
                if ( pCharTarg->IsPriv(PRIV_DETAIL))
                {
                        pCharTarg->SysMessagef( "%s missed you.", (LPCTSTR) GetName());
                }
        ...
        if ( OnTrigger( CTRIG_Hit, pCharTarg ) == TRIGRET_RET_TRUE )
                return( WAR_SWING_EQUIPPING );

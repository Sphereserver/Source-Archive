    ...
    void CServer::SysMessage( LPCTSTR pszMsg, HUE_TYPE wHue ) const
    {
    ...
    void CServer::PrintAdminTelnet( LPCTSTR pszMsg ) const
    {
        if ( ! m_iAdminTelnets )
                return;
    ...
    void CServer::PrintStr( LPCTSTR pszMsg ) const
    {
        ...
        PrintAdminTelnet( pszMsg );     // echo out to admin telnets.
    }
    ...
    int CServer::PrintPercent( long iCount, long iTotal )
    {
        ...
        PrintAdminTelnet( sProgress );
        while ( len-- ) PrintAdminTelnet( "\x08" );     // backspace over it.
    ...
    LPCTSTR CServer::GetStatusStringRegister( bool fFirst ) const
    {
        ...
        {
                TCHAR szVersion;
                sprintf( pTemp, GRAY_TITLE ", Name=\"%s\", RegPass=\"%s\", Port=%d, Ver=\"%s\", TZ=%d, EMail=\"%s\", URL=\"%s\",
 Lang=\"%s\", CliVer=\"%s\", AccApp=%d" DEBUG_CR,
                        (LPCTSTR) GetName(),
                        (LPCTSTR) m_sRegisterPassword,
                        m_ip.GetPort(),
                        (LPCTSTR) GRAY_VERSION,
                        m_TimeZone,
                        (LPCTSTR) m_sEMail,
                        (LPCTSTR) m_sURL,
                        (LPCTSTR) m_sLang,
        ...
        {
                sprintf( pTemp, GRAY_TITLE ", Name=\"%s\", RegPass=\"%s\", Age=%i, Accounts=%d, Clients=%i, Items=%i, Chars=%i, 
Mem=%iK, Notes=\"%s\"" DEBUG_CR,
                        GetName(),
                        (LPCTSTR) m_sRegisterPassword,
                        GetAgeHours()/24,
    ...
    LPCTSTR CServer::GetStatusString( BYTE iIndex ) const
    {
        ...
        case 0x21:      // '!'
                ...
                {
                        TCHAR szVersion;
                        sprintf( pTemp, GRAY_TITLE ", Name=%s, Port=%d, Ver=%s, TZ=%d, EMail=%s, URL=%s, Lang=%s, CliVer=%s" DEB
UG_CR,
                                GetName(),
                                m_ip.GetPort(),
                                GRAY_VERSION,
    ...
    do_saving:
        ...
        case 'P':
                ...
                {
                        pSrc->SysMessagef( "Profiles %s: (%d sec total)" DEBUG_CR, m_Profile.IsActive() ? "ON" : "OFF", m_Profil
e.GetActiveWindow());
                        for ( int i=0; i < PROFILE_QTY; i++ )
                        {
                                pSrc->SysMessagef( "'%s' = %s" DEBUG_CR, (LPCTSTR) m_Profile.GetName((PROFILE_TYPE) i), (LPCTSTR
) m_Profile.GetDesc((PROFILE_TYPE) i ));
                        }
                }
                break;
        ...
        default:
                pSrc->SysMessagef( "unknown command '%c'" DEBUG_CR, sText );
                break;
        }
...
";
        ...
        // <p>The Web server cannot find the file or script you asked for. Please check the URL to ensure that the path is corre
ct</p>
    ...
    LPCTSTR const CServer::sm_szVerbKeys =
    {
        "ACCOUNT",
        "ALLCLIENTS",
        "B",
    ...
    bool CServer::r_Verb( CScript &s, CTextConsole * pSrc )
    {
        ...
        case SV_EXPORT: // "EXPORT" name  
                ...
                if ( s.HasArgs())
                {
                        ...
                        // IMPFLAGS_ITEMS
                        if ( ! g_World.Export( Arg_ppCmd, pSrc->GetChar(),
                                (Arg_Qty>=2)? atoi(Arg_ppCmd) : IMPFLAGS_ITEMS,
                                (Arg_Qty>=3)? atoi(Arg_ppCmd) : SHRT_MAX ))
                        {
        ...
        case SV_IMPORT: // "IMPORT" name  
                ...
                if (s.HasArgs())
                {
                        ...
                        // IMPFLAGS_ITEMS
                        if ( ! g_World.Import( Arg_ppCmd, pSrc->GetChar(),
                                (Arg_Qty>=2)?atoi(Arg_ppCmd) : IMPFLAGS_BOTH,
                                (Arg_Qty>=3)?atoi(Arg_ppCmd) : SHRT_MAX ))
                        {
    ...
    bool CServer::CommandLine( int argc, TCHAR * argv )
    {
        ...
        for ( int argn=1; argn<argc; argn++ )
        {
                ...
                case 'D':
                        // dump all the defines to a file.
                        {
                                ...
                                for ( ; i < g_Exp.m_VarDefs.GetCount(); i++ )
                                {
                                        ...
                                        File.Printf( "%s=%s\n",
                                                g_Exp.m_VarDefs->GetKey(), g_Exp.m_VarDefs->GetValStr());
                                }
                        }
                        continue;
                ...
                case '3':
                        ...
                        for ( int j=0; j<g_Cfg.m_ResHash.m_Array.GetCount(); j++ )
                        {
                                CResourceDef* pResDef = g_Cfg.m_ResHash.m_Array;
                                ASSERT(pResDef);
...
() );
                ...
                default:
    do_unknown:
                        g_Log.Event( LOGM_INIT|LOGL_CRIT, "Don't recognize command line data '%s'" DEBUG_CR, (LPCTSTR)( argv ));
                        break;
                }
        }
    ...
    CClient * CServer::SocketsReceive( CGSocket & socket, bool fGod ) // Check for messages from the clients
    {
        ...
        if ( hSocketClient < 0 || hSocketClient == INVALID_SOCKET )     // LINUX case is signed ?
        {
                ...
                g_Log.Event( LOGL_FATAL|LOGM_CLIENTS_LOG, "Failed at client connection to '%s'(?)" DEBUG_CR, (LPCTSTR) client_ad
dr.GetAddrStr());
                return NULL;
        }
    ...
    void CServer::SocketsReceive() // Check for messages from the clients
    {
        ...
        for ( pClient = GetClientHead(); pClient!=NULL; pClient = pClientNext )
        {
                ...
                {
                        pClient->m_timeLastEvent = CServTime::GetCurrentTime(); // We should always get pinged every couple minu
tes or so
    ...
    bool CServer::SocketsInit() // Initialize sockets
    {
        ...
        if ( ! iRet )
        {
                ...
                else
                {
                        ...
                        {
                                g_Log.Event( LOGM_INIT, "Alias '%s'." DEBUG_CR, (LPCTSTR) pHost->h_aliases );
                        }
                        // h_addrtype == 2
                        // h_length = 4

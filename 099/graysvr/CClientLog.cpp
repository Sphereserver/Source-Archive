    ...
    BYTE CClient::sm_xCompress_Buffer;  // static
    ...
    bool CClient::addRelay( const CServerDef * pServ )
    {
        ...
        cmd.Relay.m_ip = ( dwAddr >> 24 ) & 0xFF;
        cmd.Relay.m_ip = ( dwAddr >> 16 ) & 0xFF;
        cmd.Relay.m_ip = ( dwAddr >> 8  ) & 0xFF;
        cmd.Relay.m_ip = ( dwAddr          ) & 0xFF;
    ...
    LOGIN_ERR_TYPE CClient::Login_ServerList( const char * pszAccount, const char * pszPassword )
    {
        ...
        int iLen = sizeof(cmd.ServerList) - sizeof(cmd.ServerList.m_serv) + ( j * sizeof(cmd.ServerList.m_serv));
    ...
    REGRES_TYPE CClient::OnRxAutoServerRegister( const BYTE * pData, int iLen )
    {
        ...
        if ( pServNew->GetName() == '\0' )
        {
                ...
                g_Log.Event( LOGL_WARN|LOGM_ACCOUNTS, "%x:Bad reg info, No server name '%s'" DEBUG_CR, m_Socket.GetSocket(), (LP
CTSTR)pData );
                delete pServNew;
        ...
        {
                g_Log.Event( LOGL_WARN|LOGM_ACCOUNTS, "%x:Bad regpass '%s'!='%s' for server '%s'" DEBUG_CR, m_Socket.GetSocket()
, (LPCTSTR) pServNew->m_sRegisterPassword, (LPCTSTR) pServName->m_sRegisterPassword, (LPCTSTR) pServName->GetName());
                delete pServNew;
    ...
    bool CClient::OnRxConsoleLoginComplete()
    {
        ...
        {
                SysMessage( "Sorry you don't have telnet permission" DEBUG_CR );
                return( false );
        }
        ...
        m_Targ_Mode = CLIMODE_TELNET_READY;
        SysMessagef( "Welcome to the " GRAY_TITLE " console '%s','%s'" DEBUG_CR, GetName(), PeerName.GetAddrStr());
        return( true );
    }
    ...
    bool CClient::OnRxConsoleLogin()
    {
        ...
        ASSERT( m_ConnectType == CONNECT_TELNET );
        ASSERT( m_Targ_Mode == CLIMODE_TELNET_PASS );
    
        const TCHAR * pszUser = m_Targ_Text;
        ...
        CGString sMsg;
        if ( LogIn( szUser, szPass, sMsg ) != LOGIN_SUCCESS )
        {
                if ( ! sMsg.IsEmpty())
                {
                        SysMessage( sMsg );
    ...
    bool CClient::OnRxConsole( const BYTE * pData, int iLen )
    {
        ...
        while ( iLen -- )
        {
                ...
                case CLIMODE_TELNET_USERNAME:
                        ...
                        m_Targ_Text += ':';
                        m_Targ_Mode = CLIMODE_TELNET_PASS;
                ...
                case CLIMODE_TELNET_READY:
                        g_Serv.OnConsoleCmd( m_Targ_Text, this );
                        break;
                }
        }
    ...
    bool CClient::OnRxPing( const BYTE * pData, int iLen )
    {
        ...
        if ( pData == ' ' )
        {
                ...
                SysMessagef( "Welcome to %s Telnet" DEBUG_CR, (LPCTSTR) g_Serv.GetName());
                ...
                if ( g_Cfg.m_fLocalIPAdmin && PeerName.IsLocalAddr() )
                {
                        ...
                        CAccountRef pAccount = g_Accounts.Account_FindChecked( "Administrator" );
                ...
                m_Targ_Mode = CLIMODE_TELNET_USERNAME;
                SysMessage( "Username?:" DEBUG_CR );
                return( true );
        }
    
        // Answer the ping and drop.
        m_ConnectType = CONNECT_PING;
        LPCTSTR pTemp = g_Serv.GetStatusString( pData );
    ...
    bool CClient::OnRxWebPageRequest( BYTE * pRequest, int iLen )
    {
        ...
        TCHAR * ppRequest;
        int iQtyArgs = Str_ParseCmds( (TCHAR*) ppLines, ppRequest, COUNTOF(ppRequest), " " );
        if ( iQtyArgs < 2 )
                return( false );
        ...
        if ( ! memcmp( ppLines, "POST", 4 ))
        {
                ...
                g_Log.Event( LOGM_HTTP|LOGL_EVENT, "%x:HTTP Page Post '%s'" DEBUG_CR, m_Socket.GetSocket(), (LPCTSTR) ppRequest 
);
                ...
                // Is it one of our defined pages ?
                CWebPageDef * pWebPage = g_Cfg.FindWebPage(ppRequest);
                ...
                if ( pWebPage )
                {
                        if ( pWebPage->ServPagePost( this, ppRequest, ppLines, iContentLength ))
                                return fKeepAlive;
                }
        ...
        else
        {
                ...
                g_Log.Event( LOGM_HTTP|LOGL_EVENT, "%x:HTTP Page Request '%s', alive=%d" DEBUG_CR, m_Socket.GetSocket(), (LPCTST
R) ppRequest, fKeepAlive );
        }
    
        if ( CWebPageDef::ServPage( this, ppRequest, &dateIfModifiedSince ))
        {
                return fKeepAlive;
        }
        return( false );
    ...
    void CClient::xFlush()
    {
        ...
        int iLenRet;
        if ( m_ConnectType != CONNECT_GAME /* || m_Crypt.GetClientVer() >= 0x200040 */ )        // acting as a login server to t
his client.
        {
    ...
    void CClient::xSendReady( const void *pData, int length ) // We could send the packet now if we wanted to but wait til we ha
ve more.
    {
        ...
        if ( m_ConnectType == CONNECT_UNK ) // first thing
        {
                ...
                if ( iCountNew > 5 )
                {
                        if ( Event.m_CryptHeader == 0xFFFFFFFF )
                        {
                                ...
                                {
                                        // Server Registration message
                                        REGRES_TYPE retcode = OnRxAutoServerRegister( &Event.m_Raw, iCountNew-5 );

    ...
    bool CMailSMTP::SendLine( LPCTSTR pszMessage, int iResponseExpect )
    {
        ...
        // Send the message
        int iLenRet = m_Socket.Send( sMsg, sMsg.GetLength());
    ...
    bool CMailSMTP::ReceiveResponse()
    {
        ...
        TCHAR szResponse;
        int numReceived = m_Socket.Receive(szResponse,sizeof(szResponse)-1);
    ...
    bool CMailSMTP::OpenConnection(LPCTSTR szMailServerIPAddress)
    {
        // Create a socket
        bool fStatus = m_Socket.Create();
        if( ! fStatus)
                return( false );
    
        // connect to the SMTP Service (port 25)
        int iStatus = m_Socket.Connect(szMailServerIPAddress,PORT_SMTP);
        if( iStatus )
                return( false );
    ...
    bool CMailSMTP::SendMail( LPCTSTR pszDstMail, LPCTSTR pszSrcMail, CGObArray< CGString * > * pStrings )
    {
        // See RFC 821 for more on this.
        // mail on port 25
    
        LPCTSTR pszAddr = strrchr( pszDstMail, '@' );
        ...
        for ( int i=0;sm_Lines != NULL;i++)
        {
                CGString sMsg;
                switch ( i )
                {
                case 0: sMsg.Format( sm_Lines, (LPCTSTR) g_Serv.m_ip.GetAddrStr()); break;
                case 1: sMsg.Format( sm_Lines, pszSrcMail ); break;
                case 2: sMsg.Format( sm_Lines, pszDstMail ); break;
                default:
                        sMsg = sm_Lines;
                        break;
                }
        ...
        int iQty = pStrings->GetCount();
        for ( i=0; iQty--; i++ )
        {
                LPCTSTR pszLine = *(pStrings->GetAt(i));
                if ( pszLine == '.' &&
                        pszLine == '\r' )
                {
                        pszLine = "..";
    ...
    bool CMailPOP::Init()
    {
        ...
        if ( ! Create())
        {
                // g_Log.Event( LOGL_ERROR, "Unable to create socket" DEBUG_CR);
                return( false );
        }
        ...
        if ( ! Bind( PORT_SMTP ))
        {
                ...
                // g_Log.Event( LOGL_ERROR, "Unable to bind listen socket port %d - Error code: %i" DEBUG_CR, m_ip.GetPort(), bc
ode );
    ...
    bool CMailPOP::Check()
    {
        ...
        {
                SOCKET hSocket = m_MailClients->GetSocket();
        ...
        for ( i=0; i<m_MailClients.GetCount(); i++ )
        {
                ...
                {
                        TCHAR szData;
                        int count = m_MailClients->Receive( szData, sizeof( szData ));
        ...
        if ( FD_ISSET( GetSocket(), &readfds))
        {
                ...
                {
                        // g_Log.Event( LOGL_ERROR, "Error at client connection!" DEBUG_CR);
                        return false ;
                }

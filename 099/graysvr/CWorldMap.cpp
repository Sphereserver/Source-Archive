    ...
    CItem * CWorld::CheckNaturalResource( const CPointMap & pt, IT_TYPE Type, bool fTest )
    {
        // RETURN: 
        ...
        RESOURCE_ID rid = pResGroup->GetMemberID( pResGroup->GetRandMemberIndex());
        const CRegionResourceDef *pOreDef = dynamic_cast <const CRegionResourceDef *>( g_Cfg.ResourceGetDef( rid ));
        if ( pOreDef == NULL )
        {
                return( NULL );
        }
    
        pResBit = CItem::CreateScript( ITEMID_WorldGem );
        ASSERT(pResBit);
    
        pResBit->SetType( Type );
        pResBit->SetAttr( ATTR_INVIS | ATTR_MOVE_NEVER );       // use the newbie flag just to show that it has just been create
d.
    ...
    void CWorld::GetHeightPoint( const CPointMap & pt, CGrayMapBlockState & block, const CRegionBase * pRegion )
    {
        // Height of statics at/above given coordinates
        // do gravity here for the z.
    
        WORD wBlockThis = 0;    // what are the current blocking flags ?
        const CGrayMapBlock * pMapBlock = GetMapBlock( pt );
        ASSERT(pMapBlock);
    ...
    signed char CWorld::GetHeightPoint( const CPointBase & pt, WORD & wBlockFlags, const CRegionBase * pRegion ) // Height of pl
ayer who walked to X/Y/OLDZ
    {
        ...
        GetHeightPoint( pt, block, pRegion );

    ...
    void CChar::Action_StartSpecial( CREID_TYPE id )
    {
        ...
        case CREID_GIANT_SPIDER:
                // Leave a web patch.
                {
                        ...
                        CItem * pItem = CItem::CreateScript( (ITEMID_TYPE) sm_Webs );
    ...
    SKILL_TYPE CChar::Skill_GetBest( int iRank ) const // Which skill is the highest for character p
    {
        ...
        for ( int i=0;i<SKILL_QTY;i++)
        {
                ...
                {
                        if ( dwSkillTmp > pdwSkills )
                        {
                                memmove( pdwSkills+j+1, pdwSkills+j, iRank-j );
                                pdwSkills = dwSkillTmp;
                                break;
                        }
                }
        }
    
        dwSkillTmp = pdwSkills;
        delete  pdwSkills;
    ...
    short CChar::Skill_GetAdjusted( SKILL_TYPE skill ) const
    {
        ...
        case SKILL_PEACEMAKING:
                ...
                return( Skill_GetBase( (SKILL_TYPE) skill ) * sMusicianshipPercent + iAdjSkill );
                }
        };
        // END WESTY MOD
    ...
    int CChar::Skill_GetMax( SKILL_TYPE skill ) const
    {
        ...
        if ( m_pPlayer )
        {
                ...
                int iSkillMax = pSkillClass->m_SkillLevelMax;
    ...
    void CChar::Skill_Experience( SKILL_TYPE skill, int difficulty )
    {
        ...
        for ( int i=STAT_STR; i<STAT_BASE_QTY; i++ )
        {
                ...
                int iStatTarg = pSkillDef->m_Stat;
                ...
                int iChance = g_Cfg.m_StatAdv.GetChancePercent( difficulty );
    
                // adjust the chance by the percent of this that the skill uses.
                iChance = ( iChance * pSkillDef->m_StatBonus * pSkillDef->m_StatPercent ) / 10000;
                        ...
                        ( g_Cfg.m_wDebugFlags & DEBUGF_ADVANCE_STATS ))
                {
                        SysMessagef( "%s Difficult=%d Gain Chance=%d.%d%% Roll=%d%%",
                                (LPCTSTR) g_Stat_Name, difficulty, iChance/10, iChance%10, iRoll/10 );
                }
    #endif
                ...
                ! IsPriv( PRIV_GM ))
        {
                ...
                if ( iRoll < iChanceForLoss )
                {
                        ...
                        for ( int i=STAT_STR; i<STAT_BASE_QTY; i++ )
                        {
                                if ( iminval > pSkillDef->m_StatBonus )
                                {
                                        imin = i;
                                        iminval = pSkillDef->m_StatBonus;
                                }
                        }
    ...
    bool CChar::Skill_MakeItem( ITEMID_TYPE id, CGrayUID uidTarg, SKTRIG_TYPE stage )
    {
        ...
        if ( stage == SKTRIG_START )
        {
                ...
                CResourceQty RetMainSkill = pItemDef->m_SkillMake;
    ...
    int CChar::Skill_NaturalResource_Setup( CItem * pResBit )
    {
        ...
        const CRegionResourceDef * pOreDef = dynamic_cast <const CRegionResourceDef *>( g_Cfg.ResourceGetDef( pResBit->m_itResou
rce.m_rid_res ));
        if ( pOreDef == NULL )
        {
                return( -1 );
...
rce.m_rid_res ));
    ...
    #define SKILL_SMELT_FORGE_DIST 3
        ...
        else
        {
                ...
                {
                        RESOURCE_ID rid = pOreDef->m_BaseResources.GetResourceID();
                        ...
                        if ( pBaseDef->IsType( IT_GEM ))
                        {
                                ...
                                ASSERT(pGem);
                                pGem->SetAmount( iOreQty * pBaseDef->m_BaseResources.GetResQty() );
                                ItemBounce( pGem );
                                continue;
                        }
                        if ( pBaseDef->IsType( IT_INGOT ))
                        {
                                ...
                                pIngotDef = pBaseDef;
                                iIngotQty = pOreDef->m_BaseResources.GetResQty();
                        }
                }
        }
        ...
        if (( dirPrv != dir ) || ! Calc_GetRandVal(10))
        {
                ...
                if ( dist )
                {
                        ...
                        if ( dist < 16 ) // Closing in message?
                                ...
                                pszDist = " very far";
                        sMsgDist.Format( _TEXT( "%s to the %s" ), pszDist, (LPCTSTR) CPointBase::sm_szDirs );
                }
                else
                {
                        sMsgDist = _TEXT(" here");
    ...
    int CChar::Skill_Alchemy( SKTRIG_TYPE stage )
    {
        ...
        CResourceQty item = pPotionDef->m_BaseResources;
    ...
    int CChar::Skill_Taming( SKTRIG_TYPE stage )
    {
        ...
        if ( stage == SKTRIG_STROKE )
        {
                ...
                CGString sSpeak;
                sSpeak.Format( sm_szTameSpeak, (LPCTSTR) pChar->GetName());
                Speak( sSpeak );
...
!", 
    ...
    int CChar::Skill_Fighting( SKTRIG_TYPE stage )
    {
        ...
        if ( stage == SKTRIG_START )
        {
                ...
                int iDifficulty = g_Cfg.Calc_CombatChanceToHit( this, Skill_GetActive(), m_Act_Targ.CharFind(), m_uidWeapon.Item
Find());
    ...
    int CChar::Skill_Inscription( SKTRIG_TYPE stage )
    {
        if ( stage == SKTRIG_START )
        {
                ...
                {
                        const CSpellDef * pSpellDef = g_Cfg.GetSpellDef( (SPELL_TYPE)dwSpell );
                        if ( pSpellDef == NULL )
                                continue;
                        if ( pSpellDef->m_idScroll == m_atCreate.m_ItemID )
                        {
                                // Consume mana.
                                UpdateStatVal( STAT_INT, -pSpellDef->m_wManaUse );
                        }
                }
        }
    ...
    void CChar::Skill_Fail( bool fCancel )
    {
        ...
        if ( pSkillDef->HasTrigger( SKTRIG_FAIL ))
        {
                ...
                {
                        TRIGRET_TYPE iRet = CScriptObj::OnTriggerScript( s, CSkillDef::sm_szTrigName, this );
                        if ( iRet == TRIGRET_RET_TRUE  )
                                goto do_cleanup;
                }
        }

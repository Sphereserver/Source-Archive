    ...
    LPCTSTR CProfileData::GetName( PROFILE_TYPE id ) const
    {
        ...
        static LPCTSTR const sm_pszProfileName =
        {
                "IDLE",
    ...
    LPCTSTR CProfileData::GetDesc( PROFILE_TYPE id ) const
    {
        ...
        int iCount = m_PrvTimes.m_iCount;
    
        if ( id >= PROFILE_TIME_QTY )
        {
                sprintf( pszTmp, "%i bytes", (int) m_PrvTimes.m_Time );
        }
        else if ( m_Frequency )
        {
                sprintf( pszTmp, "%i.%04i sec ",
                        (int)( m_PrvTimes.m_Time / ( m_Frequency )),
                        (int)((( m_PrvTimes.m_Time * 10000 ) / ( m_Frequency )) % 10000 ),
                        iCount );
        }
        else
        {
                sprintf( pszTmp, "%i ticks ",
                        (int)( m_PrvTimes.m_Time ),
                        iCount );
        }
        return( pszTmp );
    }
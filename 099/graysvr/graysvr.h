    ...
    class CClient : public CGObListRec, public CChatChanMember, public CTextConsole, public CScriptObj
    {
    ...
    private:
        ...
        DWORD   m_Walk_LIFO;    // Client > 1.26 must match these .
    ...
    public:
        ...
        static BYTE sm_xCompress_Buffer;
    ...
    public:
        ...
        union
        {
                ...
                CGrayUIDBase m_tmSetupCharList;
    ...
    #define ADMIN_CLIENTS_PER_PAGE 8
                        DWORD m_Item;   // This saves the inrange tracking targets or other context
                } m_tmGumpAdmin;
    ...
    private:
        ...
        CQueueBytes m_bout;             // CCommand out buffer. (to client) (we can build output to multiple clients at the same
 time)
    ...
    private:
        ...
        LOGIN_ERR_TYPE Login_ServerList( const char * pszAccount, const char * pszPassword );           // Initial login (Login 
on "loginserver", new format)
        bool Login_Relay( int iServer );                        // Relay player to a certain IP
        void Announce( bool fArrive ) const;
    
        LOGIN_ERR_TYPE Setup_CharListReq( const char * pszAccount, const char * pszPassword, DWORD dwAccount ); // Gameserver lo
gin and character listing
    ...
    public:
        bool Event_DoubleClick( CGrayUID uid, bool fMacro, bool fTestTouch );
    ...
    public:
        ...
        bool r_GetRefSingle( LPCTSTR & pszKey, CScriptObj * & pRef );
        bool r_VerbSingle( CScript & s, CTextConsole * pSrc ); // Execute script type command on me
        bool r_WriteValSingle( LPCTSTR pszKey, CGString & s, CTextConsole * pSrc );
        bool r_LoadValSingle( CScript & s );
        void r_DumpLoadKeysSingle( CTextConsole * pSrc );
        void r_DumpVerbKeysSingle( CTextConsole * pSrc );
    
        bool r_GetRef( LPCTSTR & pszKey, CScriptObj * & pRef );
        bool r_Verb( CScript & s, CTextConsole * pSrc ); // Execute script type command on me
        bool r_WriteVal( LPCTSTR pszKey, CGString & s, CTextConsole * pSrc );
        bool r_LoadVal( CScript & s );
        ...
        void xSendReady( const void *pData, int length ); // We could send the packet now if we wanted to but wait til we have m
ore.
        ...
        void xProcessMsg( bool fGood ); // Process a packet
        bool xDispatchMsg();
        ...
        int  addContents( const CItemContainer * pCont, bool fCorpseEquip = false, bool fCorpseFilter = false, bool fShop = fals
e ); // Send items
        ...
        void addEffect( EFFECT_TYPE motion, ITEMID_TYPE id, const CObjBaseTemplate * pDst, const CObjBaseTemplate * pSrc, BYTE s
peed = 5, BYTE loop = 1, bool explode = false );
        void addSound( SOUND_TYPE id, const CObjBaseTemplate * pBase = NULL, int iRepeat = 1 );
        void addReSync();
        void addPing( BYTE bCode = 0 );
    
        void addBark( LPCTSTR pText, const CObjBaseTemplate * pSrc, HUE_TYPE wHue = HUE_TEXT_DEF, TALKMODE_TYPE mode = TALKMODE_
SAY, FONT_TYPE font = FONT_BOLD );
        void addBarkUNICODE( const NCHAR * pText, const CObjBaseTemplate * pSrc, HUE_TYPE wHue = HUE_TEXT_DEF, TALKMODE_TYPE mod
e = TALKMODE_SAY, FONT_TYPE font = FONT_BOLD, CLanguageID lang = 0 );
        void addBarkSpeakTable( SPKTAB_TYPE index, const CObjBaseTemplate * pSrc, HUE_TYPE wHue = HUE_TEXT_DEF, TALKMODE_TYPE mo
de = TALKMODE_SAY, FONT_TYPE font = FONT_BOLD );
        void addObjMessage( LPCTSTR pMsg, const CObjBaseTemplate * pSrc, HUE_TYPE wHue = HUE_TEXT_DEF ); // The message when an 
item is clicked
        void addSysMessage( LPCTSTR pMsg, HUE_TYPE wHue = HUE_TEXT_DEF ); // System message (In lower left corner)
        ...
        void addTarget( CLIMODE_TYPE targmode, LPCTSTR pMsg, bool fAllowGround = false, bool fCheckCrime=false ); // Send target
ting cursor to client
        ...
        int  addShopMenuSellFind( CItemContainer * pSearch, CItemContainer * pFrom1, CItemContainer * pFrom2, int iConvertFactor
, CCommand * & pCur );
        ...
        void addCharStatWindow( CGrayUID uid ); // Opens the status window
        ...
        void addGumpDialog( CLIMODE_TYPE mode, const CGString * sControls, int iControls, const CGString * psText, int iTexts, i
nt x, int y, CObjBase * pObj = NULL );
        ...
        void SysMessage( LPCTSTR pMsg, HUE_TYPE wHue = HUE_TEXT_DEF ) const; // System message (In lower left corner)
        bool CanSee( const CObjBaseTemplate * pObj ) const;
        bool CanHear( const CObjBaseTemplate * pSrc, TALKMODE_TYPE mode ) const;
    
        bool Dialog_Setup( CLIMODE_TYPE mode, RESOURCE_ID_BASE rid, CObjBase * pObj );
    ...
    {
    protected:
        CProfileDataRec m_PrvTimes;
        CProfileDataRec m_CurTimes;
    ...
    public:
        ...
        // admin console.
        int m_iAdminTelnets;            // how many of my clients are admin consoles ?
        CGString m_sConsoleText;                // local console input.
    ...
    public:
        ...
        void SysMessage( LPCTSTR pMsg, HUE_TYPE wHue = HUE_TEXT_DEF ) const;
        void PrintAdminTelnet( LPCTSTR pszMsg ) const;
    ...
    extern class CBackTask : public CGrayThread
    {
    ...
    extern class CMainTask : public CGrayThread
    {

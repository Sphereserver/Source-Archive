    ...
    LPCTSTR const CChar::sm_szTrigName =        // static
    {
        _TEXT("@AAAUNUSED"),
    ...
    bailout:
        ...
        if ( ! IsIndividualName() && pCharDef->GetTypeName() == '#' )
        {
    ...
    bool CChar::ReadScriptTrig( CCharBase * pCharDef, CTRIG_TYPE trig )
    {
        ...
        if ( ! pCharDef->ResourceLock(s))
                return( false );
        if ( ! OnTriggerFind( s, sm_szTrigName ))
                return( false );
        return( ReadScript( s ));
    }
    ...
    void CChar::InitPlayer( const CEvent * pBin, CClient * pClient )
    {
        ...
        if ( g_Cfg.IsObscene(pBin->Create.m_charname))  // Is the name unacceptable?
        {
                g_Log.Event( LOGL_WARN|LOGM_ACCOUNTS,
                        ...
                        pClient->m_Socket.GetSocket(), (LPCTSTR) pBin->Create.m_charname, (LPCTSTR) pClient->GetAccount()->GetNa
me() );
        ...
        if ( ! g_Cfg.m_StartDefs.IsValidIndex( iStartLoc ))
                iStartLoc = 0;
        m_ptHome = g_Cfg.m_StartDefs->m_pt;
    ...
    LPCTSTR const CChar::sm_szRefKeys =
    {
        "ACCOUNT",
        "ACT",
        "FINDEQUIP",
    ...
    bool CChar::r_GetRef( LPCTSTR & pszKey, CScriptObj * & pRef )
    {
        if ( IsClient())
        {
                if ( GetClient()->r_GetRefSingle( pszKey, pRef ))
                        return( true );
        }
    ...
    LPCTSTR const CChar::sm_szLoadKeys =
    {
        "ac",
        "ACCOUNT",
        "ACT",
    ...
    void CChar::r_DumpLoadKeys( CTextConsole * pSrc )
    {
        if ( IsClient())
        {
                GetClient()->r_DumpLoadKeysSingle(pSrc);
        }
    ...
    void CChar::r_DumpVerbKeys( CTextConsole * pSrc )
    {
        if ( IsClient())
        {
                GetClient()->r_DumpVerbKeysSingle(pSrc);
        }
    ...
    bool CChar::r_WriteVal( LPCTSTR pszKey, CGString & sVal, CTextConsole * pSrc )
    {
        ...
        if ( IsClient())
        {
                if ( GetClient()->r_WriteValSingle( pszKey, sVal, pSrc ))
                        return( true );
        }
    ...
    do_default:
                if ( m_pPlayer )
                {
                        if ( m_pPlayer->r_WriteValPlayer( this, pszKey, sVal ))
                                return( true );
                }
                if ( m_pNPC )
                {
                        if ( m_pNPC->r_WriteValNPC( this, pszKey, sVal ))
                                return( true );
                }
                ...
                if ( i >= 0 && IsSkillBase((SKILL_TYPE)i))
                {
        ...
        case CHC_SKILLCHECK:    // odd way to get skills checking into the triggers.
                ...
                SKILL_TYPE iSkill = g_Cfg.FindSkillKey( ppArgs );
                if ( iSkill == SKILL_NONE )
                        return( false );
                sVal.FormatVal( Skill_CheckSuccess( iSkill, Exp_GetVal( ppArgs )));
                }
                return( true );
        ...
        case CHC_SEX:   // <SEX milord/milady>  sep chars are :,/
                ...
                TCHAR * ppArgs;
                Str_ParseCmds( (TCHAR*) pszKey, ppArgs, COUNTOF(ppArgs), ":,/" );
                sVal = ( pCharDef->IsFemale()) ? ppArgs : ppArgs;
                }
                return( true );
        case CHC_ISEVENT:
                if ( pszKey != '.' )
    ...
    do_default:
                if ( m_pPlayer )
                {
                        if ( m_pPlayer->r_LoadValPlayer( this, s ))
                                return( true );
                }
                if ( m_pNPC )
                {
                        if ( m_pNPC->r_LoadValNPC( this, s ))
                                return( true );
                }
        ...
        case CHC_FLAGS:
                ...
                m_StatFlag = ( s.GetArgVal() &~ (STATF_SaveParity|STATF_Pet|STATF_Spawned)) | ( m_StatFlag & (STATF_SaveParity|S
TATF_Pet|STATF_Spawned) );
                return true;
        case CHC_FONT:
    ...
    void CChar::r_Serialize( CGFile & a )
    {
        ...
        if ( m_pPlayer )
        {
                m_pPlayer->r_SerializePlayer(this, a);
        }
        if ( m_pNPC )
        {
                m_pNPC->r_SerializeNPC(this, a);
        }
    ...
    void CChar::r_Write( CScript & s )
    {
        ...
        if ( m_pPlayer )
        {
                m_pPlayer->r_WritePlayer(this, s);
        }
        if ( m_pNPC )
        {
        ...
        for ( ;j<STAT_QTY;j++)
        {
                if ( ! m_Stat )
                        continue;
                s.WriteKeyVal( g_Stat_Name, m_Stat );
        }
    ...
    LPCTSTR const CChar::sm_szVerbKeys =
    {
        "AFK",
        "ALLSKILLS",
        "ANIM",         // "Bow", "Salute"
    ...
    bool CChar::r_Verb( CScript &s, CTextConsole * pSrc ) // Execute command from script
    {
        ...
        {
                if ( GetClient()->r_VerbSingle( s, pSrc ))
                        return( true );
        }
        ...
        case CHV_ANIM:
                ...
                {
                        int Arg_piCmd;          // Maximum parameters in one line
                        int Arg_Qty = Str_ParseCmds( s.GetArgRaw(), Arg_piCmd, COUNTOF(Arg_piCmd));
    
                        return UpdateAnimate( (ANIM_TYPE) Arg_piCmd, false,
                                ( Arg_Qty > 1 ) ? Arg_piCmd : false,
                                ( Arg_Qty > 2 ) ? Arg_piCmd : 1 );
                }
                break;
        case CHV_ATTACK:
        ...
        case CHV_DUPE:  // = dupe a creature !
                {
                        CChar * pChar = CChar::CreateNPC( GetID());
                        ASSERT(pChar);
    
                        // What about all the stuff it's wearing ???s
                        pChar->MoveNearObj( pSrc->GetChar() ? pSrc->GetChar() : this , 1 );
                }
                break;
        case CHV_TAGLIST:
        ...
        case CHV_KILL:
                {
                        ...
                        OnTakeDamage( 20000, pCharSrc, DAMAGE_GOD );
                        m_StatHealth = 0;
                        g_Log.Event( LOGL_EVENT|LOGM_KILLS|LOGM_GM_CMDS, "'%s' was KILLed by '%s'" DEBUG_CR, (LPCTSTR) GetName()
, (LPCTSTR) pSrc->GetName());
                }
                break;
    ...
    basicform:
                                ...
                                else
                                {
                                        ...
                                        sMsg.Format( "I am in %s in %s (%s)",
                                                (LPCTSTR) m_pArea->GetName(), (LPCTSTR) pRoom->GetName(), (LPCTSTR) GetTopPoint(
).WriteUsed());
                                }
                        }
                        else

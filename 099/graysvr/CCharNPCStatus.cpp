    ...
    int CChar::NPC_GetVendorMarkup( const CChar * pChar ) const
    {
        ...
        CVarDefBase * pVar = m_TagDefs.FindKeyPtr("VENDORMARKUP");
        if ( pVar )
        {
    ...
    int CChar::NPC_OnHearName( LPCTSTR pszText ) const
    {
        ...
        for ( i=0; pszText != '\0'; i++ )
        {
                ...
                {
                        // found name.
                        while ( ISWHITESPACE( pszText ))
                                i ++;
                        return( i );    // Char name found
                }
                if ( toupper( pszName ) != toupper( pszText ))  // not the name.
                        break;
        }
    ...
    bool CChar::NPC_CheckWalkHere( const CPointBase & pt, const CRegionBase * pArea, WORD wBlockFlags ) const
    {
        ...
        if ( m_pArea != NULL ) // most decisions are based on area.
        {
                ...
                if ( Noto_IsCriminal() && Stat_Get(STAT_INT) > 20 )     // I'm evil
                {
        ...
        while (true)
        {
                ...
                case IT_TELEPAD:
    try_avoid:
                        if ( Calc_GetRandVal( Stat_Get(STAT_INT)) > Calc_GetRandVal( iIntToAvoid ))
                                return( false );
                        break;
                }
        }
    ...
    CItemVendable * CChar::NPC_FindVendableItem( CItemVendable * pVendItem, CItemContainer * pContBuy, CItemContainer * pContSto
ck ) // static
    {
    ...
    int CChar::NPC_WantThisItem( CItem * pItem ) const
    {
        ...
        if ( iRet >= 0 )
        {
                return( pCharDef->m_Desires.GetResQty() );
        }
    ...
    domemorybase:
        ...
        CItemMemory * pMemory = Memory_FindObjTypes( pCharTarg, MEMORY_FIGHT|MEMORY_HARMEDBY|MEMORY_IRRITATEDBY|MEMORY_SAWCRIME|
MEMORY_AGGREIVED );
        if ( pMemory )
        {
                iHostility += 50;

    ...
    bool CLog::OpenLog( LPCTSTR pszBaseDirName )        // name set previously.
    {
        ...
        {
                if ( pszBaseDirName && pszBaseDirName == '\0' )
                {
                        if ( *pszBaseDirName == '0' )
                        {
                                Close();
    ...
    int CLog::EventStr( DWORD wMask, LPCTSTR pszMsg )
    {
        ...
        try
        {
                ...
                {
    #ifndef _WIN32
                        Open(NULL, OF_SHARE_DENY_WRITE|OF_READWRITE|OF_TEXT);   // LINUX needs to close and re-open for each log
 line !
    #endif
                }
                ...
                if ( pThread && pThread->m_pScriptContext )
                {
                        ...
                        sprintf( szScriptContext, "(%s,%d)", pThread->m_pScriptContext->GetFileTitle(), LineContext.m_iLineNum )
;
                }
                else
                {
    ...
    #ifdef _WIN32
    #ifdef _CONSOLE
                        SetConsoleTextAttribute( hOut, FOREGROUND_RED|BACKGROUND_RED|BACKGROUND_GREEN|BACKGROUND_BLUE|FOREGROUND
_INTENSITY );
    #else

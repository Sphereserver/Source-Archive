...
() );
        ...
        szData = 0;
        int iLen = strcpylen( &szData, g_Serv.GetStatusStringRegister(fFirst), sizeof( szData )-5);
        iLen += 5;
    ...
    void CBackTask::PollServers()
    {
        ...
        for ( int i=0; ! g_Serv.m_iExitFlag; i++ )
        {
                ...
                if ( g_Cfg.m_sMainLogServerDir.IsEmpty())       // the main server does not do this because too many people use 
dial ups .
                {
                        if ( pServ->PollStatus())
                                continue;

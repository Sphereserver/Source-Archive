// ntservice.cpp
    ...
    bailout1:
        ...
        hEvents = m_hServerStopEvent;
        hEvents = CreateEvent(NULL, TRUE, FALSE, NULL);
        if (hEvents == NULL)
        {
                goto bailout1;
        }
    ...
    bailout3:
        ...
        else
        {
                unsigned long lType;
                unsigned long lSize;
                RegQueryValueEx(hKey, "WorkingPath", 0, &lType, (unsigned char *) &szGlobalPath, &lSize);
        }
    ...
    void CNTService::CmdInstallService()
    {
        ...
        SC_HANDLE schService = CreateService(
                ...
                szInternalName,                         // Internal name of the service (used when controlling the service using
 "net start" or "netsvc")
    ...
    bailout2:
        ...
        if (RegSetValueEx(hKey, "WorkingPath", 0, REG_SZ, (const unsigned char *) &szPath, strlen(szPath)))
        {
                ReportEvent( EVENTLOG_ERROR_TYPE, 0,
                        "Install RegSetValueEx", GetLastErrorText(szErr, sizeof(szErr)));
        }
    ...
    )
    {
        TCHAR * argv;
        argv = NULL;
        int argc = Str_ParseCmds( lpCmdLine, &argv, COUNTOF(argv)-1, " \t" ) + 1;
    #else
    ...
    do_not_nt_service:
        ...
        // HWINSTA hWndStation = GetProcessWindowStation();
    
        // NOTE: we have no idea if we are being run as an NT service or not yet.
        // Open the temporary log file.
        g_Log.Open( "c:\\" GRAY_FILE "svc.log", OF_WRITE | OF_TEXT | OF_SHARE_DENY_WRITE );
    
        g_Service.ReportEvent( EVENTLOG_INFORMATION_TYPE, 0, 
                "Start", g_Serv.GetName());
        ...
        if ( ! g_Cfg.LoadIni( true ))
        {
                ...
                if (szPath == 0x00)
                {
                        g_Service.ReportEvent( EVENTLOG_ERROR_TYPE, 0, 
                                "Couldn't find the application path!" );

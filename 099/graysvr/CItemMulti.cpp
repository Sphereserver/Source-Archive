    ...
    void CItemMulti::Multi_Create( CChar * pChar, DWORD dwKeyCode )
    {
        ...
        {
                fNeedKey |= Multi_CreateComponent( (ITEMID_TYPE) pMultiDef->m_Components.m_id,
                        pMultiDef->m_Components.m_dx,
                        pMultiDef->m_Components.m_dy,
                        pMultiDef->m_Components.m_dz,
                        dwKeyCode );
        }
    ...
    int CItemMulti::Multi_IsComponentOf( const CItem * pItem ) const
    {
        ...
        for ( int i=0; i<iQty; i++ )
        {
                if ( xdiff != pMultiDef->m_Components.m_dx ||
                        ydiff != pMultiDef->m_Components.m_dy )
                        continue;
                return( i );
        }
    ...
    bool CItemMulti::Ship_Face( DIR_TYPE dir )
    {
        ...
        for ( i=0; i<iCount; i++ )
        {
                ...
                if ( pObj->IsItem())
                {
                        ...
                        if ( i>=0 )
                        {
                                pItem->SetDispID( pMultiNew->m_Components.m_id );
                                pt = GetTopPoint();
                                pt.m_x += pMultiNew->m_Components.m_dx;
                                pt.m_y += pMultiNew->m_Components.m_dy;
                                pt.m_z += pMultiNew->m_Components.m_dz;
                                pItem->MoveTo( pt );
                                continue;
                        }
                }
    ...
    void CItemMulti::OnHearRegion( LPCTSTR pszCmd, CChar * pSrc )
    {
        ...
        {
                CResourceLink * pLink = pMultiDef->m_Speech;
                ASSERT(pLink);
                CResourceLock s;
    ...
    LPCTSTR const CItemMulti::sm_szVerbKeys =
    {
        "SHIPANCHORDROP",
        "SHIPANCHORRAISE",
        "SHIPBACK",
    ...
    bool CItemMulti::r_Verb( CScript & s, CTextConsole * pSrc ) // Execute command from script
    {
        ...
        DIR_TYPE DirFace = sm_Ship_FaceDir;
        int DirMoveChange;

    ...
    CResource::CResource()
    {
        ...
        m_fUseIRC = false;
        m_iPollServers = 15 * 60 * TICK_PER_SEC;
    ...
    CResource::~CResource()
    {
        ...
        {
                CResourceDef* pResDef = m_ResHash.m_Array;
                ASSERT(pResDef);
                pResDef->UnLink();
        }
        Unload(false);
    ...
    enum RC_TYPE
    {
        ...
        RC_ALLOWKEYLOCKDOWN,    // m_fAllowKeyLockdown
        RC_ANSI,
    ...
    const CAssocReg CResource::sm_szLoadKeys =
    {
        { "ACCTFILES",                          { ELEM_CSTRING, offsetof(CResource,m_sAcctBaseDir)      }},
        { "ALLOWKEYLOCKDOWN",           { ELEM_BOOL,    offsetof(CResource,m_fAllowKeyLockdown) }},
        ...
        { "LOCALIPADMIN",                       { ELEM_BOOL,    offsetof(CResource,m_fLocalIPAdmin)     } }, // The local ip is 
assumed to be the admin.
        { "LOG" },
        { "LOGMASK" },          // GetLogMask
        ...
        { "MURDERMINCOUNT",                     { ELEM_INT,     offsetof(CResource,m_iMurderMinCount)   }},             // amoun
t of murders before we get title.
    ...
    {
        r_DumpKeys( pSrc, (LPCTSTR const *) sm_szLoadKeys, sizeof(sm_szLoadKeys));
    }
    
    bool CResource::r_LoadVal( CScript &s )
    {
        int i = FindTableHeadSorted( s.GetKey(), (LPCTSTR const *) sm_szLoadKeys, COUNTOF( sm_szLoadKeys )-1, sizeof(sm_szLoadKe
ys));
        if ( i < 0 )
        {
        ...
        default:
                return( sm_szLoadKeys.m_elem.SetValStr( this, s.GetArgRaw()));
        }
    ...
    bool CResource::r_WriteVal( LPCTSTR pszKey, CGString & sVal, CTextConsole * pSrc )
    {
        ...
        int i = FindTableHeadSorted( pszKey, (LPCTSTR const *) sm_szLoadKeys, COUNTOF( sm_szLoadKeys )-1, sizeof(sm_szLoadKeys) 
);
        if ( i<0 )
        {
        ...
        default:
                return( sm_szLoadKeys.m_elem.GetValStr( this, sVal ));
        }
    ...
    void CResource::r_Write( CScript &s )
    {
        ...
        {
                if ( sm_szLoadKeys.m_elem.m_type != ELEM_VOID )
                {
                        if ( ! sm_szLoadKeys.m_elem.CompareVal( this, &defaults ))
                                continue;
                }
                ...
                CGString sVal;
                if ( ! r_WriteVal( sm_szLoadKeys.m_pszKey, sVal, &g_Serv ))
                        continue;
    
                s.WriteKey( sm_szLoadKeys.m_pszKey, sVal );
        }
    }
    ...
    CServerRef CResource::Server_GetDef( int index )
    {
        ...
        if ( ! m_Servers.IsValidIndex(index))
                return( NULL );
        return( CServerRef( STATIC_CAST <CServerDef*>( m_Servers )));
    }
    ...
    takedefault:
                ...
                return( STATIC_CAST <CWebPageDef*>( m_WebPages ));
        }
        ...
        {
                if ( m_WebPages == NULL )       // not sure why this would happen
                        continue;
                CWebPageDef * pWeb = STATIC_CAST <CWebPageDef*>(m_WebPages );
                ASSERT(pWeb);
    ...
    bool CResource::IsObscene( LPCTSTR pszText ) const
    {
        ...
        {
                MATCH_TYPE ematch = Str_Match( m_Obscene, pszText );
    ...
    const CGrayMulti * CResource::GetMultiItemDefs( ITEMID_TYPE itemid )
    {
        ...
        {
                m_MultiDefs->HitCacheTime();
        }
        const CGrayMulti * pMulti = m_MultiDefs;
        ASSERT(pMulti);
        return( pMulti );
    }
    ...
    PLEVEL_TYPE CResource::GetPrivCommandLevel( LPCTSTR pszCmd ) const
    {
        ...
        for ( ; ilevel >= 0; ilevel-- )
        {
                LPCTSTR const * pszTable = m_PrivCommands.GetBasePtr();
                int iCount = m_PrivCommands.GetCount();
    ...
    CPointMap CResource::GetRegionPoint( LPCTSTR pCmd ) const // Decode a teleport location number into X/Y/Z
    {
        ...
        if ( pCmd == '-' )      // Get location from start list.
        {
                int i = ( - atoi(pCmd)) - 1;
                if ( ! m_StartDefs.IsValidIndex( i ))
                        i = 0;
                return( m_StartDefs->m_pt );
        }
    ...
    bool CResource::LoadResourceSection( CScript * pScript )
    {
        ...
        if ( restype < 0 )
        {
                // support old versions that just had special files with  entries
                ...
                if ( restype < 0 )
                {
                        g_Log.Event( LOGL_WARN|LOGM_INIT, "Unknown section '%s' in '%s'" DEBUG_CR, (LPCTSTR) pScript->GetKey(), 
(LPCTSTR) pScript->GetFileTitle());
                        return( false );
                }
        }
        else
        ...
        if ( ! rid.IsValidUID())
        {
                DEBUG_ERR(( "Invalid %s block index '%s'" DEBUG_CR, (LPCTSTR) pScript->GetSection(), (LPCTSTR) pScript->GetArgSt
r()));
                return( false );
        }
        ...
        case RES_ADVANCE:
                ...
                while ( pScript->ReadKeyParse())
                {
                        ...
                        if ( i >= STAT_BASE_QTY )
                                continue;
                        m_StatAdv.Load( pScript->GetArgStr());
                }
                return( true );
        ...
        case RES_PLEVEL:
                {
                        ...
                        while ( pScript->ReadKey())
                        {
                                m_PrivCommands.AddSortString( pScript->GetKey());
                        }
                }
                return( true );
        case RES_RESOURCES:
...
;
    ...
    RESOURCE_ID CResource::ResourceGetNewID( RES_TYPE restype, LPCTSTR pszName, CVarDefNum ** ppVarNum )
    {
        ...
        if ( pszName == '\0' )  // absense of resourceid = index 0
        {
                // This might be ok.
                return( RESOURCE_ID( restype, 0, iPage ) );
        }
    
        if ( isdigit(pszName))  // Its just an index.
        {
                ...
                case RES_SCROLL:                // SCROLL_GUEST=message scroll sent to player at guest login. SCROLL_MOTD: SCROL
L_NEWBIE
                ...
                if ( g_Serv.m_iModeCode != SERVMODE_ResyncLoad )        // this really is ok.
                {
                        ...
                        if ( index >= 0 )       // i found it. So i have to find something else.
                        {
                                ...
                                ASSERT(pDef);
                                DEBUG_WARN(( "redefinition of '%s' block %d ('%s')" DEBUG_CR, (LPCTSTR) GetResourceBlockName(res
type), index, (LPCTSTR) pDef->GetName() ));
                        }
                }
    #endif
        ...
        CVarDefBase * pVarBase = g_Exp.m_VarDefs.FindKeyPtr( pszName );
        if ( pVarBase )
        {
                ...
                if ( restype != rid.GetResType())
                {
                        ...
                        case RES_PLEVEL:
                                ...
                                break;
                        default:
                                DEBUG_ERR(( "Redefined name '%s' from %s to %s" DEBUG_CR, (LPCTSTR) pszName, (LPCTSTR) GetResour
ceBlockName(rid.GetResType()), (LPCTSTR) GetResourceBlockName(restype) ));
                                return( ridinvalid );
                        }
                }
        ...
        if ( pszName )
        {
                ...
                int iVarNum = g_Exp.m_VarDefs.SetNumNew( pszName, rid.GetPrivateUID() );
                if ( iVarNum >= 0 )
                {
    ...
    CResourceDef * CResource::ResourceGetDef( RESOURCE_ID_BASE rid ) const
    {
        ...
        case RES_SKILL:
                if ( ! m_SkillIndexDefs.IsValidIndex(index))
                        return( NULL );
                return( const_cast <CSkillDef *>( m_SkillIndexDefs ));
    
        case RES_SPELL:
                if ( ! m_SpellDefs.IsValidIndex(index))
                        return( NULL );
                return( const_cast <CSpellDef *>( m_SpellDefs ));
    
        case RES_SERVER:
    #if 0
                if ( ! m_Servers.IsValidIndex(index))
                        return( NULL );
                return( dynamic_cast <CServerDef *>( m_Servers ));
    #endif
    ...
    bool CResource::Load( bool fResync )
    {
        ...
        try
        {
                g_VerData.Load( g_Install.m_File );
        }
        catch ( CGrayError &e )
        {
        ...
        if ( g_Serv.GetName() == '\0' ) // make sure we have a set name
        {
                TCHAR szName;
                int iRet = gethostname( szName, sizeof( szName ));
                g_Serv.SetName(( ! iRet && szName ) ? szName : GRAY_TITLE );
        }

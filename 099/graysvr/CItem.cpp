    ...
    LPCTSTR const CItem::sm_szTrigName =        // static
    {
        _TEXT("@AAAUNUSED"),
        "@Click",
        "@Create",
    ...
    CItem * CItem::CreateScript( ITEMID_TYPE id ) // static
    {
        ...
        if ( pItemDef->HasTrigger( ITRIG_Create ))
        {
                ...
                {
                        if ( pItem->OnTriggerFind( s, sm_szTrigName ))
                        {
    ...
    CItem * CItem::CreateHeader( TCHAR * pArg, CObjBase * pCont, bool fDupeCheck )
    {
        // Just read info on a single item carryed by a CChar.
        // ITEM=#id,#amount,R#chance
    
        RESOURCE_ID rid = g_Cfg.ResourceGetID( RES_ITEMDEF, pArg );
        if ( ! rid.IsValidUID())
                return( NULL );
    ...
    LPCTSTR const CItem::sm_szTemplateTable =
    {
        "BUY",
        "CONTAINER",
        "ITEM",
        ...
        while ( s.ReadKeyParse())
        {
                ...
                case ITC_CONTAINER:
                        fItemAttrib = false;
                        {
                                ...
                                if ( pCont == NULL )
                                {
                                        DEBUG_ERR(( "CreateTemplate CContainer %s is not a container" DEBUG_CR, pItem->GetResour
ceName() ));
                                }
                                else
                                {
        ...
        if ( g_World.m_iLoadVersion < 53 )
        {
                ...
                if ( IsType(IT_POTION))
                {
                        ...
                        if ( m_itPotion.m_Type > COUNTOF(sm_OldPotions))
                                ...
                                m_itPotion.m_Type = sm_OldPotions;
                }
    ...
    usetitle:
                ...
                if ( IsAttr(ATTR_MAGIC))
                {
                        if ( m_itWeapon.m_spelllevel && IsTypeArmorWeapon() && ! IsType(IT_WAND))
                        {
                                ...
                                len += sprintf( pTemp+len, "%c%d ", ( m_itWeapon.m_spelllevel<0 ) ? '-':'+', abs( g_Cfg.GetSpell
Effect( SPELL_Enchant, m_itWeapon.m_spelllevel )));
                        }
                        else
                        {
    ...
    bool CItem::SetBaseID( ITEMID_TYPE id )
    {
        ...
        if ( pItemDef == NULL )
        {
                DEBUG_ERR(( "SetBaseID 0%x invalid item uid=0%x" DEBUG_CR, id, GetUID()));
                return false;
        }
    ...
    void CItem::SetAmount( int amount )
    {
        ...
        if ( IsType(IT_ORE))
        {
                ...
                SetDispID( ( GetAmount() >= COUNTOF(sm_Item_Ore)) ? ITEMID_ORE_4 : sm_Item_Ore );
        }
        ...
        CItemContainer* pParentCont = dynamic_cast <CItemContainer*> (GetParent() );
        if (pParentCont)
        {
                ASSERT( IsItemEquipped() || IsItemInContainer());
                CItemBase * pItemDef = Item_GetDef();
    ...
    LPCTSTR const CItem::sm_szLoadKeys =
    {
        "ADDSPELL",
        "AMOUNT",
        "ATTR",
    ...
    bool CItem::r_WriteVal( LPCTSTR pszKey, CGString & sVal, CTextConsole * pSrc )
    {
        ...
        case IC_HITPOINTS:
                ...
                break;
        case IC_HITSMAX:
                sVal.FormatVal( m_itArmor.m_Hits_Max );
                break;
        case IC_ID:
                goto dodefault;
        case IC_LAYER:
    ...
    bool CItem::r_LoadVal( CScript & s ) // Load an item Script
    {
        ...
        case IC_HITPOINTS:
                ...
                m_itArmor.m_Hits_Cur = s.GetArgRange();
                ...
                return true;
        case IC_HITSMAX:
                m_itArmor.m_Hits_Max = s.GetArgRange();
                return true;
        case IC_ID:
    ...
    bool CItem::r_Load( CScript & s ) // Load an item from script
    {
        ...
        if ( iResultCode )
        {
                DEBUG_ERR(( "Item 0%x Invalid, id=%s, code=0%x" DEBUG_CR, GetUID(), GetResourceName(), iResultCode ));
                Delete();
                return( true );
        }
    ...
    LPCTSTR const CItem::sm_szVerbKeys =
    {
        "BOUNCE",
        "CONSUME",
        "DECAY",
    ...
    bool CItem::r_Verb( CScript & s, CTextConsole * pSrc ) // Execute command from script
    {
        ...
        case CIV_USE:
                if ( ! pCharSrc )
                        return( false );
                pCharSrc->Use_Obj( this, s.HasArgs() ? s.GetArgVal() : true );
                break;
        default:
                DEBUG_CHECK(0);
                return( false );
    ...
    TRIGRET_TYPE CItem::OnTrigger( LPCTSTR pszTrigName, CTextConsole * pSrc, CScriptTriggerArgs * pArgs )
    {
        ...
        if ( pChar )
        {
                ...
                if ( pChar->m_pPlayer )
                {
                        DWORD dwPlot = pChar->m_pPlayer->m_dwPlot1;
                        for ( int i=0; dwPlot; i++, dwPlot >>= 1 )
                        {
                                ...
                                pResourceLink = dynamic_cast <CResourceLink *>( g_Cfg.ResourceGetDef( RESOURCE_ID( RES_PLOTITEM,
 i )));
                                if ( pResourceLink == NULL )
                                {
                                        DEBUG_ERR(( "0%x '%s' has unhandled " DEBUG_CR, pChar->GetUID(), pChar->GetName(), i ));
                                        pChar->m_pPlayer->m_dwPlot1 &= ~dwPlot;
                                        continue;
                                }
                ...
                // Is there an  block call here?
    
                if ( iAction > XTRIG_UNKNOWN )
                {
                        CGrayUID uidOldAct = pChar->m_Act_Targ;
                        pChar->m_Act_Targ = GetUID();
                        iRet = pChar->OnTrigger( (CTRIG_TYPE)( CTRIG_itemClick+(iAction-ITRIG_Click)),  pSrc, pArgs );
        ...
        if ( m_type >= IT_TRIGGER )
        {
                ...
                if ( pResourceLink == NULL )
                {
                        if ( pChar )
                        {
                                DEBUG_ERR(( "0%x '%s' has unhandled  for 0%x '%s'" DEBUG_CR, GetUID(), GetName(), GetType(), pCh
ar->GetUID(), pChar->GetName()));
                        }
                        else
                        {
                                DEBUG_ERR(( "0%x '%s' has unhandled " DEBUG_CR, GetUID(), GetName(), GetType() ));
                        }
    ...
    #define BASE_SIGHT 26 // 32 (UO_MAP_VIEW_RADAR) is the edge of the radar circle (for the most part)
        ...
        if ( pMeter->IsTerrainWater())
        {
                ...
                if ( ptLand.IsValidPoint())
                        sSearch.Format( "You see land to the %s. ", (LPCTSTR) CPointBase::sm_szDirs );
                else if (rLight > 3)
        ...
        while (true)
        {
                ...
                else
                {
                        ...
                        if (iDist < ptCoords.GetDist(pItemSighted->GetTopPoint())) // Only find the closest item to us, give boa
ts a preference
                        {
                                pItemSighted = pItem;
                        }
        ...
        if (iBoatSighted) // Report boat sightings
        {
                ...
                if (iBoatSighted == 1)
                        sSearch.Format("You can see a %s to the %s. ", (LPCTSTR) pBoatSighted->GetName(), CPointBase::sm_szDirs 
);
                else
                        sSearch.Format("You see many ships, one is to the %s. ", (LPCTSTR) CPointBase::sm_szDirs );
        ...
        if (iItemSighted) // Report item sightings, also boats beyond the boat visibility range in the radar screen
        {
                ...
                if (iItemSighted == 1)
                {
                        if ( iDist > UO_MAP_VIEW_RADAR) // if beyond ship visibility in the radar window, don't be specific
                                sSearch.Format("You can see something floating in the water to the %s. ", (LPCTSTR) CPointBase::
sm_szDirs );
                        else
                                sSearch.Format("You can see %s to the %s.", (LPCTSTR) pItemSighted->GetNameFull(false), (LPCTSTR
) CPointBase::sm_szDirs );
                }
                else
                {
                        if ( iDist > UO_MAP_VIEW_RADAR) // if beyond ship visibility in the radar window, don't be specific
                                sSearch.Format("You can see many things, one is to the %s. ", CPointBase::sm_szDirs );
                        else
                                sSearch.Format("You can see many things, one is %s to the %s. ", (LPCTSTR) pItemSighted->GetName
Full(false), (LPCTSTR) CPointBase::sm_szDirs );
                }
                strcat( pResult, sSearch);
        }
        ...
        if (iCharSighted > 0) // Report creature sightings, don't be too specific (that's what tracking is for)
        {
                ...
                if (iCharSighted == 1)
                        sSearch.Format("You see a creature to the %s", CPointBase::sm_szDirs );
                else
                        sSearch.Format("You can see many creatures, one is to the %s.", CPointBase::sm_szDirs );
                strcat( pResult, sSearch);
        }
        return pResult;
    }
    ...
    bool CItem::OnSpellEffect( SPELL_TYPE spell, CChar * pCharSrc, int iSkillLevel, CItem * pSourceItem )
    {
        ...
        if ( OnTrigger( ITRIG_SPELLEFFECT, pCharSrc, &Args ) == TRIGRET_RET_TRUE )
                return false;
        ...
        case SPELL_Mass_Dispel:
                ...
                case IT_SPELL:
                        ...
                        if ( IsTopLevel())
                        {
                                CItem * pItem = CItem::CreateScript( ITEMID_FX_SPELL_FAIL );
    ...
    int CItem::OnTakeDamage( int iDmg, CChar * pSrc, DAMAGE_TYPE uType )
    {
        ...
        // Break armor etc..
        if ( IsTypeArmorWeapon() && ( uType & ( DAMAGE_HIT_BLUNT | DAMAGE_HIT_PIERCE | DAMAGE_HIT_SLASH | DAMAGE_GOD|DAMAGE_MAGI
C|DAMAGE_FIRE )))
        {
    ...
    forcedamage:
                ...
                if ( pChar && pChar != pSrc )
                {
                        ...
                        if ( m_itArmor.m_Hits_Cur < m_itArmor.m_Hits_Max / 2 )
                        {
                                ...
                                {
                                        sMsg.Format( "Your %s is damaged and looks %s", (LPCTSTR) GetName(), (LPCTSTR) Armor_Get
RepairDesc());
                                }
                        }
                        if ( sMsg.IsEmpty())
    ...
    bool CItem::OnTick()
    {
        ...
        case IT_POTION:
                ...
                if ( RES_GET_INDEX(m_itPotion.m_Type) == SPELL_Explosion )
                {
                        ...
                        {
                                // Set it off.
                                OnGetHit( 1, m_uidLink.CharFind(), DAMAGE_GENERAL | DAMAGE_FIRE );
                        }
                        else
                        {
                                m_itPotion.m_tick --;

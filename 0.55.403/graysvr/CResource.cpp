...
enum RC_TYPE
{
	...
	RC_DEBUGFLAGS,
	RC_DECAYTIMER,
	RC_DISGUISENAME,		// m_sDisguiseName
...
const CAssocReg CResource::sm_szLoadKeys[RC_QTY+1] =
{
	...
	{ "DISGUISENAME",			{ ELEM_CSTRING,	offsetof(CResource,m_sDisguiseName)	}},
...
bool CResource::r_LoadVal( CScript &s )
{
	...
	case RC_DECAYTIMER:
		...
		break;
	case RC_DISGUISENAME:
		m_sDisguiseName = s.GetArgStr();
		if ( m_sDisguiseName.IsEmpty() ) m_sDisguiseName = EVO_TITLE;
		break;
	...
	case RES_DEFNAME:
		...
		while ( pScript->ReadKeyParse())
		{
			...
			if ( fNewStyleDef )
			{
				...
				g_Exp.m_DefMessages.SetStr(pszKey, true, pScript->GetArgStr(), true);
				continue;
			}
...
//#endif
	...
	if ( g_Serv.GetName()[0] == '\0' )	// make sure we have a set name
	{
		...
		g_Serv.SetName(( ! iRet && szName[0] ) ? szName : (LPCTSTR)g_Cfg.m_sDisguiseName.GetPtr() );
	}
	...
	{
		CStartLoc * pStart = new CStartLoc((LPCTSTR)g_Cfg.m_sDisguiseName.GetPtr());
		ASSERT(pStart);
...
LPCTSTR CResource::GetDefaultMsg( LPCTSTR pszKey )
{
	...
	g_Log.Event(LOGM_INIT|LOGL_ERROR, "Using of non-fast default message '%s'" DEBUG_CR, (LPCTSTR)pszKey);
	//	assume this message being in old format
	CVarDefBase	*pVar = g_Exp.m_DefMessages.GetKey(pszKey);
	if ( !pVar )
	{

...
bool CSector::r_Verb( CScript & s, CTextConsole * pSrc )
{
	...
	case SEV_ALLCLIENTS:	// "ALLCLIENTS"
		{
			// Send a verb to all clients in the sector
...
void CSector::r_Write()
{
	...
	if ( IsLightOverriden())
	{
		g_World.m_FileWorld.WriteKeyVal( "LOCALLIGHT", GetLight());
	}
	if ( IsRainOverriden())
	{
...
void CSector::RespawnDeadNPCs()
{
	...
	for ( ; pChar != NULL; pChar = pCharNext )
	{
		...
		pChar->Spell_Resurrection(-1,NULL);
...
void CSector::OnTick( int iPulseCount )
{
	...
	// long lPulseCount = CServTime::GetCurrentTime().GetTimeRaw() / SECTOR_TICK_PERIOD;
	if ( ! HasClients())
	{
...
//#endif
	...
	for ( ; pChar != NULL; pChar = pCharNext )
	{
		...
		try
		{
			...
			{
				strcpy(inLocalBlock, "EnvirOnChange");
				pChar->OnTrigger( CTRIG_EnvironChange, pChar );
			}
		...
		{
			g_Log.CatchEvent( &e, "Sector OnTickChar (%s) (UID=0%lx)", inLocalBlock, (DWORD) pChar->GetUID());
		}
		catch (...)	// catch all
		{
			g_Log.CatchEvent( NULL, "Sector OnTickChar (%s) (UID=0%lx)", inLocalBlock, (DWORD) pChar->GetUID());
		}
#endif
	}

	...
	case NPCACT_TALK_FOLLOW:
		...
		if ( m_Act_Targ != pSrc->GetUID())
		{
			if ( NPC_Act_Talk())
			{
				...
				CGString sMsg;
				sMsg.Format( g_Cfg.GetDefaultMsg( DEFMSG_NPC_GENERIC_INTERRUPT ),
...
bool CChar::NPC_OnTrainHear( CChar * pCharSrc, LPCTSTR pszCmd )
{
	...
	for ( ; i<MAX_SKILL; i++ )
	{
		...
		CGString sMsg;
		sMsg.Format( g_Cfg.GetDefaultMsg( DEFMSG_NPC_TRAINER_PRICE ),
...
bool CChar::NPC_Act_Begging( CChar * pChar )
{
	...
	{
		g_Cfg.GetDefaultMsg( DEFMSG_NPC_BEGGAR_BEG_1 ),
		g_Cfg.GetDefaultMsg( DEFMSG_NPC_BEGGAR_BEG_2 ),
		g_Cfg.GetDefaultMsg( DEFMSG_NPC_BEGGAR_BEG_3 ),
		g_Cfg.GetDefaultMsg( DEFMSG_NPC_BEGGAR_BEG_4 ),
		g_Cfg.GetDefaultMsg( DEFMSG_NPC_BEGGAR_BEG_5 ),
		g_Cfg.GetDefaultMsg( DEFMSG_NPC_BEGGAR_BEG_6 ),
	};
...
int CChar::NPC_WalkToPoint( bool fRun )
{
	...
	if ( g_Cfg.IsSetEF(EF_Pathfinding) )
	{
		CPathFinder path(this, pTarg);
		CPointMap	local = GetTopPoint();

		if ( path.FindPath() == PATH_NONEXISTENT ) return(2);
		path.ReadStep();

		local.m_x = path.m_xPath[1];
		local.m_y = path.m_yPath[1];
		local.m_x += path.m_realXoffset;
		local.m_y += path.m_realYoffset;
		Dir = pMe.GetDir(local);
		pMe.Move(Dir);
		if ( !CanMoveWalkTo(pMe, true, false, Dir ) ) return (2);
	}
	else
	{
		pMe.Move( Dir );
		if ( ! CanMoveWalkTo(pMe, true, false, Dir ) )
		{
			...
			if ( ! CanMoveWalkTo(pMe, true, false, Dir ))
			{
				...
				// try to move it.
				if ( Stat_GetAdjusted(STAT_INT) > iRand )
				{
					//	Scan point for items that could be moved by me
					//	and move them to my position
					CWorldSearch	AreaItems(pMe);
					while ( true )
					{
						...
						if ( pItem->IsAttr(ATTR_MOVE_NEVER|ATTR_OWNED|ATTR_INVIS|ATTR_STATIC) )
							bClearedWay = false;
						else if ( !CanCarry(pItem) ) bClearedWay = false;
						else if ( !(pItem->Item_GetDef()->m_Can & CAN_I_BLOCK) ) ;	// item does not block
						else
						{
							...
							if ( !Calc_GetRandVal(4) )	// 25% chance for item review
							{
								if (( st == NPCACT_WANDER ) || ( st == NPCACT_LOOKING ) || ( st == NPCACT_GO_HOME ) || ( st == NPCACT_GO_OBJ ))
								{
									...
									if ( pVend )	// the item is valuable
									{
										...
										if ( Calc_GetRandVal(price) < Calc_GetRandVal(STAT_INT) )
										{
					...
					if ( !bClearedWay && ( Stat_GetAdjusted(STAT_INT) > iRand*3 ))
					{
						CWorldSearch	AreaItems(ptFirstTry);
						while ( true )
						{
							CItem	*pItem = AreaItems.GetItem();
							if ( !pItem ) break;
						...
						//	Since it was the second try, we should restore DIRection
						//	we were faced to while trying the second place to walk
						if ( bClearedWay ) Dir = pMe.GetDir(m_Act_p);
					}
				}
			...
			if ( ! pSpellDef->IsSpellType( SPELLFLAG_HARM ))
			{
				if ( pSpellDef->IsSpellType(SPELLFLAG_TARG_CHAR) && pSpellDef->IsSpellType(SPELLFLAG_GOOD) )
				{
					...
					while ( true )
					{
						...
						pMemory = pTarget->Memory_FindObj(pChar);
						if ( pMemory )
						{
...
bool CChar::NPC_OnItemGive( CChar * pCharSrc, CItem * pItem )
{
	...
	if ( pItem->IsType(IT_GOLD))
	{
		if ( m_pNPC->m_Brain == NPCBRAIN_BANKER )
		{
			...
			CGString sMsg;
			sMsg.Format( g_Cfg.GetDefaultMsg( DEFMSG_NPC_BANKER_DEPOSIT ), pItem->GetAmount());
			Speak( sMsg );
...
bool CChar::NPC_OnTickFood( int nFoodLevel )
{
	...
	switch ( skill )
	{
  	...
  	case NPCACT_STAY:
  		if ( nFoodLevel > 25 ) return false;	// Only if very hungry
  		break;
  	case NPCACT_FLEE:
  	case NPCACT_BREATH:
  	case NPCACT_LOOTING:
  		if (nFoodLevel > 12) return false;		// Only if starving...some things are more important
	case NPCACT_WANDER:
  	case NPCACT_TALK:
  	case NPCACT_TALK_FOLLOW:
  	case NPCACT_GO_HOME:
...
void CChar::NPC_OnTickAction()
{
	...
	try
	{
		...
		{
			strcpy(inLocalBlock, "Fighting (skill)");
			NPC_Act_Fight();
		}
		...
		case SKILL_NONE:
			...
			strcpy(inLocalBlock, "Idle");
			NPC_Act_Idle();
			break;

		case SKILL_BEGGING:
			strcpy(inLocalBlock, "Begging");
		...
		case NPCACT_GUARD_TARG:
			...
			strcpy(inLocalBlock, "Following Target");
		...
		case NPCACT_STAY:
			...
			break;
		case NPCACT_GOTO:
			strcpy(inLocalBlock, "Going To");
			NPC_Act_Goto();
			break;
		case NPCACT_WANDER:
			strcpy(inLocalBlock, "Wandering");
			NPC_Act_Wander();
			break;
		case NPCACT_FLEE:
			strcpy(inLocalBlock, "Flying Away");
			NPC_Act_Flee();
			break;
		case NPCACT_TALK:
		case NPCACT_TALK_FOLLOW:
			// Got bored just talking to you.
			strcpy(inLocalBlock, "Talking");
			if ( ! NPC_Act_Talk())
			{
				strcpy(inLocalBlock, "Idle After Talking");
				NPC_Act_Idle();	// look for something new to do.
			}
			break;
		case NPCACT_GO_HOME:
			strcpy(inLocalBlock, "Going Home");
			NPC_Act_GoHome();
			break;
		case NPCACT_EATING:
			strcpy(inLocalBlock, "Eating");
			NPC_Act_Eating();
			break;
		case NPCACT_LOOTING:
			strcpy(inLocalBlock, "Looting");
			NPC_Act_Looting();
			break;
		case NPCACT_LOOKING:
			strcpy(inLocalBlock, "Looking Around");
			if ( NPC_LookAround( true ) )
				break;
			strcpy(inLocalBlock, "Idle Looking");
			NPC_Act_Idle();
			break;
		...
		strcpy(inLocalBlock, "Set New Timer");
	...
	{
		g_Log.CatchEvent( &e, "NPC Char OnTick (%s) (UID=0%lx)", inLocalBlock, (DWORD) GetUID());
	}
	catch (...)	// catch all
	{
		g_Log.CatchEvent( NULL, "NPC Char OnTick (%s) (UID=0%lx)", inLocalBlock, (DWORD) GetUID());
	}
#endif
}

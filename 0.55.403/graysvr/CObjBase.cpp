...
void CObjBase::Emote( LPCTSTR pText, CClient * pClientExclude, bool fForcePossessive )
{
	...
	if ( pObjTop->IsChar())
	{
		...
		if ( pObjTop != this )
		{
			sMsgThem.Format( g_Cfg.GetDefaultMsg( DEFMSG_EMOTE_1 ), (LPCTSTR) pObjTop->GetName(), (LPCTSTR) GetName(), (LPCTSTR) pText );
			sMsgYou.Format( g_Cfg.GetDefaultMsg( DEFMSG_EMOTE_2 ), (LPCTSTR) GetName(), (LPCTSTR) pText );
		}
		else if ( fForcePossessive )
		{
			// ex. "You see joes poor shot ruin an arrow"
			sMsgThem.Format( g_Cfg.GetDefaultMsg( DEFMSG_EMOTE_3 ), (LPCTSTR) GetName(), (LPCTSTR) pText );
			sMsgYou.Format( g_Cfg.GetDefaultMsg( DEFMSG_EMOTE_4 ), pText );
		}
		else
		{
			sMsgThem.Format( g_Cfg.GetDefaultMsg( DEFMSG_EMOTE_5 ), (LPCTSTR) GetName(), (LPCTSTR) pText );
			sMsgYou.Format( g_Cfg.GetDefaultMsg( DEFMSG_EMOTE_6 ), pText );
		}
	}
	else
	{
		// Top level is an item. Article ?
		sMsgThem.Format( g_Cfg.GetDefaultMsg( DEFMSG_EMOTE_7 ), (LPCTSTR) GetName(), (LPCTSTR) pText );
		sMsgYou = sMsgThem;
	}
...
bool CObjBase::r_Verb( CScript & s, CTextConsole * pSrc ) // Execute command from script
{
	...
	case OV_NEWITEM:	// just add an item but don't put it anyplace yet..
		{
			CItem * pItem = CItem::CreateHeader(s.GetArgRaw(), NULL, false, pSrc->GetChar() );
			if ( pItem == NULL )
			{
	...
	case OV_SDIALOG:
		{
			...
			pClientSrc->Dialog_Setup( CLIMODE_DIALOG,

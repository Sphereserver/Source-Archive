...
bool CChar::Spell_Resurrection( int iSkillLossPercent, CItemCorpse * pCorpse )
{
	...
	SetHue( m_prev_Hue );
	Stat_SetVal(STAT_STR, IMULDIV( Stat_GetMax(STAT_STR), g_Cfg.m_iHitpointPercentOnRez, 100 ) );
	...
	/*if ( iSkillLossPercent > 0 )
	{
		...
		{
			int iVal = Stat_COMMENT_GetBase( (STAT_TYPE) i );
			if ( iVal <= 25 )
				continue;
			Stat_COMMENT_SetBase( (STAT_TYPE) i, iVal - IMULDIV( iVal, Calc_GetRandVal( iSkillLossPercent ), 100 ));
		}
	}*/ //used for death option stuff, no longer needed
...
bool CChar::Spell_Equip_OnTick( CItem * pItem )
{
	...
	case SPELL_Poison:
		...
		{
		CGString sMsg;
		sMsg.Format( g_Cfg.GetDefaultMsg( DEFMSG_SPELL_LOOKS ), (LPCTSTR) sm_Poison_Message[iLevel] );
...
CItem * CChar::Spell_Effect_Create( SPELL_TYPE spell, LAYER_TYPE layer, int iSkillLevel, int iDuration, CObjBase * pSrc )
{
	...
	if (	( g_Cfg.IsSetOF( OF_Magic_StackStats )	&&	layer == LAYER_SPELL_STATS )
		||	layer == LAYER_NONE )
...
bool CChar::Spell_TargCheck()
{
		...
		&& !( !pObj && pSpellDef->IsSpellType( SPELLFLAG_TARG_XYZ ) ) )
	{
		...
		if ( pSpellDef->IsSpellType( SPELLFLAG_TARG_CHAR ))
		{
			...
			if ( pChar == NULL )
			{
				SysMessageDefault( DEFMSG_SPELL_TARG_NOEFFECT );
				return( false );
			}
		}
#endif
...
bool CChar::OnSpellEffect( SPELL_TYPE spell, CChar * pCharSrc, int iSkillLevel, CItem * pSourceItem )
{
	...
	if ( pSpellDef->IsSpellType( SPELLFLAG_HARM ))
	{
		...
		if (	!g_Cfg.IsSetOF( OF_Magic_CanHarmSelf )
			&&	pCharSrc == this )
			return( false );

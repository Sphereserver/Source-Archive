...
bool CClient::Setup_Start( CChar * pChar ) // Send character startup stuff to player
{
	...
	else if ( !fNoMessages )
	{
		...
		sMsg.Format("Last logged: %s", GetAccount()->m_TagDefs.GetKeyStr("LastLogged"));
		addSysMessage(sMsg);
	}
	...
	{
		CGString sMsg;
		sMsg.Format( g_Cfg.GetDefaultMsg( DEFMSG_GMPAGES ), g_World.m_GMPages.GetCount());
		addSysMessage( sMsg );
	}
	if ( g_Cfg.m_fRequireEmail )
	{
		...
		{
			sMsg.Format( g_Cfg.GetDefaultMsg( DEFMSG_EMAIL_NOTSET_1 ),
		...
		else if ( ! IsPriv( PRIV_EMAIL_VALID ))
		{
			...
			{
				sMsg.Format( g_Cfg.GetDefaultMsg( DEFMSG_EMAIL_FAILED  ),
					(LPCTSTR)GetAccount()->GetName(), (LPCTSTR)( GetAccount()->m_sEMail ), GetAccount()->m_iEmailFailures );
			}
			else
			{
				sMsg.Format( g_Cfg.GetDefaultMsg( DEFMSG_EMAIL_NOTVERIFIED  ),
					(LPCTSTR)GetAccount()->GetName(), (LPCTSTR)( GetAccount()->m_sEMail ));
			}
			addSysMessage( sMsg );
			addSysMessage( g_Cfg.GetDefaultMsg( DEFMSG_EMAIL_NOTSET_2 ) );
...
DELETE_ERR_TYPE CClient::Setup_Delete( int iSlot ) // Deletion of character
{
	...
	r_Call("f_onchar_delete", pChar, NULL, NULL, &tr);
...
LOGIN_ERR_TYPE CClient::LogIn( CAccountRef pAccount, CGString & sMsg )
{
	...
	enum TRIGRET_TYPE tr;
	r_Call("f_onaccount_login", NULL, &Args, NULL, &tr);
	if ( tr == TRIGRET_RET_TRUE )
	{

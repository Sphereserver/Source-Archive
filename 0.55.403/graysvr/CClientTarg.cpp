...
int CClient::OnSkill_AnimalLore( CGrayUID uid, int iSkillLevel, bool fTest )
{
	...
	{
		sTmp.Format( g_Cfg.GetDefaultMsg( DEFMSG_ANIMALLORE_RESULT ), (LPCTSTR) pChar->GetName(),(LPCTSTR) pChar->Char_GetDef()->GetTradeName());
	...
	if ( pCharOwner == NULL )
	{
		sTmp.Format( g_Cfg.GetDefaultMsg( DEFMSG_ANIMALLORE_FREE ), (LPCTSTR) pszHe, (LPCTSTR) pszHis );
	}
	else
	{
		sTmp.Format( g_Cfg.GetDefaultMsg( DEFMSG_ANIMALLORE_MASTER ), pszHe, ( pCharOwner == m_pChar ) ? g_Cfg.GetDefaultMsg( DEFMSG_ANIMALLORE_MASTER_YOU ) : (LPCTSTR) pCharOwner->GetName());
		// How loyal to master ?
	}
	addObjMessage( sTmp, pChar );
	...
	sTmp.Format( g_Cfg.GetDefaultMsg( DEFMSG_ANIMALLORE_FOOD ), (LPCTSTR) pszHe, (LPCTSTR) pszText );
	addObjMessage( sTmp, pChar );
...
int CClient::OnSkill_ItemID( CGrayUID uid, int iSkillLevel, bool fTest )
{
	...
	{
		len += strcpylen( szTemp+len, g_Cfg.GetDefaultMsg( DEFMSG_ITEM_MAGIC ) );
	}
	else if ( pItem->IsAttr(ATTR_NEWBIE|ATTR_MOVE_NEVER))
	{
		len += strcpylen( szTemp+len, g_Cfg.GetDefaultMsg( DEFMSG_ITEM_NEWBIE ) );
	}
#endif
	...
	if ( pItemVend == NULL )
	{
		SysMessagef( _TEXT( g_Cfg.GetDefaultMsg( DEFMSG_ITEMID_NOVAL ) ));
	}
	else
	{
		SysMessagef( _TEXT( g_Cfg.GetDefaultMsg( DEFMSG_ITEMID_GOLD ) ),
...
int CClient::OnSkill_EvalInt( CGrayUID uid, int iSkillLevel, bool fTest )
{
	...
	if ( iSkillLevel > 400 )	// magery skill and mana level ?
	{
		...
		SysMessagef( g_Cfg.GetDefaultMsg( DEFMSG_EVALINT_RESULT_2 ),
...
int CClient::OnSkill_Anatomy( CGrayUID uid, int iSkillLevel, bool fTest )
{
	...
	CGString sTmp;
	sTmp.Format( g_Cfg.GetDefaultMsg( DEFMSG_ANATOMY_RESULT ), (LPCTSTR) pChar->GetName(),
...
bool CClient::OnTarg_Pet_Stable( CChar * pCharPet )
{
	...
	CGString sMsg;
	sMsg.Format( g_Cfg.GetDefaultMsg( DEFMSG_NPC_STABLEMASTER_REM ),

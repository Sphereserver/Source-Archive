...
bool CAccounts::Account_SaveAll()
{
	...
	s.Printf( "\\\\ %s %s accounts file" DEBUG_CR
		...
		(LPCTSTR)g_Cfg.m_sDisguiseName.GetPtr(), g_Serv.GetName());
...
{
/*
	if ( pAccount->m_Chars.GetCharCount() )
	{
...
bool CAccounts::Cmd_ListUnused( CTextConsole * pSrc, LPCTSTR pszDays, LPCTSTR pszVerb, LPCTSTR pszArgs )
{
	...
	pSrc->SysMessagef( "%d of %d accounts unused for %d days", iCount,
		iCountOrig, iDaysTest );
	if ( pszVerb && ! strcmpi( pszVerb, "DELETE" ) )
	{
		...
		if ( iDeleted < iCount )
			...
			pSrc->SysMessagef( "All %d unused accounts deleted.", iDeleted );
		else
			pSrc->SysMessagef( "No accounts deleted.", iDeleted );
	}
...
bool CAccount::Kick( CTextConsole * pSrc, bool fBlock )
{
	...
	CGString sMsg;
	sMsg.Format( g_Cfg.GetDefaultMsg(DEFMSG_ACC_KICK), (LPCTSTR) GetName(), pszAction, (LPCTSTR) pSrc->GetName() );
	g_Log.Event( LOGL_EVENT|LOGM_GM_CMDS, "%s" DEBUG_CR, (LPCTSTR) sMsg );
...
bool CAccount::r_LoadVal( CScript & s )
{
	...
	case AC_CHATNAME:
		...
		break;
	case AC_COMMENT:
		m_TagDefs.SetStr( "COMMENT", true, s.GetArgStr());
		break;
	case AC_EMAIL:
	case AC_EMAILLINK:
...
bool CAccount::r_Verb( CScript &s, CTextConsole * pSrc )
{
	...
	case AV_DELETE: // "DELETE"
		...
		if ( g_Accounts.Account_Delete( this ) ) pSrc->SysMessageDefault(DEFMSG_ACC_DEL);
		else pSrc->SysMessageDefault(DEFMSG_ACC_CHAR_DEL);
		return( true );
	case AV_EMAIL:

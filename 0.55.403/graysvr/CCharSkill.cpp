...
bool CChar::Skill_MakeItem_Success()
{
	...
	else
	{
		...
		case 0:
			// Shoddy quality
			sMakeMsg.Format(g_Cfg.GetDefaultMsg( DEFMSG_MAKESUCCESS_1 ));
			quality = Calc_GetRandVal( 25 ) + 1;
			break;
		case 1:
			// Poor quality
			sMakeMsg.Format(g_Cfg.GetDefaultMsg( DEFMSG_MAKESUCCESS_2 ));
			quality = Calc_GetRandVal( 25 ) + 26;
			break;
		case 2:
			// Below average quality
			sMakeMsg.Format(g_Cfg.GetDefaultMsg( DEFMSG_MAKESUCCESS_3 ));
		...
		case 4:
			...
			sMakeMsg.Format(g_Cfg.GetDefaultMsg( DEFMSG_MAKESUCCESS_4 ));
			quality = Calc_GetRandVal( 25 ) + 126;
			break;
		case 5:
			// Excellent quality
			sMakeMsg.Format(g_Cfg.GetDefaultMsg( DEFMSG_MAKESUCCESS_5 ));
			quality = Calc_GetRandVal( 25 ) + 151;
			break;
		case 6:
			// Superior quality
			sMakeMsg.Format(g_Cfg.GetDefaultMsg( DEFMSG_MAKESUCCESS_6 ));
		...
		if (( iSkillLevel > 999 && ( quality > 175 )) && ( !g_Cfg.IsSetOF(OF_NoItemNaming)))
		{
			...
			CGString csNewName;
			csNewName.Format( g_Cfg.GetDefaultMsg( DEFMSG_GRANDMASTER_MARK ), (LPCTSTR) pItem->GetName(), (LPCTSTR) GetName());
...
bool CChar::Skill_Tracking( CGrayUID uidTarg, DIR_TYPE & dirPrv, int iDistMax )
{
	...
	if (( dirPrv != dir ) || ! Calc_GetRandVal(10))
	{
		...
		if ( dist )
		{
			...
			sMsgDist.Format( _TEXT( g_Cfg.GetDefaultMsg( DEFMSG_TRACKING_DIST_0 ) ), pszDist, (LPCTSTR) CPointBase::sm_szDirs[ dir ] );
		}
		else
		{
			sMsgDist = _TEXT(" here");
...
int CChar::Skill_Taming( SKTRIG_TYPE stage )
{
	...
	if ( pMemory && pMemory->m_itEqMemory.m_Action == NPC_MEM_ACT_TAMED)
	{
		...
		CGString sSpeak;
		sSpeak.Format( g_Cfg.GetDefaultMsg( DEFMSG_TAMING_REMEMBER ), (LPCTSTR) pChar->GetName());
...
int CChar::Skill_Healing( SKTRIG_TYPE stage )
{
	...
	if ( stage == SKTRIG_START )
	{
		...
		{
			CGString sMsg;
			sMsg.Format( g_Cfg.GetDefaultMsg( DEFMSG_HEALING_TO ), pChar->GetName());
			Emote( sMsg );
		}
		else
		{
	...
	{
		CGString sText;
		sText.Format( g_Cfg.GetDefaultMsg( DEFMSG_HEALING_RES ), pChar->GetName());

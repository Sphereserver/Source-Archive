...
void CChar::EatAnim( LPCTSTR pszName, int iQty )
{
	...
	sEmoteMessage.Format( g_Cfg.GetDefaultMsg( DEFMSG_EATSOME ), (LPCTSTR) pszName );
	Emote(sEmoteMessage);
}
...
CItemCorpse * CChar::MakeCorpse( bool fFrontFall )
{
		...
		GetDispID() != CREID_BLADES )
	{
		...
		CGString sName;
		sName.Format( g_Cfg.GetDefaultMsg( DEFMSG_CORPSE_OF ), (LPCTSTR) GetName());
...
bool CChar::Death()
{
	...
	int iKillStrLen = sprintf( pszKillStr, g_Cfg.GetDefaultMsg( DEFMSG_KILLED_BY ),
...
CRegionBase * CChar::CanMoveWalkTo( CPointBase & ptDst, bool fCheckChars, bool fCheckOnly, DIR_TYPE dir )
{
	...
	if ( ! IsStatFlag( STATF_DEAD | STATF_Sleeping | STATF_Insubstantial ) && fCheckChars )
	{
		...
		while (true)
		{
			...
			{
				sMsg = g_Cfg.GetDefaultMsg( DEFMSG_INVISIBLE );
				continue;
			}
			else if ( pChar->IsStatFlag( STATF_Hidden ))
			{
				// reveal hidden people ?
				sMsg.Format( g_Cfg.GetDefaultMsg( DEFMSG_HIDING_STUMBLE ), (LPCTSTR) pChar->GetName());
				pChar->Reveal(STATF_Hidden);
			}
			else if ( pChar->IsStatFlag( STATF_Sleeping ))
			{
				sMsg.Format( g_Cfg.GetDefaultMsg( DEFMSG_STEPON_BODY ), (LPCTSTR) pChar->GetName());
				// ??? damage.
			}
			else if ( iStamReq < 0 || Stat_GetVal(STAT_DEX) < iStamReq + 1 )
			{
				sMsg.Format( g_Cfg.GetDefaultMsg( DEFMSG_CANTPUSH ), (LPCTSTR) pChar->GetName());
				if ( ! IsPriv( PRIV_GM ))
				{
					SysMessage( sMsg );
					return NULL;
			...
			else
			{
				...
				sMsg.Format( g_Cfg.GetDefaultMsg( DEFMSG_PUSH ), (LPCTSTR) pChar->GetName());
			}
...
TRIGRET_TYPE CChar::OnTrigger( LPCTSTR pszTrigName, CTextConsole * pSrc, CScriptTriggerArgs * pArgs )
{
	...
	try
	{
		...
		strcpy(inLocalBlock, "Body Triggers");
		...
		{
			strcpy(inLocalBlock, "Controlled NPC Triggers");
			// RES_EVENTS
	...
	{
		g_Log.CatchEvent( &e, "Char Trigger in %s (%s) (UID=0%lx)", pszTrigName, inLocalBlock, (DWORD) GetUID());
	}
	catch (...)	// catch all
	{
		g_Log.CatchEvent( NULL, "Char Trigger in %s (%s) (UID=0%lx)", pszTrigName, inLocalBlock, (DWORD) GetUID());
	}
#endif
...
bool CChar::OnTick()
{
	...
	try
	{
		...
		if ( iTimeDiff >= TICK_PER_SEC )	// don't bother with < 1 sec times.
		{
			...
			strcpy(inLocalBlock, "Decay Items");
			CItem* pItemNext;
			CItem* pItem=GetContentHead();
			for ( ; pItem!=NULL; pItem=pItemNext )
			{
			...
			strcpy(inLocalBlock, "Stats Ticking");
		...
		{
			strcpy(inLocalBlock, "Timer Expired");
			// My turn to do some action.
			switch ( Skill_Done())
			{
			case -SKTRIG_ABORT: strcpy(inLocalBlock, "Skill Abort"); Skill_Fail( true ); break;	// fail with no message or credit.
			case -SKTRIG_FAIL: strcpy(inLocalBlock, "Skill Fail"); Skill_Fail( false ); break;
			case -SKTRIG_QTY: strcpy(inLocalBlock, "Skill Cleanup"); Skill_Cleanup(); break;
			}
			if ( ! m_pPlayer )
			{
				// What to do next ?
		...
		strcpy(inLocalBlock, "Update Stats");
		if ( IsClient() )
	...
	{
		g_Log.CatchEvent( &e, "Char OnTick (%s) (UID=0%lx)", inLocalBlock, (DWORD) GetUID());
	}
	catch (...)	// catch all
	{
		g_Log.CatchEvent( NULL, "Char OnTick (%s) (UID=0%lx)", inLocalBlock, (DWORD) GetUID());
	}
#endif

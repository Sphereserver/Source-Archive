...
void CPartyDef::SetLootFlag( CChar * pChar, bool fSet )
{
	...
	pChar->SysMessage( fSet ?
		g_Cfg.GetDefaultMsg( DEFMSG_PARTY_LOOT_ALLOW ) :
...
bool CPartyDef::AcceptEvent( CChar * pCharAccept, CGrayUID uidInviter )	// static
{
	...
	CGString sMsg;
	sMsg.Format( g_Cfg.GetDefaultMsg( DEFMSG_PARTY_JOINED ), (LPCTSTR) pCharAccept->GetName() );
...
bool CPartyDef::RemoveChar( CGrayUID uid, CGrayUID uidAct )
{
	...
	LPCTSTR pszForceMsg = ( (DWORD) uidAct != (DWORD) uid ) ? g_Cfg.GetDefaultMsg( DEFMSG_PARTY_PART_1 ) : g_Cfg.GetDefaultMsg( DEFMSG_PARTY_PART_2 );

	// Tell the kicked person they are out
	pChar->SysMessagef( g_Cfg.GetDefaultMsg( DEFMSG_PARTY_LEAVE_2 ), pszForceMsg );
	...
	else
	{
		...
		CGString sMsg;
		sMsg.Format( g_Cfg.GetDefaultMsg( DEFMSG_PARTY_LEAVE_1 ), (LPCTSTR) pChar->GetName(), (LPCTSTR) pszForceMsg );
		SysMessageAll(sMsg);
	}
	return true;
}

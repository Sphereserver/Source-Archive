...
CPathFinder::CPathFinder(CChar *pChar, CPointMap ptTarget)
{
	...
	pt.m_x -= m_realXoffset;
	pt.m_y -= m_realYoffset;
	m_pChar->SetTopPoint(pt);
...
int CPathFinder::FindPath(int pid)
{
	...
	int startX = m_pChar->GetTopPoint().m_x;
	int startY = m_pChar->GetTopPoint().m_y;
...
noPath:
	m_xPath[pid] = m_pChar->GetTopPoint().m_x;
	m_yPath[pid] = m_pChar->GetTopPoint().m_y;
...
void CPathFinder::FillMap()
{
	...
	ptChar.m_x += m_realXoffset;
	ptChar.m_y += m_realYoffset;
	m_pChar->SetTopPoint(ptChar);
	for ( x = 0; x < PATH_SIZE; x++ )
	{
		for ( y = 0; y < PATH_SIZE; y++ )
		{
			realX = x + m_realXoffset;
			realY = y + m_realYoffset;
			pt.m_x = realX;
			pt.m_y = realY;
			...
			pArea = m_pChar->CanMoveWalkTo(pt, false, true, ptChar.GetDir(pt));
			if ( !pArea ) m_walkability[x][y] = PATH_UNWALKABLE;
...
int CPathFinder::ReadPathX(int pathLocation, int pid)
{
	...
	{
		x = m_pathBank[pid][pathLocation*2-2];
	}
	return x;
}	
...
int CPathFinder::ReadPathY(int pathLocation, int pid)
{
	...
	if ( m_pathLocation[pid] <= m_pathLength[pid] )
	{
		y = m_pathBank[pid][pathLocation*2-1];
	}
	return y;
}	

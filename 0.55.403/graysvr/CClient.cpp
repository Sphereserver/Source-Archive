...
bool CClient::CanInstantLogOut() const
{
	...
	//	Use REGION_FLAG_INSTA_LOGOUT instead - Vjaka
	...
	if ( g_Cfg.m_fArriveDepartMsg )
	{
		...
		for ( CClient * pClient = g_Serv.GetClientHead(); pClient!=NULL; pClient = pClient->GetNext())
		{
			...
			if ( sMsg.IsEmpty())
			{
				...
				sMsg.Format( _TEXT( g_Cfg.GetDefaultMsg( DEFMSG_ARRDEP_1 ) ),
					(LPCTSTR) m_pChar->GetName(),
					(fArrive)? _TEXT(g_Cfg.GetDefaultMsg( DEFMSG_ARRDEP_2 )) : _TEXT(g_Cfg.GetDefaultMsg( DEFMSG_ARRDEP_3 )),

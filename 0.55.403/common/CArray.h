...
#if 0
// Not really working though i'm not sure why.
class CQSortArray
{
private:
	const void * m_pData;
	const int m_iQty;
	const int m_iSize;
	int (*m_fnCompare)( void * pLeft, void * pRight );
	BYTE * m_pTmp;
private:
	void * GetEntry(int i) const
	{
		ASSERT(i<m_iQty);
		return( ((BYTE*)m_pData)+(i*m_iSize));
	}
	void QSort( int left, int right );
public:
	CQSortArray( void * pData, int iQty, int iSize, int (*fnCompare)( void * pLeft, void * pRight )) :
		m_pData( pData ),
		m_iQty( iQty ),
		m_iSize( iSize ),
		m_fnCompare( fnCompare )
	{
		m_pTmp = new BYTE [iSize];
	}
	void QSort()
	{
		QSort( 0, m_iQty-1 );

...
bool CScriptTriggerArgs::r_Verb( CScript & s, CTextConsole * pSrc )
{
	...
	LPCTSTR		pszKey;
	pszKey	= s.GetKey();
	EXC_TRY(("r_Verb('%s %s',%x)", pszKey, s.GetArgStr(), pSrc));
...
bool CScriptObj::r_LoadVal( CScript & s )
{
	...
	if ( index == SSC_DEFMSG )
	{
		...
		g_Exp.m_DefMessages.SetStr(pszKey, true, s.GetArgStr(), true);
		return(false);
	}
#endif
...
jump_in:
		...
		switch ( iCmd )
		{
		case SK_FORITEM:	EXC_SET(EXC_FORITEM); iRet = OnTriggerForLoop( s, 1, pSrc, pArgs, pResult );			break;
		case SK_FORCHAR:	EXC_SET(EXC_FORCHAR); iRet = OnTriggerForLoop( s, 2, pSrc, pArgs, pResult );			break;
		case SK_FORCLIENTS:	EXC_SET(EXC_FORCLIENTS); iRet = OnTriggerForLoop( s, 0x12, pSrc, pArgs, pResult );		break;
		case SK_FOROBJ:		EXC_SET(EXC_FOROBJ); iRet = OnTriggerForLoop( s, 3, pSrc, pArgs, pResult );			break;
		case SK_FORPLAYERS:	EXC_SET(EXC_FORPLAYERS); iRet = OnTriggerForLoop( s, 0x22, pSrc, pArgs, pResult );		break;
		case SK_FOR:		EXC_SET(EXC_FOR); iRet = OnTriggerForLoop( s, 4, pSrc, pArgs, pResult );			break;
		case SK_WHILE:		EXC_SET(EXC_WHILE); iRet = OnTriggerForLoop( s, 8, pSrc, pArgs, pResult );			break;
		case SK_FORCHARLAYER:
		case SK_FORCHARMEMORYTYPE:
			{
				EXC_SET(EXC_FORCHARLAYER);

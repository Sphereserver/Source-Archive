...
void CClient::Event_Talk( LPCTSTR pszText, HUE_TYPE wHue, TALKMODE_TYPE mode ) // PC speech
{
	...
	if ( ! g_Cfg.IsConsoleCmd( pszText[0] ))
	{
		...
		{
			g_Log.Event( LOGM_PLAYER_SPEAK, "%x:'%s' Says '%s' mode=%d" DEBUG_CR, m_Socket.GetSocket(), m_pChar->GetName(), szText, mode );
		}
...
void CClient::Event_TalkUNICODE( const CEvent * pEvent )
{
	...
	if ( pEvent->TalkUNICODE.m_mode == TALKMODE_TOKENIZED )
	{
	...
	if ( ! g_Cfg.IsConsoleCmd( szText[0] ))
	{
		if ( g_Log.IsLoggedMask( LOGM_PLAYER_SPEAK ))
		{
			g_Log.Event( LOGM_PLAYER_SPEAK, "%x:'%s' Says UNICODE '%s' '%s' mode=%d" DEBUG_CR, m_Socket.GetSocket(), m_pChar->GetName(), pAccount->m_lang.GetStr(), szText, pEvent->Talk.m_mode );
		}
		m_pChar->SpeakUTF8( szText, 
			pEvent->Talk.m_wHue, 
			(TALKMODE_TYPE) pEvent->Talk.m_mode,
			m_pChar->m_fonttype, 
			pAccount->m_lang );
	}
...
void CClient::Event_GumpDialogRet( const CEvent * pEvent )
{
	...
	for ( i = 0; i < iTextQty; i++)
	{
		...
		TCHAR scratch[SCRIPT_MAX_LINE_LEN]; // use this as scratch storage

		// Do a loop and "convert" from unicode to normal ascii
		CvtNUNICODEToSystem( scratch, sizeof(scratch), pMsg->GumpDialogRet.m_texts[0].m_utext, lenstr );

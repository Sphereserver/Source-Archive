...
CStoneMember::CStoneMember( CItemStone * pStone, CGrayUID uid, STONEPRIV_TYPE iType, LPCTSTR pTitle, CGrayUID loyaluid, bool fVal1, bool fVal2 )
{
	m_uidLinkTo = uid;
	m_sTitle = pTitle;
	m_iPriv = iType;
...
onlyplayers:
	...
	if (pMember)
	{
		...
		{
			sStr.Format("%s is already %s %s.", (LPCTSTR) pChar->GetName(), (LPCTSTR) pMember->GetPrivName(), (LPCTSTR) GetName());
			Speak(sStr);
			return NULL;
		}

...
bool CChar::Spell_Equip_OnTick( CItem * pItem )
{
	...
	case SPELL_Poison:
		...
		SysMessagef( "You feel %s", (LPCTSTR) sm_Poison_Message[iLevel] );
...
bool CChar::Spell_CastDone( void )
{
	...
	case SPELL_Create_Food:
		...
		if ( pObj == NULL )
		{
			...
			CItem * pItem = CItem::CreateScript( sm_Item_Foods[ Calc_GetRandVal( COUNTOF( sm_Item_Foods )) ] );
	...
	case SPELL_Bone_Armor:
		{
			...
			for ( int i=0; i<COUNTOF(sm_Item_Bone); i++ )
			{
				if ( ! Calc_GetRandVal( 2+iGet ))
					break;
				CItem * pItem = CItem::CreateScript( (ITEMID_TYPE) sm_Item_Bone[i] );
				pItem->MoveToCheck( m_Act_p, this );
				iGet++;
			}
			if ( ! iGet )
...
reflectit:
	...
	case SPELL_Night_Sight:
		Spell_Effect_Create( SPELL_Night_Sight, fPotion ? LAYER_FLAG_Potion : LAYER_SPELL_Night_Sight, iSkillLevel, 0, pCharSrc );
		break;
	...
	case SPELL_Incognito:
		Spell_Effect_Create( SPELL_Incognito, fPotion ? LAYER_FLAG_Potion : LAYER_SPELL_Incognito, iSkillLevel, 0, pCharSrc );
		break;
...
#define SPELL_MAX_POLY_STAT 150
			...
			CItem * pSpell = Spell_Effect_Create( SPELL_Polymorph, fPotion ? LAYER_FLAG_Potion : LAYER_SPELL_Polymorph, iSkillLevel, 0, pCharSrc );
			SetID(creid);

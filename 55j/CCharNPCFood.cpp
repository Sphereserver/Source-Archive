...
{
	CItem * pResBit = g_World.CheckNaturalResource( GetTopPoint(), IT_WATER, true );
  	if ( pResBit && pResBit->GetAmount() > 0 )
  	{
  		...
  		{
  			// I caught something!
  			int iEaten = pResBit->ConsumeAmount( min( m_food, iBite ));
			EatAnim( "fish", iEaten );
  		}
  		return true;
  	}
...
bool CChar::NPC_Food_GrassCheck(int iAmount, int iBite, int iSearchDist )
{
    ...
    CItem * pResBit = g_World.CheckNaturalResource( GetTopPoint(), IT_GRASS, true );
    if ( pResBit && pResBit->GetAmount())
    {
    	// Yummy!  There is some here!  Eat it.
    	// First, find out how much I want.
		int iEaten = pResBit->ConsumeAmount( min( m_food * 10, iBite ));
...
bool CChar::NPC_Food_VendorCheck(int iAmount, int iBite)
{
	...
	if ( pItem != NULL )
	{
		...
		pItem->UnStackSplit( min( iAmount - m_food, pItem->GetAmount()), this );
		ItemBounce( pItem );
		return true;
	}
...
void CChar::NPC_Food_Search()
{
    ...
    TCHAR szFoodEntries[SCRIPT_MAX_LINE_LEN];

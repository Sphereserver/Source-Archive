...
bool CValueRangeDef::Load( TCHAR * pszDef )
{
	...
	int piVal[2];
	int iQty = g_Exp.GetRangeVals((LPCSTR&)pszDef, piVal, COUNTOF(piVal));
	if ( iQty< 0 ) 
		return(false);
	...
	case RGC_ID:	// "ID"
		{
			...
			CResourceQty rec;
			rec.SetResourceID( 
				g_Cfg.ResourceGetID( RES_CHARDEF, (LPCSTR&)ppCmd[0] ),
				( iArgs > 1 && ppCmd[1][0] ) ? Exp_GetVal((LPCSTR&)ppCmd[1] ) : 1 );
	...
	CResourceDef( id )
{
	...
	m_iRegenRate[STAT_STR] = 6*TICK_PER_SEC; // Seconds to heal ONE hp (base before stam and food adjust)
	m_iRegenRate[STAT_INT] = 5*TICK_PER_SEC; // Seconds to heal ONE mn
	m_iRegenRate[STAT_DEX] = 3*TICK_PER_SEC; // Seconds to heal ONE stm
	m_iRegenRate[STAT_FoodRegen] = 30*60*TICK_PER_SEC; // Food usage (1 time per 30 minutes)

...
void CNTWindow::OnSize( UINT nType, int cx, int cy )
{
	if ( nType != SIZE_MINIMIZED && nType != SIZE_MAXHIDE && m_wndLog.m_hWnd )
	{
		if ( ! m_iFontHeight )
		{
			...
			m_iFontHeight = abs( logfont.lfHeight );
			ASSERT(m_iFontHeight);
		}

		m_wndLog.MoveWindow( 0, 0, cx, cy-m_iFontHeight, TRUE );
		m_wndInput.MoveWindow( 0, cy-m_iFontHeight, cx, m_iFontHeight, TRUE );
	}
}
	...
	case IDM_OPTIONS:
		...
		else
		{
			theApp.m_dlgOptions.m_hWnd = ::CreateDialog( theApp.m_hInstance,
				MAKEINTRESOURCE(IDM_OPTIONS), HWND_DESKTOP, CStatusWnd::DefWindowProc );
		}
#endif
		break;
	case IDM_STATUS:
...
void NTWindow_SetWindowTitle( LPCTSTR pszText )
{
	...
	CGString sTitle;
	sTitle.Format( _TEXT("%s - %s (%s) %s"), theApp.m_pszAppName, g_Serv.GetName(), pszMode, pszText ? pszText : "" );

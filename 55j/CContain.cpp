...
void CItemContainer::OnOpenEvent( CChar * pCharOpener, const CObjBaseTemplate * pObjTop )
{
		...
		IsType(IT_EQ_VENDOR_BOX))
	{
		...
		{
			sMsg.Format( "You have %d stones in your %s", iStones, (LPCTSTR) GetName() );
		}
		else
		{
			sMsg.Format( "%s has %d stones in %s %s", pCharTop->GetPronoun(), iStones, (LPCTSTR) pCharTop->GetPossessPronoun(), (LPCTSTR) GetName() );
		}

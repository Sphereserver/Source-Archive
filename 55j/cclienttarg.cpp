...
bool CClient::OnTarg_Obj_Set( CObjBase * pObj )
{
	...
	// Parse the command.
	CGString sLogMsg;
	sLogMsg.Format( "'%s' commands uid=0%lx (%s) to '%s'", (LPCTSTR) GetName(), pObj->GetUID(), (LPCTSTR) pObj->GetName(), (LPCTSTR) m_Targ_Text );
	...
	case CV_TILE:
		{
			...
			for ( int mx = rect.m_left; mx <= rect.m_right; mx++)
			{
				for(int my = rect.m_top; my <= rect.m_bottom; my++)
				{
					if ( ++iArg >= iArgQty )
						iArg = 1;
					CItem * pItem = CItem::CreateTemplate( (ITEMID_TYPE) RES_GET_INDEX(piArgs[iArg]));
					ASSERT(pItem);
					pItem->SetAttr( ATTR_MOVE_NEVER );
...
int CClient::OnSkill_EvalInt( CGrayUID uid, int iSkillLevel, bool fTest )
{
	...
	SysMessagef( "%s looks %s.", (LPCTSTR) pChar->GetName(), (LPCTSTR) sm_szIntDesc[iIntEntry] );
...
bool CClient::OnTarg_Party_Add( CChar * pChar )
{
	...
	pChar->SysMessagef( "You have been invited to join %s's party. Type /accept to join or /decline to decline offer.", (LPCTSTR) m_pChar->GetName());

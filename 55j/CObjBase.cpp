...
bool GetDeltaStr( CPointMap & pt, TCHAR * pszDir )
{
	...
	int iTmp = Exp_GetVal((LPCSTR&)ppCmd[1] );

	if ( isdigit( chDir ) || chDir == '-' )
	{
		pt.m_x += Exp_GetVal((LPCSTR&)ppCmd[0] );
		pt.m_y += iTmp;
		pt.m_z += Exp_GetVal((LPCSTR&)ppCmd[2] );
	}
...
bool CObjBase::r_Verb( CScript & s, CTextConsole * pSrc ) // Execute command from script
{
	...
	case OV_EFFECT: // some visual effect.
		{
			...
			Effect( (EFFECT_TYPE) piCmd[0], (ITEMID_TYPE) RES_GET_INDEX(piCmd[1]),
				pCharSrc,
	...
	case OV_SPEAKUA:
		// This can have full args. SAYUA Color, Mode, Font, Lang, Text Text
		{
			...
			SpeakUTF8( pszArgs[4],
				(HUE_TYPE) ( pszArgs[0][0] ? Exp_GetVal((LPCSTR&)pszArgs[0]) : HUE_TEXT_DEF ),
				(TALKMODE_TYPE) ( pszArgs[1][0] ? Exp_GetVal((LPCSTR&)pszArgs[1]) : TALKMODE_SAY ),
				(FONT_TYPE) ( pszArgs[2][0] ? Exp_GetVal((LPCSTR&)pszArgs[2]) : FONT_NORMAL ),
	...
	case OV_SPELLEFFECT:	// spell, strength, noresist
		{
			...
			OnSpellEffect( (SPELL_TYPE) RES_GET_INDEX( piCmd[0] ), pCharSrc, piCmd[1], NULL );
		}
		break;

	...
	m_Socket( client )
{
	...
	m_ConnectType = CONNECT_UNK;	// don't know what sort of connect htis is yet.
	...
	{
		g_Log.Event( LOGM_CLIENTS_LOG, "%x:Client connected [Total:%i] from '%s'." DEBUG_CR,
...
CClient::~CClient()
{
	...
	if ( ! PeerName.IsSameIP( g_BackTask.m_RegisterIP ))
	{
		g_Log.Event( LOGM_CLIENTS_LOG, "%x:Client disconnected [Total:%i]" DEBUG_CR, m_Socket.GetSocket(), g_Serv.StatGet(SERV_STAT_CLIENTS));
	}
...
scp_hearall:
	...
	case CC_LISTEN:
		goto scp_hearall;
	case CC_PRIVHIDE:
		GetAccount()->TogPrivFlags( PRIV_PRIV_NOSHOW, s.GetArgStr() );
		break;
	case CC_PRIVSHOW:
		// Hide my priv title.
...
do_additem:
		if ( s.HasArgs())
		{
			...
			RESOURCE_ID rid = g_Cfg.ResourceGetID( RES_ITEMDEF, (LPCSTR&)pszArgs );
...
do_admin:
	...
	case CV_BANKSELF: // open my own bank
		...
		break;
	case CV_CAST:
		return Cmd_Skill_Magery( (SPELL_TYPE) g_Cfg.ResourceGetIndexType( RES_SPELL, s.GetArgStr()), dynamic_cast <CObjBase *>(pSrc));
	case CV_CHARLIST:
	...
	case CV_EXTRACT:
		...
		else
		{
			...
			m_tmTile.m_id = Exp_GetVal((LPCSTR&)ppArgs[1]);	// extract id.
	...
	case CV_HELP:
		...
		{
			Dialog_Setup( CLIMODE_DIALOG, g_Cfg.ResourceGetIDType( RES_DIALOG, IsPriv(PRIV_GM) ? "d_HELPGM" : "d_HELP" ), m_pChar );
		}
		else
		{

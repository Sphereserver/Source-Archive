...
CItem * CItem::CreateHeader( TCHAR * pArg, CObjBase * pCont, bool fDupeCheck )
{
	...
	RESOURCE_ID rid = g_Cfg.ResourceGetID( RES_ITEMDEF, (LPCSTR&)pArg );
	if ( ! rid.IsValidUID())
		return( NULL );
...
bool CItem::r_LoadVal( CScript & s ) // Load an item Script
{
	...
	case IC_HITPOINTS:
		...
		m_itArmor.m_Hits_Cur = m_itArmor.m_Hits_Max = s.GetArgRange();
		return true;
	case IC_ID:
...
TRIGRET_TYPE CItem::OnTrigger( LPCTSTR pszTrigName, CTextConsole * pSrc, CScriptTriggerArgs * pArgs )
{
	...
	if ( pChar )
	{
		...
		if ( pChar->m_pPlayer )
		{
			...
			for ( int i=0; dwPlot; i++, dwPlot >>= 1 )
			{
				...
				pResourceLink = dynamic_cast <CResourceLink *>( g_Cfg.ResourceGetDef( RESOURCE_ID( RES_PLOTITEM, i )));
				if ( pResourceLink == NULL )
				{
					DEBUG_ERR(( "0%x '%s' has unhandled [PLOTITEM %d]" DEBUG_CR, pChar->GetUID(), pChar->GetName(), i ));
	...
	if ( m_type >= IT_TRIGGER )
	{
		...
		if ( pResourceLink == NULL )
		{
			if ( pChar )
			{
				DEBUG_ERR(( "0%x '%s' has unhandled [TYPEDEF %d] for 0%x '%s'" DEBUG_CR, GetUID(), GetName(), GetType(), pChar->GetUID(), pChar->GetName()));
			}
			else
			{
				DEBUG_ERR(( "0%x '%s' has unhandled [TYPEDEF %d]" DEBUG_CR, GetUID(), GetName(), GetType() ));
			}

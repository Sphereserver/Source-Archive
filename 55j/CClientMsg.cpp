...
int CClient::addShopItems( CChar * pVendor, LAYER_TYPE layer )
{
	...
	for ( CItem* pItem = pContainer->GetContentHead(); pItem!=NULL; pItem = pItem->GetNext())
	{
		...
		int lenname = strcpylen( pCur->VendOpenBuy.m_item[0].m_text, pVendItem->GetName());
...
void CClient::addGumpDialog( CLIMODE_TYPE mode, const CGString * psControls, int iControls, const CGString * psText, int iTexts, int x, int y, CObjBase * pObj )
{
	...
	{
		CGString sTmp;
		sTmp.Format( "{%s}", (LPCTSTR) psControls[i] );
...
void CClient::addGumpDialogAdmin( int iAdPage )
{
	...
	for ( i=0; pClient && i<ADMIN_CLIENTS_PER_PAGE; pClient = pClient->GetNext())
	{
		...
		if ( pChar != NULL )
		{
			...
			CPointMap pt = pChar->GetTopPoint();
			sText[iTexts++].Format( "%s", PeerName.GetAddrStr()) ;
		...
		else
		{
			...
			sText[iTexts++] = accountName;
			sText[iTexts++] = "N/A";
			sText[iTexts++] = PeerName.GetAddrStr();
			sText[iTexts++] = "N/A";
		}
		i++;
	}
...
void CClient::addGumpDialogAdmin( int iAdPage )
{
	...
	addGumpDialog( CLIMODE_DIALOG_ADMIN, sControls, iControls, sText, iTexts, 0x10, 0x46 );
}
